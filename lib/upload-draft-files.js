/**
 * Keep picked audio + cover on this device through the submit flow.
 * Does not hop, attach, mint a store release, or resume a saved draft.
 */
(function (root, factory) {
  ...
  function coverInput(doc) {
    if (!doc || !doc.querySelector) return null;
    return doc.querySelector('[data-art-input]') || doc.getElementById('edit-art');
  }
  ...
  function collectFileMeta(win) {
    ...
    var cover = doc ? fileFromInput(coverInput(doc)) : null;
    return {
  ...
  function fileFromHeld(value) {
    ...
    if (isHeldRecord(value)) {
      var mime = value.type || (/\.(jpe?g|png)$/i.test(value.name || '') ? 'image/jpeg' : 'application/octet-stream');
      if (typeof Blob === 'function' && value.buffer) {
        try {
          var blob = new Blob([value.buffer], { type: mime });
          if (typeof File === 'function') {
            return new File([blob], value.name || 'cover.jpg', {
              type: mime,
              lastModified: value.lastModified || Date.now(),
            });
          }
          blob.name = value.name || 'cover.jpg';
          return blob;
        } catch (err) {}
      }
      return {
        name: value.name || 'cover.jpg',
        type: mime,
  ...
  function persistCoverFile(win, file) {
    win = win || (typeof globalThis !== 'undefined' ? globalThis : null);
    if (!file) return Promise.resolve(null);
    return openHold(win).then(function (db) {
      if (!db) return file;
      return putSlot(db, COVER_KEY, file);
    });
  }

  function deleteSlot(db, key) {
    return new Promise(function (resolve) {
      if (!db) {
        resolve(false);
        return;
      ...
      try {
        ...
        tx.objectStore(HOLD_STORE).delete(key);
      } catch (err) {
        resolve(false);
      }
    });
  ...
  function clearPickedAudio(win) {
    win = win || (typeof globalThis !== 'undefined' ? globalThis : null);
    return openHold(win).then(function (db) {
      if (!db) return true;
      return deleteSlot(db, AUDIO_KEY).then(function () {
        return deleteSlot(db, MASTER_KEY);
      });
    });
  }
  ...
  function persistPickedFiles(win) {
    ...
    return openHold(win).then(function (db) {
      ...
      function writeAudio() {
        ...
        return putSlot(db, AUDIO_KEY, audio).then(function () {
          return putSlot(db, MASTER_KEY, audio);
        });
      }
      return writeAudio().then(function () {
        return cover ? putSlot(db, COVER_KEY, cover) : cover;
      }).then(function () {
  ...
  return {
    ...
    persistCoverFile: persistCoverFile,
    persistPickedFiles: persistPickedFiles,
    clearPickedAudio: clearPickedAudio,

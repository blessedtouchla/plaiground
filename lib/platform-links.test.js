...
function run() {
  ...
  ['spotify', 'apple-music', 'amazon-music', 'deezer', 'tidal'].forEach(function (slug) {
    assert.ok(hints.indexOf(slug) !== -1, 'hint list includes ' + slug);
  });
  assert.ok(hints.indexOf('youtube-music') === -1, 'hint list must not offer YouTube Music');
  assert.ok(!platformLinks.platformList().some(function (row) { return row.slug === 'youtube-music'; }));
  const filtered = platformLinks.setCatalog([
    { slug: 'spotify', name: 'Spotify' },
    { slug: 'youtube-music', name: 'YouTube Music' },
    { slug: 'tidal', name: 'Tidal' },
  ]);
  assert.deepStrictEqual(filtered.map(function (row) { return row.slug; }), ['spotify', 'tidal']);
  platformLinks.setCatalog(null);

...
function hintCookieHeader(req, maxAge) {
  const parts = [
...
function attachSession(req, res, userId, options) {
  ...
  res.setHeader('Set-Cookie', [
    cookieHeader(signSession(userId, { remember }), req, ttl),
    hintCookieHeader(req, ttl),
  ]);
}
...
function publicUser(row) {
  ...
  return {
    ...
    profile: (function () {
      const stored = profile.seedFromAccount(profile.readStored(row), artist, row.tonegrid_artist_id);

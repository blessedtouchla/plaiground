...
const SIGNED_HINT = 'plaiground_signed';
const SESSION_TTL_SEC = 30 * 60;
const SESSION_REMEMBER_TTL_SEC = 30 * 24 * 60 * 60;
const COOKIE_SHARE_DOMAIN = '.wannaplai.com';
const COOKIE_SHARE_HOSTS = { 'wannaplai.com': true, 'www.wannaplai.com': true };
...
// Read-time staff/test allowlist. Gmail-normalized, same as findByEmail.
// Does not write a Stripe customer or subscription.
const STAFF_PRO_EMAIL = 'emailplaiground@gmail.com';
...
function hasStaffProOverride(email) {
  return emailsEquivalent(email, STAFF_PRO_EMAIL);
}

function applyStaffPlan(row) {
  if (!row || !hasStaffProOverride(row.email)) return row;
  if (normalizePlan(row.plan) === 'pro') return row;
  return Object.assign({}, row, { plan: 'pro' });
}
...
function normalizePaidPlan(value, email) {
  if (hasStaffProOverride(email)) return 'pro';
...
function requestHost(req) {
  ...
  const host = forwarded || headerValue(req, 'host');
  return host.split(':')[0].trim().toLowerCase();
}

function cookieDomain(req) {
  return COOKIE_SHARE_HOSTS[requestHost(req)] ? COOKIE_SHARE_DOMAIN : '';
}

function sessionTtl(remember) {
  return remember ? SESSION_REMEMBER_TTL_SEC : SESSION_TTL_SEC;
}

function signSession(userId, options) {
  const remember = Boolean(options && options.remember);
  const exp = Math.floor(Date.now() / 1000) + sessionTtl(remember);
  const data = { u: String(userId), e: exp };
  if (remember) data.r = 1;
...
function verifySession(token) {
  ...
  return { userId: data.u, exp: data.e, remember: data.r === 1 };
}
...
function cookieHeader(token, req, maxAge, options) {
  const parts = [
    COOKIE + '=' + (token || ''),
    'Path=/',
    'HttpOnly',
  ...
  const domain = options && Object.prototype.hasOwnProperty.call(options, 'domain')
...
function hintCookieHeader(req, maxAge, options) {
  const parts = [
    SIGNED_HINT + '=' + (Number(maxAge) > 0 ? '1' : ''),
    'Path=/',
    'SameSite=Lax',
    'Max-Age=' + String(maxAge),
    'Expires=' + cookieExpiresAt(maxAge),
...
function cookieHeaders(req, sessionValue, hintMaxAge, options) {
  const headers = [
    cookieHeader(sessionValue, req, hintMaxAge, options),
    hintCookieHeader(req, hintMaxAge, options),
  ];
  if (cookieDomain(req) && !(options && Object.prototype.hasOwnProperty.call(options, 'domain'))) {
    headers.push(cookieHeader('', req, 0, { domain: '' }));
    headers.push(hintCookieHeader(req, 0, { domain: '' }));
  }
  return headers;
}

function sessionRemember(session) {
  return Boolean(session && session.remember === true);
}

function attachSession(req, res, userId, options) {
  const remember = options && Object.prototype.hasOwnProperty.call(options, 'remember')
    ? Boolean(options.remember)
    : sessionRemember(sessionFromRequest(req));
  const ttl = sessionTtl(remember);
  res.setHeader('Set-Cookie', cookieHeaders(req, signSession(userId, { remember }), ttl));
}

function clearSession(req, res) {
  res.setHeader('Set-Cookie', cookieHeaders(req, '', 0));
}
...
function publicUser(row) {
  if (!row) return null;
  const view = applyStaffPlan(row);
  const releases = Array.isArray(view.tonegrid_release_ids) ? view.tonegrid_release_ids.filter(Boolean) : [];
  const tracks = Array.isArray(view.tonegrid_track_ids) ? view.tonegrid_track_ids.filter(Boolean) : [];
  const confirmed = isConfirmed(view);
  const quota = plans.evaluate(view);
  const artist = profile.displayArtistName(view.artist_name);
  const stored = profile.recoverRoster(profile.readStored(row), artist, row.tonegrid_artist_id);
  return {
    email: view.email,
    artist: artist,
    ...
    profile: Object.assign({}, stored, {
      artists: (stored.artists || []).map(function (item) {
        return profile.displayArtist(item);
      }),
    }),
    upload: {
      ...
      period: quota.period,
      plan: quota.plan,
      album_allowed: quota.album_allowed === true,
    },
  };
}

...
const SIGNED_RECIPIENT = new Set(['completed', 'manually completed', 'signed']);
const DOCUMENT_SENDER = 'document sender';
...
function collectErrorParts(value, out) {
  ...
  if (Array.isArray(value)) {
    value.forEach((item) => collectErrorParts(item, out));
    return;
  }
  if (typeof value === 'object') {
    if (typeof value.message === 'string' && value.message.trim()) out.push(value.message.trim());
    Object.keys(value).forEach((key) => {
      if (key === 'message') return;
      collectErrorParts(value[key], out);
    });
  }
}
...
function signwellErrorMessage(payload, fallback) {
  ...
  if (payload.error && typeof payload.error === 'object') collectErrorParts(payload.error, parts);
  if (typeof payload.message === 'string' && payload.message.trim()) parts.push(payload.message.trim());
  if (payload.errors != null) collectErrorParts(payload.errors, parts);
  if (payload.fields != null) collectErrorParts(payload.fields, parts);
  const unique = [];
  parts.forEach((part) => {
    if (part && unique.indexOf(part) === -1) unique.push(part);
  });
  if (unique.length) return scrub(unique.join(' '));
  return fallback || 'SignWell rejected the request.';
}
...
function isDocumentSenderRecipient(row) {
  return String((row && row.placeholder_name) || '').trim().toLowerCase() === DOCUMENT_SENDER;
}

function isSignedRecipient(row) {
  if (!row || typeof row !== 'object') return false;
  if (row.signed === true) return true;
  return SIGNED_RECIPIENT.has(String(row.status || row.signed_status || '').trim().toLowerCase());
}
...
function documentSigned(payload) {
  ...
  const recipients = recipientsOf(doc).filter((row) => !isDocumentSenderRecipient(row));
  if (!recipients.length) return false;
  return recipients.every(isSignedRecipient);
}

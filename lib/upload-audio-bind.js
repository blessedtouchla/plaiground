...
(function (root) {
  ...
  function lockFilePicker(input) {
    if (!input) return;
    try { input.removeAttribute('capture'); } catch (err) {}
    var helper = root.PlaigroundAudioAccept;
    var list = (helper && helper.ACCEPT) || '.wav,.flac,.mp3,.mpeg,.mpga,audio/wav,audio/x-wav,audio/flac,audio/x-flac,audio/mpeg,audio/mp3,audio/x-mpeg,audio/x-mp3,audio/mpeg3,audio/mpg';
    if (/audio\/\*|image|\*\/\*/i.test(list)) {
      list = '.wav,.flac,.mp3,.mpeg,.mpga,audio/wav,audio/x-wav,audio/flac,audio/x-flac,audio/mpeg,audio/mp3,audio/x-mpeg,audio/x-mp3,audio/mpeg3,audio/mpg';
    }
    try { input.setAttribute('accept', list); } catch (err2) {}
  }

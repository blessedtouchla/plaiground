...
function run() {
  ...
  assert.strictEqual(status.isReturnedRelease({
    uuid: '6629b532-2e78-4be6-84eb-e4dfa9ac33e5',
    title: 'Metete en el groove',
    status: 'draft',
  }), true, 'Metete lock id stays returned even when the store still says draft');
  assert.ok(!status.isUnsubmittedDraft({
    uuid: '6629b532-2e78-4be6-84eb-e4dfa9ac33e5',
    title: 'Metete en el groove',
    status: 'draft',
  }), 'Metete lock id is not dropped as an unsubmitted draft');
  assert.strictEqual(status.isReturnedRelease({
  ...
  assert.strictEqual(status.label('pending'), 'PLAIGROUND QC');
  assert.strictEqual(status.label('pending_review'), 'PLAIGROUND QC');
  ...
  assert.strictEqual(status.label('qc_inspection'), 'Platform QC');
  assert.strictEqual(status.group('qc_inspection'), 'platform_qc');
  assert.strictEqual(status.dot('qc_inspection'), 'yellow');
  ...
  assert.strictEqual(status.label('mystery'), 'PLAIGROUND QC');
  ...
  assert.strictEqual(status.displayInfo(credited).label, 'PLAIGROUND QC');
  assert.strictEqual(status.problemAlert({ status: 'pending', label: 'PLAIGROUND QC' }), '', 'pending stays quiet');
  ...
  assert.strictEqual(status.pendingCount({
    tonegrid_release_ids: ['aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'],
    profile: { releases: [{ tonegrid_release_id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', tonegrid_status: 'qc_inspection' }] },
  }), 1, 'Platform QC stays in the in-progress count');
  assert.strictEqual(status.pendingCount({
    tonegrid_release_ids: [],
  ...
  assert.strictEqual(pendingCards[0].label, 'PLAIGROUND QC');
  ...
  assert.strictEqual(mixed[0].label, 'PLAIGROUND QC');
  ...
  assert.strictEqual(deliveredApproved.out, 'Out now', 'live without a street date says Out now');
  assert.strictEqual(status.displayInfo({
    status: 'live',
    release_date: '2020-01-01',
  }).out, 'Out now', 'past street date is Out now');
  assert.strictEqual(status.displayInfo({
    status: 'approved',
    delivered_at: '2026-09-08T16:10:57Z',
    release_date: '2026-12-25',
  }).out, 'Out Dec 25', 'future street date is Out MMM D in America/Los_Angeles');
  assert.strictEqual(status.displayInfo({ status: 'pending', release_date: '2026-12-25' }).out, '', 'non-live has no Out subline');
  assert.strictEqual(status.displayInfo({ status: 'qc_inspection', release_date: '2026-12-25' }).out, '');
  assert.strictEqual(status.displayInfo({ status: 'approved', release_date: '2026-12-25' }).out, '', 'Processing has no Out subline');
  assert.strictEqual(status.outLine({ status: 'live', release_date: '2026-12-25' }), 'Out Dec 25');
  ...
  assert.strictEqual(status.displayInfo({ status: 'pending' }).label, 'PLAIGROUND QC');
  assert.strictEqual(status.displayInfo({ status: 'pending', delivered_at: '' }).label, 'PLAIGROUND QC', 'empty delivered_at does not flip pending');
  assert.strictEqual(status.displayInfo({ status: 'qc_inspection' }).label, 'Platform QC', 'qc_inspection is Platform QC not Processing');
  assert.strictEqual(status.displayInfo({ status: 'qc_inspection' }).group, 'platform_qc');
  assert.strictEqual(status.displayInfo({ status: 'qc_inspection' }).dot, 'yellow');
  assert.strictEqual(status.displayInfo({ status: 'qc_inspection', delivered_at: '' }).label, 'Platform QC');
  assert.strictEqual(status.displayInfo({
    status: 'processing',
  ...
  assert.strictEqual(deliveredCard.release_date, '');
  assert.strictEqual(deliveredCard.updated_at, '');
  ...
  assert.strictEqual(unknownCard.label, 'PLAIGROUND QC');
  ...
  assert.strictEqual(idOnly[0].label, 'PLAIGROUND QC');

...
function run() {
  ...
  assert.ok(!html.includes('site.css?v=20260917r1'), 'r1 stamp is retired after the sticky/soft-copy leftover');
  ...
  assert.ok(/\.review-page \.review-actions[\s\S]*?background:\s*#000/.test(cut), 'sticky bar is a solid dock, not a fade over Stores');
  assert.ok(/body\.review-page main\.page[\s\S]*?padding-bottom:\s*168px/.test(cut), 'page clears the sticky Submit dock');
  ...
  assert.ok(submitted.includes('site.css?v=20260917s5'), 'Submitted Tesla stamp is not reverted');

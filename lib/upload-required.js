...
function trackHasStoreAudio(track) {
  ...
  var status = String(track.audio_status || '').toLowerCase();
  if (status === 'processing' || status === 'uploaded' || status === 'ready' || status === 'ok') {
    return true;
  }
...
function audioAttachAccepted(payload) {
  if (!payload || typeof payload !== 'object') return false;
  if (payload.error || payload.ok === false) return false;
  var row = payload.track && typeof payload.track === 'object' ? payload.track : payload;
  return trackHasStoreAudio(row) || trackHasStoreAudio(payload);
}
...
function validateSplit(payload) {
  ...
  if (!Array.isArray(writers) || !writers.length) {
    return { error: 'Writer 1 needs a legal first and last name.' };
  }
  for (var i = 0; i < writers.length; i += 1) {
    var writer = writers[i] || {};
    ...
    if (!first || !last) {
      return { error: 'Writer ' + (i + 1) + ' needs a legal first and last name.' };
    }
...
var api = {
  ...
  audioAttachAccepted: audioAttachAccepted,

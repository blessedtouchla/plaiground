...
function liveArtFile() {
  try {
    if (typeof document === 'undefined' || !document.querySelector) return null;
    var input = document.querySelector('[data-art-input]');
    var file = (input && input.files && input.files[0]) || (input && input._plaigroundFile) || null;
    if (file && Number(file.size) > 0) return file;
  } catch (err) {}
  return null;
}
...
function hasPersistedAudio(fields) {
  ...
  if (filled(fields.track_id)) return true;
  return false;
}
...
function hasPersistedArtwork(fields) {
  ...
  if (filled(fields.artwork_name) || filled(fields.artworkName)) return true;
  if (filled(fields.artwork_object_key) || filled(fields.artworkObjectKey)) return true;
  if (filled(fields.artwork_url) || filled(fields.cover_url) || filled(fields.image_url)) return true;
  if (liveArtFile()) return true;
  return false;
}

function rowHasAudio(body, row) {
  if (hasPersistedAudio(body) || hasPersistedAudio(row)) return true;
  if (filled(body && (body.audio_object_key || body.audioObjectKey))) return true;
  if (filled(row && (row.audio_object_key || row.audio_url))) return true;
  var tracks = (row && row.tracks) || (body && body.tracks) || [];
  if (!Array.isArray(tracks)) return false;
  for (var i = 0; i < tracks.length; i += 1) {
    var track = tracks[i] || {};
    if (filled(track.uuid || track.id || track.audio_object_key || track.audio_url)) return true;
  }
  return false;
}

function rowHasArtwork(body, row) {
  if (hasPersistedArtwork(body) || hasPersistedArtwork(row)) return true;
  if (filled(body && (body.artwork_object_key || body.artworkObjectKey || body.artwork_name || body.artworkName || body.artwork_url || body.cover_url))) return true;
  if (filled(row && (row.artwork_object_key || row.artwork_url || row.cover_url || row.image_url || row.artwork_name))) return true;
  if (liveArtFile()) return true;
  return false;
}
...
function validateSubmit(body, row) {
  body = body || {};
  row = row || {};
  var title = trim(body.title || body.songTitle || body.song_title || row.title);
  ...
  if (!date) return { error: 'Release date is required.' };
  if (!rowHasArtwork(body, row)) return { error: 'Artwork is required before send.' };
  if (!rowHasAudio(body, row)) return { error: 'Audio is required before send.' };
  if (!writerNameOf(body, row)) return { error: 'Songwriter name is required.' };
  ...
  return {
    ...
    title: title,
    release_date: date,
    writer_name: writerNameOf(body, row),
  };
}

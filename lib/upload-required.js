...
function validateAttest(fields) {
  ...
  if (madeHow === 'ai_assisted') {
    // Chips-only is enough. Describe text is a fallback if no chip is picked.
...
function validateSplit(payload) {
  ...
  for (var i = 0; i < writers.length; i += 1) {
    var writer = writers[i] || {};
    var credits = creditsApi();
    var hasLegal = credits && credits.writerHasLegalName
      ? credits.writerHasLegalName(writer)
      : filled(writer.name);
    if (!hasLegal && !filled(writer.name)) return { error: 'Writer ' + (i + 1) + ' needs a name.' };
    if (!hasLegal) return { error: 'Writer ' + (i + 1) + ' needs a first and last name.' };

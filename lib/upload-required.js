...
function rowHasArtwork(body, row) {
  ...
  if (filled(body && (body.artwork_object_key || body.artworkObjectKey))) return true;
  if (filled(row && (row.artwork_object_key || row.artwork_url || row.cover_url || row.image_url))) return true;
  return false;
}

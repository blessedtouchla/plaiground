 ...
 * Draft/pending can preview the uploaded file via audio_url / object key.
 */
...
function previewAudioOf(release) {
  ...
  var first = tracks[0] || {};
  var url = httpUrl(row.audio_url || row.preview_url || row.track_url || first.audio_url || first.preview_url);
  var key = trim(row.audio_object_key || row.audioObjectKey || first.audio_object_key || first.audioObjectKey);
...
function paintPreview(host, release, note) {
  ...
  if (!src.url && !src.key) return false;

 ...
 * Draft/pending can preview the uploaded file via audio_url / object key / held file.
 */
...
var PREVIEW_COPY = 'Preview of the uploaded file.';
...
function hopApi() {
  ...
  try {
    if (typeof window !== 'undefined' && window.PlaigroundObjectHop) return window.PlaigroundObjectHop;
  } catch (err2) {}
  return null;
}

function leftoverDraft(release) {
  try {
    var raw = typeof localStorage !== 'undefined' ? localStorage.getItem('plaiground.store.draft') : '';
    var draft = raw ? JSON.parse(raw) : null;
    if (!draft || typeof draft !== 'object') return null;
    var want = trim((release && (release.uuid || release.id || release.tonegrid_release_id)) || '').toLowerCase();
    var got = trim(draft.release_id || draft.tonegrid_release_id || '').toLowerCase();
...
function previewAudioOf(release) {
  var row = release && typeof release === 'object' ? release : {};
  var tracks = Array.isArray(row.tracks) ? row.tracks : [];
  var first = tracks[0] || {};
  var draft = leftoverDraft(row) || {};
  var url = httpUrl(row.audio_url || row.preview_url || row.track_url || first.audio_url || first.preview_url || draft.audio_url);
  var key = trim(row.audio_object_key || row.audioObjectKey || first.audio_object_key || first.audioObjectKey || draft.audio_object_key);
  return { url: url, key: key };
}

function attachHeldPreview(audio, note) {
  var files;
  try {
    files = (typeof PlaigroundUploadDraftFiles !== 'undefined' && PlaigroundUploadDraftFiles)
      || (typeof window !== 'undefined' && window.PlaigroundUploadDraftFiles);
  } catch (err) {
    files = null;
  }
  if (!files || typeof files.keepHeldFiles !== 'function') return false;
  files.keepHeldFiles(typeof window !== 'undefined' ? window : undefined).then(function (held) {
    var file = held && held.audio;
    if (!file) return;
    try {
      audio.src = URL.createObjectURL(file);
      note.textContent = PREVIEW_COPY;
    } catch (err2) {}
  });
  return true;
}
...
var PARTNER_RE = /tonegrid|interspace|distrokid/i;
var LANDED = { live: true, delivered: true, success: true, landed: true, completed: true };
var FAILED = {
  failed: true,
  fail: true,
  error: true,
...
function prettyStoreName(value, fallback) {
  ...
  if (PARTNER_RE.test(slug)) return fallback || '';
...
function destinationName(row) {
  ...
  var raw = trim(row.dsp_name || row.name || row.dsp || row.dsp_slug || '');
  if (!raw || PARTNER_RE.test(raw)) return '';
  return prettyStoreName(raw, '');
}

function deliveryLabel(status) {
  var s = slugOf(status).replace(/-/g, '_');
  if (!s) return '';
  if (LANDED[s]) return 'Landed';
  if (FAILED[s]) return 'Failed';
  if (s === 'processing' || s === 'delivering' || s === 'approved' || s === 'qc_inspection') return 'Processing';
...
function pickDeliveries(payload) {
  ...
  list.forEach(function (row) {
    ...
    var name = destinationName(row) || destinationName(parsed) || prettyStoreName(row.dsp || row.dsp_slug, '');
    var item = {
      dsp: slugOf(row.dsp || row.dsp_slug || (parsed && parsed.dsp)),
      dsp_name: name,
      status: slugOf(row.status || row.delivery_status),
      label: deliveryLabel(row.status || row.delivery_status),
    ...
    if (!item.dsp_name && !item.dsp && !item.dsp_release_id && !item.store_url) return;
    if (!item.dsp_name && item.dsp) item.dsp_name = prettyStoreName(item.dsp, '');
    if (!item.dsp_name) return;
...
function paintPreview(host, release, note) {
  var src = previewAudioOf(release);
  var audio = document.createElement('audio');
  audio.controls = true;
  audio.preload = 'metadata';
  audio.className = 'owner-audio';
  audio.setAttribute('data-owner-audio', '');
  if (src.url) {
    audio.src = src.url;
  } else if (src.key) {
    var hop = hopApi();
    if (hop && typeof hop.previewUrl === 'function') {
      hop.previewUrl(src.key).then(function (href) {
        if (href) audio.src = href;
        else attachHeldPreview(audio, note);
      });
    } else if (!attachHeldPreview(audio, note)) {
      return false;
    }
  } else if (!attachHeldPreview(audio, note)) {
    return false;
  ...
  host.className = (host.className || '').replace(/\bis-wait\b/g, '').trim() + ' is-preview';
  host.setAttribute('data-owner-player', 'preview');
  return true;
}
...
function mount(host, release, opts) {
  ...
  clearHost(host);
  host.className = (host.className || '').replace(/\bis-live\b|\bis-wait\b|\bis-preview\b/g, '').trim() + (info.live && info.links.length ? ' is-live' : ' is-wait');
  ...
  if (!info.live) {
    if (paintPreview(host, release, note)) return info;
...
function mountStoreStatus(host, release) {
  if (!host) return [];
  host.textContent = '';
  var rows = pickDeliveries(release && release.deliveries);
  rows.forEach(function (row) {
    var line = document.createElement('div');
    line.className = 'song-store-status-row';
    var name = document.createElement('span');
    name.textContent = row.dsp_name || prettyStoreName(row.dsp, 'Store');
    var st = document.createElement('span');
    st.textContent = row.label || deliveryLabel(row.status) || 'Not assigned';
    if (row.label === 'Landed' || LANDED[slugOf(row.status).replace(/-/g, '_')]) st.className = 'live';
    if (row.label === 'Failed' || FAILED[slugOf(row.status).replace(/-/g, '_')]) st.className = 'wait';
...
var api = {
  ...
  mountStoreStatus: mountStoreStatus,
  parseDelivery: parseDelivery,
  pickDeliveries: pickDeliveries,
  prettyStoreName: prettyStoreName,
  previewAudioOf: previewAudioOf,
  state: state,
};

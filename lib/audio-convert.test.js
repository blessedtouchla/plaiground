...
async function run() {
  ...
  assert.match(String(passed.contentType || ''), /multipart\/form-data/i);
  assert.doesNotMatch(String(passed.contentType || ''), /application\/json/i);
  ...
  const hoppedWav = Buffer.from('RIFF....WAVE');
  const hopped = await convert.prepareFromBytes(hoppedWav, 'song.wav', 'application/octet-stream');
  assert.ok(!hopped.error, hopped.error);
  assert.strictEqual(hopped.converted, false);
  assert.strictEqual(hopped.kind, 'wav');
  const hoppedHead = hopped.rawBody.slice(0, 400).toString('latin1');
  assert.match(hoppedHead, /name="audio"/);
  assert.match(hoppedHead, /filename="song.wav"/i);
  assert.match(hoppedHead, /Content-Type: audio\/wav/i);
  assert.doesNotMatch(hoppedHead, /application\/octet-stream/i);
  assert.ok(hopped.rawBody.indexOf(hoppedWav) !== -1);
  assert.match(String(hopped.contentType || ''), /multipart\/form-data/i);

  const hoppedFlac = await convert.prepareFromBytes(Buffer.from('fLaC....'), 'song.flac', 'application/octet-stream');
  assert.ok(!hoppedFlac.error, hoppedFlac.error);
  assert.strictEqual(hoppedFlac.kind, 'flac');
  assert.strictEqual(hoppedFlac.converted, false);
  assert.match(hoppedFlac.rawBody.slice(0, 400).toString('latin1'), /Content-Type: audio\/flac/i);
  ...
  assert.strictEqual(typeof convert.convertUploadAudio, 'function');
  const passedWav = await convert.convertUploadAudio({ name: 'master.wav', type: 'audio/wav', size: 64 });
  assert.strictEqual(passedWav.name, 'master.wav', 'browser hook must leave WAV alone');
  const pcm = convert.pcmToWavBytes([new Float32Array([0, 0.5, -0.5, 0])], 44100);
  assert.ok(pcm.byteLength > 44);

...
async function run() {
  ...
  await withEnv({
    ...
    try {
      ...
      calls.forEach((call) => {
        ...
        assert.ok(call.url.indexOf('https://api.stripe.com/v1/subscriptions') === 0);
        assert.ok(!call.body);
        assert.ok(!/\/charges|\/checkout\/sessions|payment_intents/.test(call.url));
      });
    } finally {
  ...
  assert.ok(!/>Admin</.test(adminHtml), 'owner page must not add an Admin menu item');
  ...
  assert.ok(adminHtml.includes('data-new-release data-signed-in-upload>New release</a>'));

...
const { listSignupRows, signupRow, signupRowsToCsv } = require('./admin-signups');
...
async function run() {
  ...
  await withEnv({
    ...
    listed.forEach((row) => {
      ...
      assert.ok(typeof row.email_confirmed_at === 'string');
      assert.ok(!row.email_confirmed_at || /^\d{4}-\d{2}-\d{2}T/.test(row.email_confirmed_at));
      assert.ok(!Object.prototype.hasOwnProperty.call(row, 'username'));
      assert.ok(!Object.prototype.hasOwnProperty.call(row, 'legal_name'));
      assert.ok(!/last.?login|phone|ipi|password|stripe_customer|session/i.test(JSON.stringify(row)));
    ...
    const csvAnon = await adminApi(adminReq('', { url: '/api/admin/signups.csv' }));
    assert.strictEqual(csvAnon.statusCode, 401);
    assert.ok(!/text\/csv/i.test(String(csvAnon.headers['Content-Type'] || '')));

    const csvVictoria = await adminApi(adminReq(victoria.cookie, { url: '/api/admin/signups.csv' }));
    assert.strictEqual(csvVictoria.statusCode, 403);
    assert.strictEqual(json(csvVictoria).error, 'Not allowed.');
    assert.ok(!csvVictoria.body.includes('later@example.com'));

    const csvOwner = await adminApi(adminReq(owner.cookie, { url: '/api/admin/signups.csv' }));
    assert.strictEqual(csvOwner.statusCode, 200);
    assert.strictEqual(csvOwner.headers['Content-Type'], 'text/csv; charset=utf-8');
    assert.ok(/plaiground-signups\.csv/.test(String(csvOwner.headers['Content-Disposition'] || '')));
    const csvLines = csvOwner.body.split(/\r?\n/).filter(Boolean);
    assert.strictEqual(csvLines[0], 'email,name,plan,status,signed_up_at,stripe,email_confirmed_at');
    assert.ok(csvLines[1].indexOf('stranger@example.com') === 0, 'CSV newest signups first');
    assert.ok(csvOwner.body.includes('later@example.com'));
    assert.ok(csvOwner.body.includes('emailplaiground@gmail.com'));
    assert.ok(!/password|stripe_customer|sk_live_|sk_test_|SESSION_SECRET|ToneGrid/i.test(csvOwner.body));

    const csvQuery = await adminApi(adminReq(owner.cookie, {
      url: '/api/me?action=admin-signups&format=csv',
    }));
    assert.strictEqual(csvQuery.statusCode, 200);
    assert.strictEqual(csvQuery.headers['Content-Type'], 'text/csv; charset=utf-8');
    assert.ok(csvQuery.body.indexOf('email,name,plan,status,signed_up_at,stripe,email_confirmed_at') === 0);
    ...
    const named = signupRow({
      ...
      email_confirmed_at: '2026-08-27T00:20:00.000Z',
    ...
    assert.strictEqual(named.email_confirmed_at, '2026-08-27T00:20:00.000Z');
    ...
    const quoted = signupRowsToCsv([{
      email: 'comma@example.com',
      name: 'Ada, "Night"',
      plan: 'basic',
      status: 'active',
      signed_up_at: '2026-08-27T00:16:00.000Z',
      stripe: 'no',
      email_confirmed_at: '',
    }]);
    assert.ok(quoted.indexOf('email,name,plan,status,signed_up_at,stripe,email_confirmed_at') === 0);
    assert.ok(quoted.includes('"Ada, ""Night"""'));
    assert.ok(!/password|stripe_customer|sk_live_|ToneGrid|distributor/i.test(quoted));
  ...
  assert.ok(!/ToneGrid|distributor/i.test(adminHtml.match(/id="signups"[\s\S]*?<\/section>/)[0]));
  ...
  assert.ok(vercel.includes('/api/admin/signups.csv'));
  assert.ok(vercel.includes('format=csv'));
  ...
  assert.ok(
    !fs.existsSync(path.join(__dirname, '..', 'api', 'admin-signups.js')),
    'admin signups stay on the existing /api/me Hobby function'
  );
  assert.ok(meSrc.includes('signups.csv') && meSrc.includes('format'));
  assert.ok(meSrc.includes('text/csv'));

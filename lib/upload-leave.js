...
(function (root, factory) {
  ...
  var CANCEL_CONFIRM = 'Cancel this release? This drops the draft.';
  ...
  var TEXT_IDS = [
    ...
    'tg-legal-first',
    'tg-legal-last',
    'tg-legal-first-create',
    'tg-legal-last-create',
  ];
  var DRAFT_MARKS = [
    'title',
    'name',
  ...
  function removeDraft(win) {
    try {
      if (win.localStorage && typeof win.localStorage.removeItem === 'function') {
        ...
        win.localStorage.removeItem(HELD_KEY);
      }
    } catch (err) {}
    try {
      if (win.sessionStorage && typeof win.sessionStorage.removeItem === 'function') {
        ...
        win.sessionStorage.removeItem(HELD_KEY);
        win.sessionStorage.removeItem(FRESH_KEY);
      }
    } catch (err2) {}
    try {
  ...
  function go(win, href) {
    try {
      if (win.location && typeof win.location.replace === 'function') win.location.replace(href);
  ...
  function confirmCancel(win) {
    if (!hasUnsaved(win)) return true;
    try {
      if (typeof win.confirm === 'function') return Boolean(win.confirm(CANCEL_CONFIRM));
    } catch (err) {}
    return true;
  }
  ...
  function leaveToOverview(event, win) {
    ...
    if (!confirmCancel(win)) return false;
    wipeForm(win);
  ...
  function startOver(event, win) {
    ...
    removeDraft(win);
    try {
      if (win.sessionStorage && typeof win.sessionStorage.setItem === 'function') {
        win.sessionStorage.setItem(FRESH_KEY, '1');
      }
    } catch (err) {}

...
function teslaCutCss(css) {
  ...
  const end = css.indexOf('/* —— Submitted Tesla energy');
...
function run() {
  ...
  const leftoverEnd = css.indexOf('/* —— Upload Credits Tesla cut');
  const leftover = leftoverStart === -1 ? '' : (leftoverEnd > leftoverStart ? css.slice(leftoverStart, leftoverEnd) : css.slice(leftoverStart));
  ...
  assert.ok(!upload.includes('store-client.js?v=' + STAMP), 'cr2 store-client stamp is retired after the iOS Files picker leftover');

...
function run() {
  ...
  assert.strictEqual(missing.name, 'Fuvtu');
  assert.ok(!missing.error, 'collected artist/songwriter name is enough to stamp credits');
  ...
  assert.strictEqual(qc.contributors[2].role, 'Performer');
  assert.strictEqual(qc.contributors[3].role, 'Producer');
  ...
  assert.strictEqual(declared[0].name, 'Ada Night');
  assert.strictEqual(declared[0].first_name, 'Ada');
  assert.strictEqual(declared[0].last_name, 'Night');
  assert.deepStrictEqual(storeCredits.declaredTrackContributors({
    credits: { performer: 'AI', producer: '' },
    name: 'Fuvtu',
  }), [], 'do not invent people or send AI as a performer');
  const listed = storeCredits.declaredTrackContributors({
    contributors: [{ name: 'Ada Night', role: 'engineer' }],
  });
  assert.strictEqual(listed[0].role, 'Engineer');
  const roleMerge = storeCredits.mergeTrackContributors(
    { first: 'Ada', last: 'Night', name: 'Ada Night' },
    { credits: { performer: 'Ada Night', producer: 'Ada Night' } }
  );
  assert.ok(roleMerge.contributors.some((row) => row.role === 'Songwriter'));
  assert.ok(roleMerge.contributors.some((row) => row.role === 'Performer' && row.name === 'Ada Night'));
  assert.ok(roleMerge.contributors.some((row) => row.role === 'Producer' && row.name === 'Ada Night'));
  assert.strictEqual(storeCredits.mergeTrackContributors({ name: '' }, {}), null);

...
function run() {
  assert.strictEqual(storeCredits.STORE_LABEL, 'PLAIGROUND');
  ...
  const lines = storeCredits.copyrightLines('Jane Doe', 2026);
  assert.strictEqual(lines.c_line, '(C) 2026 Jane Doe');
  assert.strictEqual(lines.p_line, '(P) 2026 Jane Doe');
  assert.strictEqual(lines.copyright_line, '© 2026 Jane Doe / ℗ 2026 Jane Doe');
  ...
  const rights = storeCredits.rightsEnvelope('Jane Doe', 2026);
  assert.strictEqual(rights.p_line, '(P) 2026 Jane Doe');
  ...
  const stored = storeCredits.storedCreditFields('Ada Night', 2026);
  ...
  const writer = storeCredits.writerCreateBody('Ada Night');
  ...
  assert.strictEqual(saved.releases[0].label, 'PLAIGROUND');
  assert.strictEqual(saved.releases[0].rights_owner, 'Ada Night');
  const merged = profile.upsertRelease(saved, {
    id: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb',
    writers: [{ name: 'Ada Night' }],
  });
  assert.strictEqual(merged.releases[0].label, 'PLAIGROUND');

...
function run() {
  ...
  assert.ok(upload.includes('site.css?v=20260912ca2'), 'Upload leftover cache-busts site.css at 20260912ca2');
  ...
  assert.ok(!leftover.includes('var(--stage-blue)'), 'Logo Blue chip paint stays off Credits/Send');
  assert.ok(/\.chip-credits\.is-on,[\s\S]*?color:\s*var\(--muted-2\)/.test(leftover), 'is-on stays a muted highlight');

...
async function run() {
  ...
  assert.strictEqual(adminStatusLabel('pending'), 'PLAIGROUND QC');
  assert.strictEqual(adminStatusLabel('pending_review'), 'PLAIGROUND QC');
  assert.strictEqual(adminStatusLabel('qc_inspection'), 'Platform QC');
  ...
  await withEnv({
    ...
    const qcLines = releaseStatus.STORE_QC_LINES.join('\n');
    const moon = listed.submissions.find((row) => row.title === 'Too the moon');
    assert.strictEqual(moon.status, 'PLAIGROUND QC');
    assert.strictEqual(moon.alert, '', 'pending house review does not invent store QC lines');
    ...
    assert.ok(body.events.some((row) => row.email === 'paid@example.com' && row.event === 'signup'));
  });
  ...
  assert.ok(adminHtml.includes('data-events-body'));
  ...
  assert.ok(/href="https:\/\/app\.tonegrid\.pro\/super\/login" target="_blank" rel="noopener noreferrer">ToneGrid dashboard<\/a>/.test(ownerNav), 'owner desk links the store partner dashboard');
  assert.ok(/href="dashboard.html">Dashboard<\/a>/.test(ownerNav), 'owner desk links the signed-in artist home');
  assert.ok(!/InterSpace|DistroKid/i.test(adminHtml));
  assert.ok(!adminHtml.includes('<iframe'));
  assert.ok(/>Admin</.test(adminHtml.match(/<nav class="side-nav"[\s\S]*?<\/nav>/)[0]), 'owner desk marks Admin as home');

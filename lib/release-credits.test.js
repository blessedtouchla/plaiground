...
function run() {
  ...
  assert.ok(credits.validateUploadLegal({
    ...
    legal_first: '',
    legal_last: '',
  }).ok, 'upload create does not require legal names — those live on Artist Profiles');
  assert.ok(credits.validateUploadLegal({ artist_mode: 'create' }).ok, 'empty create pair is filled-only on upload');
  assert.ok(credits.validateUploadLegal({ creating_artist: true }).ok, 'creating_artist without names is not an upload gate');
  ...
  assert.strictEqual(credits.validateAttestExtras({
    solo_owned_100: true,
    legal_first: '',
    legal_last: '',
  }).error, credits.WRITER_LINE);
  assert.ok(credits.validateAttestExtras({
    legal_first: 'Ada',
    legal_last: 'Night',
  }).ok);
  ...
  const legalRoster = [{ id: 'act-2', name: 'Night Drive', legal_first: '', legal_last: '' }];
  const legalWin = {
    localStorage: store,
    sessionStorage: store,
    PlaigroundMembership: {
      account() {
        return { profile: { artists: legalRoster } };
      },
    },
    fetchCalls: [],
    fetch(url, opts) {
      legalWin.fetchCalls.push({ url: url, opts: opts });
      return Promise.resolve({ ok: true, json: function () { return Promise.resolve({}); } });
    },
  };
  credits.rememberLegal('Ada', 'Night', legalWin);
  assert.strictEqual(credits.rememberedLegal(legalWin).first, 'Ada');
  assert.strictEqual(credits.rememberedLegal(legalWin).last, 'Night');

  assert.strictEqual(credits.savedDraftRelease({ title: 'Held' }), null);
  assert.strictEqual(credits.savedDraftRelease({ saved_draft: true, title: 'The Interceptors' }), null, 'saved-draft resume is cancelled');
  ...
  assert.strictEqual(mixed[0].title, 'Rainbow Road');
  assert.strictEqual(mixed[0].status, 'live');
  assert.ok(!mixed.some(function (row) { return row.id === 'local-draft' || row.local_draft; }));
  ...
  const storeDraft = credits.withSavedDraft([
    { uuid: 'eeeeeeee-eeee-4eee-8eee-eeeeeeeeeeee', title: 'Unicorn in West Hollywood', status: 'draft' },
    { uuid: '7a928125-aaaa-4aaa-8aaa-aaaaaaaaaaaa', title: 'Rainbow Road', status: 'live' },
  ], {});
  assert.strictEqual(storeDraft.length, 1, 'unsubmitted store drafts do not resume');
  assert.strictEqual(storeDraft[0].title, 'Rainbow Road');
  ...
  assert.strictEqual(keptCatalog.length, 6);
  assert.ok(!keptCatalog.some(function (row) { return row.id === 'local-draft' || row.local_draft; }));
  ...
  assert.strictEqual(credits.parkSavedDraft(win), false, 'saved draft is not parked for resume');
  assert.ok(credits.dropLeftoverSavedDraft(win), 'leftover saved draft is dropped');
  credits.clearWorkingDraft(win);
  assert.strictEqual(credits.displayDraft(win).title, undefined);
  assert.strictEqual(credits.readDraft(win).title, undefined);

  return credits.writeLegalToArtist('act-2', 'Ada', 'Night', legalWin).then(function () {
    assert.strictEqual(legalWin.PlaigroundMembership.account().profile.artists[0].legal_first, 'Ada');
    assert.strictEqual(legalWin.PlaigroundMembership.account().profile.artists[0].legal_last, 'Night');
    assert.ok(legalWin.fetchCalls.some(function (call) {
      return String(call.url) === '/api/me/artists' && !/tonegrid/i.test(String(call.url));
    }), 'missing artist legal names persist to the same artist store');
    legalWin.fetchCalls.length = 0;
    return credits.writeLegalToArtist('act-2', 'Bea', 'Vale', legalWin);
  }).then(function () {
    assert.strictEqual(legalWin.PlaigroundMembership.account().profile.artists[0].legal_first, 'Ada', 'do not overwrite an artist that already has legal names');
    assert.strictEqual(legalWin.fetchCalls.length, 0, 'second write stays off hop and off a second name store');

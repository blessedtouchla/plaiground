...
const CSS_STAMP = '20260924p3';
const JS_STAMP = '20260924p3';
...
function run() {
  ...
  assert.ok(!html.includes('site.css?v=20260924w1'), 'w1 stamp is retired after the pricing story');
  assert.ok(!html.includes('site.css?v=20260924p1'), 'p1 stamp is retired after the locked prices');
  assert.ok(!html.includes('site.css?v=20260924p2'), 'p2 stamp is retired after the pricing ladder');
  assert.ok(!html.includes('site.css?v=20260918n1'), 'n1 stamp is retired after the visible-bar fix');
  assert.ok(!html.includes('site.css?v=20260918h3'), 'h3 stamp is retired after the public nav pass');
  ...
  assert.ok(/hero-siqa-kicker">SIQA Charts</.test(hero[0]), 'SIQA Charts stays the strip header');
  assert.ok(!/Let Me Be|Week of Sep 15|#1/.test(hero[0]), 'homepage strip does not list a #1 week');
  ...
  assert.ok(html.indexOf('class="after-stay"') < html.indexOf('class="steps distro-pipe"'), 'growth section leads; store delivery comes later');
  assert.ok(html.includes('class="plans price-ladder"'), 'price ladder stays');
  ...
  assert.ok(/\.home-portal \.hero \{[\s\S]*?padding-bottom:\s*16px/.test(cut), 'homepage hero does not leave a tall empty block under the charts strip');
  assert.ok(!/\.home-portal \.hero \{[^}]*padding-bottom:\s*168px/.test(cut), 'phone hero no longer reserves a dead zone under the strip');
  ...
  assert.ok(/Thirty-four posts/.test(blog) && blog.includes('blog-no-fakes.html'), 'blog hub count is not reverted');
  assert.ok(blog.includes('blog-plai-plus-collab.html'), 'hub lists the PLAI+ collab note');

 ...
 * Solo / single collected name also sends Performer + Producer.
 */
...
function isBlankOrAiName(value) {
...
function normalizeContributorRole(value) {
  const raw = trim(value).toLowerCase();
  if (ROLE_CREDIT[raw]) return ROLE_CREDIT[raw];
  if (raw === 'performer' || raw === 'vocalist' || raw === 'artist') return ROLE_CREDIT.performer;
  if (raw === 'producer') return ROLE_CREDIT.producer;
  if (raw === 'engineer' || raw === 'mixing engineer' || raw === 'mastering engineer') {
    return ROLE_CREDIT.engineer;
  }
  return '';
}

function addDeclaredContributor(out, seen, name, role) {
  const storeRole = normalizeContributorRole(role);
  if (!storeRole || isBlankOrAiName(name)) return;
  const person = personNameFields(name);
  if (!person) return;
  const key = person.name.toLowerCase() + '|' + storeRole.toLowerCase();
  if (seen[key]) return;
  seen[key] = true;
  out.push(Object.assign({}, person, { role: storeRole }));
}

function collectedCreditNames(body) {
  const raw = body && body.credits && typeof body.credits === 'object' && !Array.isArray(body.credits)
    ? body.credits
    : {};
  return {
    performer: trim(raw.performer || body && body.performer),
    producer: trim(raw.producer || body && body.producer),
    engineer: trim(raw.engineer || body && body.engineer),
  };
}

function declaredTrackContributors(body) {
  const out = [];
  const seen = Object.create(null);
  const list = body && Array.isArray(body.contributors) ? body.contributors : [];
  list.forEach((row) => {
    if (!row || typeof row !== 'object') return;
    addDeclaredContributor(
      out,
      seen,
  ...
  const collected = collectedCreditNames(body);
  addDeclaredContributor(out, seen, collected.performer, 'performer');
  addDeclaredContributor(out, seen, collected.producer, 'producer');
  addDeclaredContributor(out, seen, collected.engineer, 'engineer');
  return out;
}

function mergeTrackContributors(songwriter, extras) {
  const fields = trackSongwriterFields(songwriter);
  const roles = declaredTrackContributors(extras);
  const contributors = [];
  const seen = Object.create(null);
  function push(row) {
    if (!row || !row.name || !row.role) return;
    const key = String(row.name).toLowerCase() + '|' + String(row.role).toLowerCase();
    if (seen[key]) return;
    seen[key] = true;
    contributors.push(row);
  }
  if (fields && Array.isArray(fields.contributors)) fields.contributors.forEach(push);
  roles.forEach(push);
  if (!contributors.length) return null;
  const payload = { contributors: contributors };
  if (fields && fields.songwriters) payload.songwriters = fields.songwriters;
  if (fields && fields.composers) payload.composers = fields.composers;
  return payload;
}
...
module.exports = {
  ...
  writerCreateBody,
  declaredTrackContributors,
  mergeTrackContributors,
  filled,
};

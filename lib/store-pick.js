...
(function (root, factory) {
  ...
  function isExcludedStore(row) {
    var slug = typeof row === 'string' ? row : (row && (row.slug || row.dsp || row.dsp_slug || ''));
    if (storeKey(slug) === 'youtube-music' || storeKey(slug) === 'youtubemusic') return true;
    var name = typeof row === 'string' ? '' : String((row && (row.name || row.title)) || '');
    return storeKey(name) === 'youtube-music' || storeKey(name) === 'youtubemusic';
  }

  function isInactiveStore(row) {
    if (!row || typeof row !== 'object') return false;
    if (row.active === false || row.is_active === false || row.enabled === false) return true;
    if (row.inactive === true) return true;
    var status = String(row.status || row.state || '').trim().toLowerCase();
    return status === 'inactive' || status === 'disabled' || status === 'off';
  }

  function isSelectableStore(row) {
    return !isInactiveStore(row) && !isExcludedStore(row);
  }
  ...
  function selectableStores(list) {
    return normalizeStores((list || []).filter(isSelectableStore));
  }

  function selectableSlugs(list) {
    return selectableStores(list).map(function (row) { return row.slug; });
  }

  var INACTIVE_STORE_COPY = 'YouTube Music is not available. Choose other stores and submit again.';

  function blockedStores(list) {
    return normalizeStores((list || []).filter(function (row) {
      return isInactiveStore(row) || isExcludedStore(row);
    }));
  }

  function selectedRaw(root) {
    if (!root) return [];
    var list = q(root, '[data-store-list]') || q(root, '[data-edit-stores]');
    var allBox = q(root, '[data-store-all]');
    return allBox && allBox.checked ? slugsFrom(list, false) : slugsFrom(list, true);
  }

  function selectedBlocked(root) {
    return blockedStores(selectedRaw(root));
  }

  function inactiveStoreError(list) {
    var blocked = blockedStores(list);
    var hasYt = blocked.some(function (row) { return isExcludedStore(row); });
    if (!blocked.length || hasYt) return INACTIVE_STORE_COPY;
    var names = blocked.map(function (row) { return row.name || prettySlug(row.slug); }).filter(Boolean);
    if (names.length === 1) {
      return names[0] + ' is not available. Choose other stores and submit again.';
    }
    return 'One of those destinations is not available. Uncheck it and submit again.';
  }
  ...
  function selected(root) {
    ...
    var list = q(root, '[data-store-list]') || q(root, '[data-edit-stores]');
    var allBox = q(root, '[data-store-all]');
    var slugs = allBox && allBox.checked ? slugsFrom(list, false) : slugsFrom(list, true);
    return selectableSlugs(slugs);
  }
  ...
  function summaryCopyStyle(root, summary) {
    var fromSummary = summary && summary.getAttribute ? summary.getAttribute('data-store-copy') : '';
    var fromRoot = root && root.getAttribute ? root.getAttribute('data-store-copy') : '';
    return String(fromSummary || fromRoot || '').trim();
  }

  function isDefaultCatalog(list) {
    var live = selectableStores(list);
    var fallback = selectableStores(DEFAULT_STORES);
    if (!live.length || live.length !== fallback.length) return false;
    return live.every(function (row, i) {
      return row && fallback[i] && row.slug === fallback[i].slug;
    });
  }

  function formatReviewSummary(picked, total, allOn, style) {
    var n = asCount(total);
    var p = asCount(picked);
    if (style === 'included') {
      if (isAllStores(p, n, allOn)) return n ? ('All ' + n + ' stores · included') : 'All stores · included';
      if (n) return p + ' of ' + n + ' stores';
      return p ? (p + ' stores') : 'All stores · included';
    }
  ...
  function formatFaqStores(list) {
    ...
    return {
      ...
      names: names,
      groups: groups,
      // Do not paint stores.length. The public sentence stays "50+ platforms."
      sentence: 'We deliver to 50+ platforms.',
    };
  }
  ...
  function bind(root, opts) {
    ...
    var stores = selectableStores(opts.stores && opts.stores.length ? opts.stores : DEFAULT_STORES);
    ...
    function emit() {
      ...
      var style = summaryCopyStyle(root, summary);
      if (style === 'included' && allOn && isDefaultCatalog(stores)) total = 0;
      if (summary) summary.textContent = formatReviewSummary(picked.length, total, allOn, style);
  ...
  return {
    ...
    inactiveStoreError: inactiveStoreError,
    INACTIVE_STORE_COPY: INACTIVE_STORE_COPY,
    isExcludedStore: isExcludedStore,
    isInactiveStore: isInactiveStore,
    isSelectableStore: isSelectableStore,
    bind: bind,
    selected: selected,

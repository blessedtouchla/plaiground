...
(function (root, factory) {
  ...
  function formatFaqStores(list) {
    var stores = normalizeStores(list && list.length ? list : DEFAULT_STORES);
    var names = stores.map(function (row) { return row.name; }).filter(Boolean);
    var count = names.length;
    return {
      count: count,
      names: names,
      sentence: count
        ? ('We deliver to ' + count + ' stores:')
        : 'We deliver to the live store catalog.',
    };
  }

  function paintFaqStores(root, list) {
    if (!root) return formatFaqStores(list);
    var info = formatFaqStores(list);
    var sentence = root.querySelector ? root.querySelector('[data-faq-store-count]') : null;
    var host = root.querySelector ? root.querySelector('[data-faq-store-list]') : null;
    if (sentence) sentence.textContent = info.sentence;
    if (host) {
      host.textContent = '';
      info.names.forEach(function (name) {
        var item = (root.ownerDocument || (typeof document !== 'undefined' ? document : null));
        item = item && item.createElement ? item.createElement('li') : { textContent: name };

...
function run() {
  ...
  const nodes = {
    ...
    '[data-stat="rejected"]': makeEl({}),
    '[data-release-rows]': makeEl({}),
    '[data-release-empty]': makeEl({}),
    '[data-release-empty-title]': makeEl({}),
    '[data-release-empty-body]': makeEl({}),
    '[data-release-empty-action]': makeEl({}),
  ...
  assert.strictEqual(nodes['[data-stat="total"]'].textContent, '4', 'TOTAL includes rejected rows');
  assert.strictEqual(nodes['[data-stat="rejected"]'].textContent, '4');
  catalog.PlaigroundCatalog.setFilter('rejected');
  catalog.PlaigroundCatalog.render({
    releases: [
      { uuid: 'dddddddd-dddd-4ddd-8ddd-dddddddddddd', title: 'Sent Back', type: 'single', status: 'rejected', rejection_reason: 'Cover art is too small.' },
      { uuid: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', title: 'Also Back', type: 'single', status: 'rejected', rejection_reason: 'Incomplete metadata' },
      { uuid: '7a928125-b12e-4609-bd37-26ce0edf819e', title: 'Rainbow Road', type: 'single', status: 'live' },
    ],
    total: 3,
    analytics: {},
  });
  assert.strictEqual(nodes['[data-stat="total"]'].textContent, '3', 'TOTAL is live + pending + rejected');
  assert.strictEqual(nodes['[data-stat="live"]'].textContent, '1');
  assert.strictEqual(nodes['[data-stat="rejected"]'].textContent, '2');
  assert.strictEqual(nodes['[data-release-table]'].hidden, true, 'Rejected tab hides the main live/pending table');
  assert.strictEqual(nodes['[data-rejected-section]'].hidden, false);
  assert.strictEqual(nodes['[data-release-empty]'].hidden, true);
  assert.ok(findByText(nodes['[data-rejected-rows]'], 'Sent Back'));
  assert.ok(findByText(nodes['[data-rejected-rows]'], 'Also Back'));
  assert.ok(!findByText(nodes['[data-release-rows]'], 'Rainbow Road'), 'Rejected tab does not list live rows');
  catalog.PlaigroundCatalog.setFilter('rejected');
  catalog.PlaigroundCatalog.render({
    releases: [
      { uuid: '7a928125-b12e-4609-bd37-26ce0edf819e', title: 'Rainbow Road', type: 'single', status: 'live' },
  ...
  assert.strictEqual(nodes['[data-release-empty]'].hidden, false);
  assert.strictEqual(nodes['[data-release-empty-title]'].textContent, 'No rejected releases.');
  assert.ok(!/ToneGrid|DistroKid|InterSpace|hop/i.test(nodes['[data-release-empty-body]'].textContent));
  assert.strictEqual(nodes['[data-rejected-section]'].hidden, true);
  assert.strictEqual(nodes['[data-stat="total"]'].textContent, '1');
  assert.strictEqual(nodes['[data-stat="rejected"]'].textContent, '0');
  catalog.PlaigroundCatalog.setFilter('all');
  catalog.PlaigroundCatalog.render({
    releases: [
      { uuid: '6629b532-2e78-4be6-84eb-e4dfa9ac33e5', title: 'Metete en el groove', type: 'single', status: 'draft', rejection_reason: 'Invalid cover art' },
      { uuid: '7a928125-b12e-4609-bd37-26ce0edf819e', title: 'Rainbow Road', type: 'single', status: 'live' },
    ],
    total: 2,
    analytics: {},
  });
  assert.ok(findByText(nodes['[data-rejected-rows]'], 'Metete en el groove'), 'Metete lock id stays in Rejected even as a store draft');
  assert.ok(!findByText(nodes['[data-release-rows]'], 'Metete en el groove'));
  assert.ok(Number(nodes['[data-stat="rejected"]'].textContent) >= 1);
  assert.ok(Number(nodes['[data-stat="total"]'].textContent) >= 2, 'TOTAL still counts the returned lock row');
  ...
  assert.ok(html.includes('data-release-filter="rejected">Rejected</a>'));
  assert.ok(html.includes('<span>Rejected</span>'));
  assert.ok(!/data-release-filter="draft"/.test(html), 'Drafts tab is gone from Releases');
  assert.ok(!/>Drafts</.test(html));
  assert.ok(!/>Draft</.test(html), 'stats bar no longer says Draft');
  assert.ok(html.includes('lib/release-status.js?v=20260915r1'));
  assert.ok(html.includes('catalog.js?v=20260915r1'));

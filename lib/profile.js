...
const CREATION_CLASSES = ['human', 'ai_infused', 'fully_ai'];
const AI_MIX_KEYS = ['lyrics', 'vocals', 'instruments', 'production_mix'];
const AI_MIX_VALUES = ['human', 'ai', 'mixed'];
...
function normalizeCreationClass(value) {
  const key = trim(value).toLowerCase().replace(/[\s-]+/g, '_');
  if (key === 'ai_assisted' || key === 'aiassisted' || key === 'assisted') return 'ai_infused';
  if (key === 'fullyai' || key === 'full_ai') return 'fully_ai';
  if (CREATION_CLASSES.indexOf(key) !== -1) return key;
  return '';
}

function normalizeAiMix(raw) {
  const src = raw && typeof raw === 'object' ? raw : {};
  const out = {};
  AI_MIX_KEYS.forEach(function (key) {
    const val = trim(src[key]).toLowerCase();
    out[key] = AI_MIX_VALUES.indexOf(val) !== -1 ? val : '';
  });
  return out;
}

function contributionsFromMix(mix, creationClass) {
  const human = [];
  const ai = [];
  const data = normalizeAiMix(mix);
  AI_MIX_KEYS.forEach(function (key) {
    const contrib = MIX_TO_CONTRIB[key];
    const val = data[key];
    if (!contrib || !val) return;
    if (val === 'human' || val === 'mixed') human.push(contrib);
    if (val === 'ai' || val === 'mixed') ai.push(contrib);
  });
  if (creationClass === 'fully_ai' && ai.indexOf('full_track_support') === -1) ai.push('full_track_support');
  return {
    human_contributions: pickKeys(human, HUMAN_CONTRIBUTIONS),
    ai_contributions: pickKeys(ai, AI_CONTRIBUTIONS),
  };
}

function inferCreationClass(raw) {
  const explicit = normalizeCreationClass(raw && raw.creation_class);
  if (explicit) return explicit;
  const human = pickKeys(raw && raw.human_contributions, HUMAN_CONTRIBUTIONS);
  const ai = pickKeys(raw && raw.ai_contributions, AI_CONTRIBUTIONS);
  const pct = normalizeInvolvement(raw && raw.ai_involvement_percent);
  if (ai.indexOf('full_track_support') !== -1 || (pct != null && pct >= 90 && !human.length)) return 'fully_ai';
  if (ai.length || (pct != null && pct > 0)) return 'ai_infused';
  if (human.length) return 'human';
  return '';
}

function inferAiMix(raw) {
  const existing = normalizeAiMix(raw && raw.ai_mix);
  const hasAny = AI_MIX_KEYS.some(function (key) { return existing[key]; });
  if (hasAny) return existing;
  const human = new Set(pickKeys(raw && raw.human_contributions, HUMAN_CONTRIBUTIONS));
  const ai = new Set(pickKeys(raw && raw.ai_contributions, AI_CONTRIBUTIONS));
  const out = normalizeAiMix({});
  Object.keys(MIX_TO_CONTRIB).forEach(function (mixKey) {
    const contrib = MIX_TO_CONTRIB[mixKey];
    const h = human.has(contrib);
    const a = ai.has(contrib);
    if (h && a) out[mixKey] = 'mixed';
    else if (h) out[mixKey] = 'human';
    else if (a) out[mixKey] = 'ai';
  });
  return out;
}
...
function normalizeArtist(raw) {
  ...
  const creationClass = inferCreationClass(raw);
  const aiMix = inferAiMix(raw);
  const fromMix = contributionsFromMix(aiMix, creationClass);
  const humanContrib = Array.isArray(raw.human_contributions)
    ? pickKeys(raw.human_contributions, HUMAN_CONTRIBUTIONS)
    : fromMix.human_contributions;
  const aiContrib = Array.isArray(raw.ai_contributions)
    ? pickKeys(raw.ai_contributions, AI_CONTRIBUTIONS)
    : fromMix.ai_contributions;
  return {
    id: artistId,
    artist_id: artistId,
    name: name,
    ...
    ai_mix: aiMix,
    human_contributions: humanContrib.length ? humanContrib : fromMix.human_contributions,
    ai_contributions: aiContrib.length ? aiContrib : fromMix.ai_contributions,
...
function normalizePendingEdit(raw) {
  ...
  return {
    ...
    creation_class: creationClass,
    ai_mix: aiMix,
    human_contributions: Array.isArray(raw.human_contributions)
      ? pickKeys(raw.human_contributions, HUMAN_CONTRIBUTIONS)
      : fromMix.human_contributions,
    ai_contributions: Array.isArray(raw.ai_contributions)
      ? pickKeys(raw.ai_contributions, AI_CONTRIBUTIONS)
      : fromMix.ai_contributions,
    ai_process_detail: trim(raw.ai_process_detail).slice(0, MAX_AI_DETAIL),
    ai_involvement_percent: normalizeInvolvement(raw.ai_involvement_percent),
...
function normalizeRelease(raw) {
  ...
  const deliveredAt = trim(raw.delivered_at || raw.deliveredAt);
...
module.exports = {
  ...
  AI_MIX_KEYS: AI_MIX_KEYS,
  AI_MIX_VALUES: AI_MIX_VALUES,
  CREATION_CLASSES: CREATION_CLASSES,
  ...
  PHOTO_RE: PHOTO_RE,
  contributionsFromMix: contributionsFromMix,
  ...
  newId: newId,
  normalizeAiMix: normalizeAiMix,
  normalizeArtist: normalizeArtist,
  normalizeCreationClass: normalizeCreationClass,

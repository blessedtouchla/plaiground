...
function submissionFrom(user, item, live, deliveries) {
  ...
  const alert = sanitizePartner(releaseStatus.problemAlert(Object.assign({}, stored, live || {}), status));
  return {
    id: item.id,
    ...
    title: title,
    status: adminStatusLabel(status),
    alert: alert,
...
async function listAdminOverview(options) {
  ...
  const [signups, checkouts, subscriptions, money, submissions, storeRoyalties] = await Promise.all([

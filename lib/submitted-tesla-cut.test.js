...
function run() {
  ...
  assert.ok(!html.includes('site.css?v=20260917s2'), 's2 stamp is retired after the quiet status line');
  assert.ok(!html.includes('site.css?v=20260917s3'), 's3 stamp is retired after summary chrome reset');
  ...
  assert.ok(html.includes('class="submit-copy"'), 'sparse copy is a sibling of the cover so desktop can use the canvas');
  ...
  assert.ok(/\.submitted-page \.wrap[\s\S]*?max-width:\s*480px/.test(cut), 'mobile wrap stays phone-narrow');
  assert.ok(/\.submitted-page \.submit-cover[\s\S]*?width:\s*min\(78vw, 320px\)/.test(cut), 'cover is the almost-above-the-fold star');
  assert.ok(/@media \(min-width: 900px\)[\s\S]*?max-width:\s*1680px/.test(cut), 'desktop wrap uses the 1680 canvas');
  assert.ok(/@media \(min-width: 900px\)[\s\S]*?grid-template-columns:\s*minmax\(400px, 560px\)/.test(cut), 'desktop is cover + copy, not a phone column');
  assert.ok(!/@media \(min-width: 800px\)[\s\S]*?\.submit-cover \{ width: 360px; \}/.test(cut), 'desktop no longer keeps a 360 phone cover');
  assert.ok(/\.submitted-page \.submit-out b[\s\S]*?font-size:\s*clamp\(2rem/.test(cut), 'Out date is larger than Total paid / Stores');
  assert.ok(/\.submitted-page #tg-status[\s\S]*?font-size:\s*12px/.test(cut), 'store status line stays quiet');
  assert.ok(/\.submitted-page \.submit-next summary[\s\S]*?appearance:\s*none/.test(cut), 'collapsed next has no UA pill chrome');
  assert.ok(/\.submitted-page \.submit-quiet-row span[\s\S]*?font-size:\s*13px/.test(cut), 'Total paid / Stores stay quiet');
  assert.ok(/\.submitted-page \.submit-copy h1[\s\S]*?font-family:\s*"Space Grotesk"/.test(cut), 'song title stays Space Grotesk');

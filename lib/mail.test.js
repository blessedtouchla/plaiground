...
async function run() {
  ...
  const files = ['lib/mail.js', 'api/auth.js', 'signup.html', 'confirm.html', 'confirmed.html', 'login.html', 'forgot.html', 'magic.html', 'README.md', '.env.example'];
  files.forEach((rel) => {
  ...
  assert.ok(forgotHtml.includes('/api/auth/mail'));
  assert.ok(forgotHtml.includes("kind: 'reset'"));

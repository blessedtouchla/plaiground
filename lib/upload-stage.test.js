...
function run() {
  ...
  assert.ok(upload.includes('data-stage-title'), 'Sleeve title paint hook exists');
  assert.ok(upload.includes('lib/upload-stage.js?v=20260912s5'), 'Upload cache-busts the stage helper');
  assert.ok(upload.includes('site.css?v=20260912ly4'), 'Upload cache-busts site.css for the splat/drop paint');
  ...
  assert.ok(upload.includes('Optional — skip if you don’t need them.'), 'muted optional helper sits under the pair');
  ...
  assert.ok(upload.includes('16-bit or higher · MP3 is converted to WAV before it goes to stores'), 'audio helper copy stays');
  ...
  assert.ok(face.classList.contains('is-on'), 'title paints onto the sleeve');
  assert.ok(audioChip.classList.contains('is-on'));
  ...
  const audioPick = el({ tagName: 'INPUT', files: [] });
  const coverPick = el({ tagName: 'INPUT', files: [] });
  ...
  assert.ok(audioBind.includes('hint.hidden = false'), 'attached audio keeps the confirm-master line');

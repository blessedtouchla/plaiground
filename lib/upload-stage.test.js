...
function run() {
  ...
  assert.ok(upload.includes('lib/upload-stage.js?v=20260912s6'), 'Upload cache-busts the stage helper');
  assert.ok(upload.includes('site.css?v=20260912ly7'), 'Upload cache-busts site.css for the type/copy paint');
  assert.ok(css.includes('.audio-bar[hidden]'), 'empty audio chip stays hidden until a file is attached');
  assert.ok(css.includes('.upload-stage-caption') || css.includes('[data-art-meta]'), 'cover captions are muted caption scale');
  assert.ok(css.includes('color: var(--muted-2)'), 'stage captions use brighter muted, not muddy dark-on-black');

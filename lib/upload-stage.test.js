...
function run() {
  ...
  assert.ok(upload.includes('lib/upload-stage.js?v=20260912s3'), 'Upload cache-busts the stage helper');
  assert.ok(upload.includes('site.css?v=20260912s3'), 'Upload cache-busts site.css for the stage paint');
  assert.ok(upload.includes('data-stage-pick="audio"'), 'Audio chip/drop opens the existing audio input');
  assert.ok(upload.includes('data-stage-pick="cover"'), 'Cover chip/sleeve opens the existing cover input');
  ...
  assert.ok(/data-stage-chip="cover"[^>]*data-stage-pick="cover"/.test(upload), 'Cover chip is a pick trigger');
  assert.ok(upload.includes('data-art-pick'), 'Add artwork still uses the existing cover pick');

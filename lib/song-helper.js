...
(function (root, factory) {
  ...
  function normalizeInterview(body) {
    var src = body || {};
    var wordsIn = src.words || {};
    var shapeIn = src.shape || {};
    var words = {};
    WORD_KEYS.forEach(function (key) {
      words[key] = cleanField(wordsIn[key], LIMITS.word);
    });
    ...
    return {
      ...
      words: words,
      shape: {
        genre: cleanField(shapeIn.genre, LIMITS.genre),
        language: language,
  ...
  function collectUserStrings(interview) {
    ...
    WORD_KEYS.forEach(function (key) { add(key, words[key]); });
    return items;
  }
  ...
  function formatAuthorship(draft, meta) {
    ...
    if (info.interview) {
      ...
      WORD_KEYS.forEach(function (key) {
        var value = info.interview.words && info.interview.words[key];
  ...
  function interviewPrompt(interview) {
    return JSON.stringify({
      task: 'Write the lyric draft JSON. Variant ' + (interview.variant || 0) + ': change the generated lines, and keep every user line identical.',
      mood: interview.mood,
      whatHappened: interview.happened,
      who: interview.who,
      why: interview.why,
      hookSentenceVerbatim: interview.line,
      wordBankVerbatim: interview.words,
      genre: interview.shape.genre,
      language: interview.shape.language,
      explicit: interview.shape.explicit === 'explicit',
      length: interview.shape.length,
    });
  }

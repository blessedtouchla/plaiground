...
async function run() {
  ...
  assert.ok(!apexRedirects.some((row) => row.source === '/api/stripe/webhook'));
  assert.ok(apexRedirects.some((row) => /\(\?!.*api\/stripe\/webhook/.test(String(row.source))));
  ...
  assert.ok(middlewareSrc.includes("hostOf(request) !== 'wannaplai.com'"));
  assert.ok(middlewareSrc.includes('www.wannaplai.com'));
  assert.ok(middlewareSrc.includes("redirect: 'manual'"));
  assert.ok(!/sk_live_|sk_test_|whsec_/.test(middlewareSrc));

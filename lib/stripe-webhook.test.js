...
const { LEGACY_PRO_YEAR_PRICE, PRICE_BY_PLAN, planForPriceId } = require('./stripe-plans');
...
const PRO_YEAR = LEGACY_PRO_YEAR_PRICE;
...
async function run() {
  ...
  assert.strictEqual(PRICE_BY_PLAN['pro:year'], undefined, 'no new Pro yearly checkout price');
  ...
  await withEnv({
    ...
    try {
      ...
      assert.strictEqual(proYearGap.statusCode, 400);
      assert.ok(/not available yet/i.test(json(proYearGap).error || ''));

      const legacyYear = mockRes();
      await checkoutApi({
        method: 'POST',
        url: '/api/create-checkout-session',
        headers: {},
        body: { priceId: PRO_YEAR },
      }, legacyYear);
      assert.strictEqual(legacyYear.statusCode, 400, 'old Pro yearly $149 is not a new checkout');

      const allPrices = [CREATOR_MONTH, CREATOR_YEAR, PRO_MONTH];

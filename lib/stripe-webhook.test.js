...
async function run() {
  ...
  await withEnv({
    ...
    assert.strictEqual(pastDue.status, 'hold');
    row = await accounts.findById(userId);
    assert.strictEqual(row.plan, 'pro');
    assert.strictEqual(row.status, 'hold');
    ...
    const failedInvoice = await applyStripeEvent(event('invoice.payment_failed', {
      customer: 'cus_ada',
      paid: false,
    }));
    assert.strictEqual(failedInvoice.applied, true);
    assert.strictEqual(failedInvoice.status, 'hold');
    assert.strictEqual((await accounts.findById(userId)).plan, 'pro');

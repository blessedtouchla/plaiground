...
async function run() {
  ...
  await withEnv({
    ...
    try {
      ...
      assert.strictEqual(monthParams.get('subscription_data[collection_method]'), 'charge_automatically');
      ...
      assert.strictEqual(yearParams.get('subscription_data[collection_method]'), 'charge_automatically');

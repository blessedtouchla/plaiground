...
const { applyPaidSessionToAccount, applyStripeEvent, periodEndUnix, recoverPaidPlan, signStripePayload, verifyStripeSignature } = require('./stripe-webhook');
...
async function run() {
  ...
  await withEnv({
    ...
    assert.strictEqual(json(badSig).error, 'Invalid signature.');

    const streamRaw = JSON.stringify(event('checkout.session.completed', { payment_status: 'paid' }));
    let bodyAccessed = false;
    const streamReq = new Readable({
      read() {
        this.push(Buffer.from(streamRaw));
        this.push(null);
      },
    });
    streamReq.method = 'POST';
    streamReq.url = '/api/stripe/webhook';
    streamReq.headers = { 'stripe-signature': 't=' + now + ',v1=nope' };
    Object.defineProperty(streamReq, 'body', {
      get() {
        bodyAccessed = true;
        return JSON.parse(streamRaw);
      },
    });
    const streamRes = mockRes();
    await checkoutApi(streamReq, streamRes);
    assert.strictEqual(bodyAccessed, false, 'must not read Vercel req.body getter before the raw stream');
    assert.strictEqual(streamRes.statusCode, 400);
    assert.strictEqual(json(streamRes).error, 'Invalid signature.');
    ...
    const prevUpdate = accounts.updateStripe;
    accounts.updateStripe = async () => {
      throw Object.assign(new Error('db down'), { code: 'ACCOUNTS_UNCONFIGURED' });
    };
    const ackTypes = [
    ...
    try {
      for (let i = 0; i < ackTypes.length; i += 1) {
        const type = ackTypes[i];
        const payload = JSON.stringify(event(type, {
          ...
          customer: 'cus_ada',
          line_items: { data: [{ price: { id: CREATOR_MONTH } }] },
          items: { data: [{ price: { id: CREATOR_MONTH } }] },
        }));
        ...
        await checkoutApi({
          method: 'POST',
          url: '/api/stripe/webhook',
          headers: { 'stripe-signature': 't=' + ackTs + ',v1=' + signStripePayload(payload, WEBHOOK_SECRET, ackTs) },
          body: payload,
        }, ackRes);
        assert.strictEqual(ackRes.statusCode, 200, type + ' must not 500 after a valid signature');
        assert.strictEqual(json(ackRes).received, true);
      }
    } finally {
      accounts.updateStripe = prevUpdate;
    }
  });
  ...
  await withEnv({
        ...
        return {
          ...
          json: async () => ({
            data: [{
              id: 'sub_switch',
              status: 'active',
              current_period_end: 1789257600,
              items: { data: [{ id: 'si_switch', price: { id: currentPrice } }] },
            }],
          }),
        };
      }
      if (href.indexOf('/v1/billing_portal/configurations') !== -1) {
        return { ok: true, status: 200, json: async () => ({ id: 'bpc_switch' }) };
      }
      if (href.indexOf('/v1/billing_portal/sessions') !== -1) {
        return { ok: true, status: 200, json: async () => ({ url: 'https://billing.stripe.com/p/session/test_portal' }) };
      }
      return {
        ok: true,
        status: 200,
    ...
    try {
      ...
      assert.strictEqual(json(billing).has_card, true);
      assert.strictEqual(json(billing).no_card, false);
      assert.strictEqual(json(billing).current_period_end, 1789257600);

      const basicBilling = mockRes();
      await checkoutApi({
        method: 'POST',
        url: '/api/create-checkout-session',
        headers: { cookie: basic.cookie },
      ...
      assert.strictEqual(basicBilling.statusCode, 200);
      assert.strictEqual(json(basicBilling).plan, 'basic');
      assert.ok(json(basicBilling).current_period_end == null, 'Basic must not receive a renewal unix');

      const portalCallsBefore = calls.filter((item) => String(item.url).indexOf('/v1/billing_portal/sessions') !== -1).length;
      const checkoutBeforePortal = calls.filter((item) => String(item.url).indexOf('/v1/checkout/sessions') !== -1).length;
      const portal = mockRes();
      await checkoutApi({
        method: 'POST',
        url: '/api/create-checkout-session',
        headers: { cookie: paid.cookie },
        body: { action: 'portal', returnUrl: 'https://www.wannaplai.com/settings.html#manage-billing' },
      }, portal);
      assert.strictEqual(portal.statusCode, 200, portal.body);
      assert.strictEqual(json(portal).url, 'https://billing.stripe.com/p/session/test_portal');
      assert.strictEqual(json(portal).portal, true);
      assert.ok(!json(portal).no_card);
      const portalCall = calls.filter((item) => String(item.url).indexOf('/v1/billing_portal/sessions') !== -1).pop();
      assert.ok(portalCall, 'portal creates a Billing Portal session');
      const portalParams = new URLSearchParams(portalCall.opts.body);
      assert.strictEqual(portalParams.get('customer'), 'cus_switch');
      assert.strictEqual(portalParams.get('flow_data[type]'), 'payment_method_update');
      assert.ok(String(portalParams.get('return_url')).indexOf('settings.html') !== -1);
      assert.strictEqual(portalParams.get('payment_method_types[0]'), null);
      assert.ok(String(portalCall.opts.body || '').indexOf('switch@example.com') === -1, 'portal does not look up the customer by email');
      assert.ok(String(json(portal).url).indexOf('dashboard.stripe.com') === -1);
      assert.strictEqual(
        calls.filter((item) => String(item.url).indexOf('/v1/billing_portal/sessions') !== -1).length,
        portalCallsBefore + 1
      );
      assert.strictEqual(
        calls.filter((item) => String(item.url).indexOf('/v1/checkout/sessions') !== -1).length,
        checkoutBeforePortal,
        'portal must not start Checkout'
      );

      const ghostPortal = mockRes();
      await checkoutApi({
        method: 'POST',
        url: '/api/create-checkout-session',
        headers: { cookie: (await signupUser('ghost-portal@example.com', 'pro')).cookie },
        body: { action: 'portal' },
      }, ghostPortal);
      assert.strictEqual(ghostPortal.statusCode, 200, ghostPortal.body);
      assert.strictEqual(json(ghostPortal).no_card, true);
      assert.strictEqual(json(ghostPortal).has_card, false);
      assert.ok(!json(ghostPortal).url, 'no Stripe customer must not receive a portal URL');
      assert.ok(/no card on file/i.test(json(ghostPortal).error));

      const staff = await signupUser('emailplaiground@gmail.com', 'basic');
      const staffPortal = mockRes();
      await checkoutApi({
        method: 'POST',
        url: '/api/create-checkout-session',
      ...
      assert.strictEqual(staffPortal.statusCode, 200, staffPortal.body);
      assert.strictEqual(json(staffPortal).no_card, true);
      assert.ok(!json(staffPortal).url, 'unpaid Pro override must not get a fake portal');
      ...
      assert.strictEqual(orphanBilling.statusCode, 200);
      assert.ok(json(orphanBilling).current_period_end == null, 'missing Stripe period end must omit the date');
    } finally {
      global.fetch = originalFetch;
    }
  });
  ...
  assert.ok(handlerSrc.includes('https://www.wannaplai.com/api/stripe/webhook'));
  assert.ok(handlerSrc.includes('must not 308') || handlerSrc.includes('does not follow redirects'));
  assert.ok(handlerSrc.includes('Never read req.body') || handlerSrc.includes('never read req.body'));
  assert.ok(handlerSrc.includes('STRIPE_WEBHOOK_SECRET'));
  assert.ok(handlerSrc.includes('must not 500'));
  assert.ok(!/sendJson\(res, 500, \{ error: 'Webhook failed\.' \}\)/.test(handlerSrc));

  const vercel = JSON.parse(fs.readFileSync(path.join(__dirname, '..', 'vercel.json'), 'utf8'));
  const apexRedirects = (vercel.redirects || []).filter((row) => (
    (row.has || []).some((rule) => rule.type === 'host' && rule.value === 'wannaplai.com')
  ));
  assert.ok(apexRedirects.length, 'other apex pages may still 308 to www');
  assert.ok(apexRedirects.some((row) => row.source === '/'));
  assert.ok(apexRedirects.some((row) => row.source === '/:file.html'));
  apexRedirects.forEach((row) => {
    const source = String(row.source || '');
    assert.ok(source.indexOf('api/stripe') === -1, 'webhook is not a redirect source');
    assert.ok(source.indexOf('(?!') === -1, 'no negative-lookahead path-to-regexp');
    assert.ok(source.indexOf('WEBHOOK') === -1, 'no WEBHOOK_PATH identifier in redirects');
  });
  const webhookRewrite = (vercel.rewrites || []).find((row) => row.source === '/api/stripe/webhook');
  assert.strictEqual(webhookRewrite && webhookRewrite.destination, '/api/create-checkout-session?action=webhook');
  assert.ok(!fs.existsSync(path.join(__dirname, '..', 'middleware.js')), 'no extra Edge middleware file');
  ...
  assert.ok(handlerSrc.includes("actionOf(body) === 'portal'") || handlerSrc.includes("action === 'portal'"));
  assert.ok(handlerSrc.includes('billing_portal/sessions'));
  assert.ok(handlerSrc.includes('payment_method_update'));
  assert.ok(handlerSrc.includes('There is no card on file.'));
  assert.ok(handlerSrc.includes('billing.stripe.com'));
  assert.ok(!/https:\/\/dashboard\.stripe\.com/.test(handlerSrc));
  ...
  assert.ok(handlerSrc.includes('periodEndUnix'));
  assert.ok(handlerSrc.includes('current_period_end'));
  assert.strictEqual(periodEndUnix({ current_period_end: 1789257600 }), 1789257600);
  assert.strictEqual(periodEndUnix({
    items: { data: [{ current_period_end: 1789344000 }] },
  }), 1789344000);
  assert.strictEqual(periodEndUnix({}), null);

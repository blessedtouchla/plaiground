...
function run() {
  ...
  const distributionOnly = [
    read('lib/store-analytics.js'),
    read('lib/store-webhook.js'),
    read('lib/store-status-refresh.js'),
    read('lib/store-analytics.test.js'),
    read('lib/store-webhook.test.js'),
  ].join('\n');
  assert.ok(!/\/publishing|\/boost|\/transfers|\/writers|\/publishers|\/ugc|migrate|takeover|\/distribute|\/approve/.test(distributionOnly), 'this slice stays distribution-only');
  assert.ok(!fs.readFileSync(path.join(__dirname, '..', 'store-client.js'), 'utf8').includes('PlaigroundStoreStatusRefresh'), 'do not rewrite the submit client');

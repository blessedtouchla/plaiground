...
async function run() {
  ...
  assert.ok(upload.includes('lib/upload-draft-files.js'), 'submit flow loads the file persist helper');
  assert.ok(upload.indexOf('lib/upload-draft-files.js') < upload.indexOf('lib/upload-credits.js'));
  assert.ok(files.persistPickedFiles, 'submit flow can persist picked files');
  assert.ok(files.clearPickedAudio, 'Clear audio can drop held audio without wiping the draft');
  assert.ok(files.restorePickedFiles, 'in-progress submit can restore picked files');
  ...
  assert.ok(!/PlaigroundObjectHop\.put/.test(credits), 'upload credits do not hop / send a leftover draft to the store');
  ...
  assert.ok(held && held.__held === 1 && held.buffer, 'held audio clones picker bytes so they survive navigation');
  ...
  const clearWin = makeWin(audio, cover);
  await files.persistPickedFiles(clearWin);
  assert.ok(clearWin.indexedDB._stores.files[files.AUDIO_KEY], 'clear fixture holds audio');
  assert.ok(clearWin.indexedDB._stores.files.master, 'clear fixture also holds the master slot');
  await files.clearPickedAudio(clearWin);
  assert.strictEqual(clearWin.indexedDB._stores.files[files.AUDIO_KEY], undefined, 'clearPickedAudio drops picked audio');
  assert.strictEqual(clearWin.indexedDB._stores.files.master, undefined, 'clearPickedAudio drops the master slot');
  assert.strictEqual(clearWin.indexedDB._stores.files[files.COVER_KEY], cover, 'clearPickedAudio leaves cover');

...
function looksLikeMp3Name(name, mime) {
  ...
  return /\.mp3$/i.test(n) || t === 'audio/mpeg' || t === 'audio/mp3';
}
...
function incomingAudioAllowed(buf) {
  const head = headerText(buf);
  if (/filename="[^"]+\.(wav|flac|mp3)"/i.test(head)) return true;
  if (/filename="/i.test(head) && !/\.(wav|flac|mp3)"/i.test(head)) return false;
  if (/content-type:\s*audio\/(mpeg|mp3|wav|x-wav|wave|flac|x-flac)/i.test(head)) return true;
  return true;
}
...
async function prepareToneGridAudio(raw) {
  ...
  if (kind === 'mp3') {
    try {
      const wav = await mp3ToWav(part.data);
      return buildAudioMultipart(wavFilename(part.filename), WAV_MIME, wav);
    } catch (err) {

...
const RETURNING = 'id, email, artist_name, plan, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, created_at, updated_at';
  ...
  return {
    ...
    async findByEmail(email) {
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, created_at, updated_at FROM users WHERE email = $1 LIMIT 1',
        [email]
      );
      return rows[0] || null;
    },
    ...
    async update(id, patch) {
      const rows = await db.query(
        'UPDATE users SET artist_name = COALESCE($2, artist_name), plan = COALESCE($3, plan), stripe_customer_id = COALESCE($4, stripe_customer_id), stripe_session_id = COALESCE($5, stripe_session_id), tonegrid_artist_id = $6, tonegrid_release_ids = $7, updated_at = now() WHERE id = $1 RETURNING ' + RETURNING,
        [
          id,
          patch.artist_name,
          patch.plan,
...
async function updateCatalog(id, { artistId, releaseId }) {
  await ensureReady();
  const row = await repo().findById(id);
  if (!row) return null;
  let artist = row.tonegrid_artist_id || null;
  if (artistId && !artist) artist = artistId;
  const releases = Array.isArray(row.tonegrid_release_ids) ? row.tonegrid_release_ids.slice() : [];
  if (releaseId && !releases.some((value) => String(value).toLowerCase() === String(releaseId).toLowerCase())) {

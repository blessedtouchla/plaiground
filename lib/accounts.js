...
const RETURNING = 'id, email, artist_name, plan, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at';
  ...
  return {
    ...
    async findByEmail(email) {
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at FROM users WHERE email = $1 LIMIT 1',
        [email]
      );
      return rows[0] || null;
    },
    ...
    async findByStripeCustomerId(customerId) {
      ...
      if (!target) return null;
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at FROM users WHERE stripe_customer_id = $1 LIMIT 1',
        [target]
      );
      return rows[0] || null;
    },
    ...
    async update(id, patch) {
      const rows = await db.query(
        'UPDATE users SET artist_name = COALESCE($2, artist_name), plan = COALESCE($3, plan), stripe_customer_id = COALESCE($4, stripe_customer_id), stripe_session_id = COALESCE($5, stripe_session_id), tonegrid_artist_id = $6, tonegrid_release_ids = $7, tonegrid_track_ids = $8, tonegrid_release_at = $9, email_confirmed_at = COALESCE($10, email_confirmed_at), updated_at = now() WHERE id = $1 RETURNING ' + RETURNING,
        [
          id,
          patch.artist_name,
          patch.plan,

...
function resetGrowthEvents() {
  try {
    require('./growth-events').resetStore();
  } catch {
    /* growth events are optional during early load */
  }
}
...
function useMemoryStore(store) {
  ...
  resetGrowthEvents();
  try {
    require('./growth-events').useMemoryStore();
  } catch {
    /* keep account memory even if events fail to load */
  }
  return memory;
}
...
function usernameKey(value) {
  const parsed = profile.normalizeUsername(value);
  return parsed.ok ? parsed.username : '';
}
...
function createMemoryStore() {
  ...
  return {
    ...
    async findByUsername(username) {
      const want = usernameKey(username);
      if (!want) return null;
      const rows = Array.from(users.values());
      for (let i = 0; i < rows.length; i += 1) {
        const have = usernameKey(profile.readStored(rows[i]).username);
        if (have && have === want) return copy(rows[i]);
      }
      return null;
    },
    ...
    async create(input) {
      ...
      const row = {
        ...
        profile: input.profile && typeof input.profile === 'object' ? input.profile : profile.emptyProfile(),
        created_at: now,
        updated_at: now,
      };
...
function isUsernameUniqueViolation(err) {
  return isUniqueViolation(err) && /username/i.test(String((err && (err.constraint || err.message)) || ''));
}
...
function neonRepo() {
  return {
    ...
    async findByUsername(username) {
      ...
      if (!want) return null;
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, status, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, profile, created_at, updated_at FROM users WHERE lower(btrim(COALESCE(profile->>\'username\', \'\'))) = $1 LIMIT 1',
        [want]
      );
      return rows[0] || null;
    },
    ...
    async listAll() {
      return db.query(
        'SELECT id, email, artist_name, plan, status, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, profile, created_at, updated_at FROM users ORDER BY created_at DESC'
      );
    },
    async findByReleaseId(releaseId) {
      const want = String(releaseId || '').trim().toLowerCase();
    ...
    async create(input) {
      const rows = await db.query(
        'INSERT INTO users (email, password_hash, artist_name, plan, profile) VALUES ($1, $2, $3, $4, $5::jsonb) RETURNING ' + RETURNING,
        [
          input.email,
          input.passwordHash,
          input.artistName,
          input.plan,
          JSON.stringify(input.profile && typeof input.profile === 'object' ? input.profile : profile.emptyProfile()),
        ]
      );
      return rows[0] || null;
    },
...
async function findByUsername(username) {
  ...
  const want = usernameKey(username);
  if (!want) return null;
  if (typeof repo().findByUsername !== 'function') return null;
  return repo().findByUsername(want);
}
...
async function createUser({ email, password, artist, plan, username }) {
  const normalized = normalizeEmail(email);
  ...
  const parsedUsername = profile.normalizeUsername(username);
  if (parsedUsername.error) {
    const err = new Error(parsedUsername.error);
    err.code = 'VALIDATION';
    throw err;
  }
  ...
  if (parsedUsername.username) {
    const taken = await findByUsername(parsedUsername.username);
    if (taken) throw usernameTakenError();
  }
  const nextProfile = Object.assign(profile.emptyProfile(), {
  ...
  try {
    ...
    if (isUsernameUniqueViolation(err)) throw usernameTakenError();
...
async function updateCatalog(id, { artistId, releaseId, trackId, at, replaceArtistId }) {
  await ensureReady();
  const row = await repo().findById(id);
  if (!row) return null;
  let artist = row.tonegrid_artist_id || null;
  if (artistId && (!artist || replaceArtistId)) artist = artistId;
  const nextIds = appendId(row.tonegrid_release_ids, releaseId);
  const next = await repo().update(id, catalogPatch(row, {
...
async function updateProfile(id, { artist, profile: nextProfile }) {
  ...
  if (!row) return null;
  const artistName = String(artist || '').trim();
  const stored = nextProfile || profile.emptyProfile();
  const parsedUsername = profile.normalizeUsername(stored.username);
  if (parsedUsername.error) {
    const err = new Error(parsedUsername.error);
    err.code = 'VALIDATION';
    throw err;
  ...
  if (parsedUsername.username) {
    const taken = await findByUsername(parsedUsername.username);
    if (taken && String(taken.id) !== String(id)) throw usernameTakenError();
  }
  stored.username = parsedUsername.username || '';
  try {
    return await repo().update(id, catalogPatch(row, {
...
async function findOrCreateFromGoogle({ email, artist, plan }) {
  const normalized = normalizeEmail(email);
  if (!isEmail(normalized)) {
    const err = new Error('A valid email is required.');
    err.code = 'VALIDATION';
  ...
  const existing = await findByEmail(normalized);
  if (existing) {
    if (existing.email_confirmed_at) return { row: existing, created: false };
    const confirmed = (await confirmEmail(existing.email)) || existing;
    return { row: confirmed, created: false };
  }
  const artistName = String(artist || '').trim();
  const nextPlan = normalizePlan(plan);
  const passwordText = crypto.randomBytes(32).toString('base64url');
  try {
    ...
    const confirmed = (await confirmEmail(created.email)) || created;
    return { row: confirmed, created: true };
  } catch (err) {
    if (isUniqueViolation(err)) {
      const raced = await findByEmail(normalized);
      if (raced) {
        if (raced.email_confirmed_at) return { row: raced, created: false };
        const confirmed = (await confirmEmail(raced.email)) || raced;
        return { row: confirmed, created: false };
      }
    }
    throw err;

...
async function notifyFirstUpload(user, releaseId) {
  if (!user || !releaseId) return;
  try {
    await require('./growth-events').recordFirstUpload(user, { release_id: releaseId });
  } catch {
    /* event hook must not block catalog writes */
  }
}
...
async function updateCatalog(id, { artistId, releaseId, trackId, at, replaceArtistId }) {
  ...
  if (releaseId) await notifyFirstUpload(next || row, releaseId);
  return next;
}
...
async function setReleaseHistory(id, releases) {
  ...
  if (ids.length) await notifyFirstUpload(next || row, ids[0]);
  return next;
}

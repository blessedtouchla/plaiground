...
const RETURNING = 'id, email, artist_name, plan, status, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at';
  ...
  return {
    ...
    async findByEmail(email) {
      ...
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, status, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at FROM users WHERE lower(btrim(email)) = $1 OR ($2 <> \'\' AND replace(split_part(split_part(lower(btrim(email)), \'@\', 1), \'+\', 1), \'.\', \'\') = $2 AND lower(split_part(btrim(email), \'@\', 2)) IN (\'gmail.com\', \'googlemail.com\')) LIMIT 1',
        [email, gmailKey]
      );
      return rows[0] || null;
    },
    ...
    async findByStripeCustomerId(customerId) {
      ...
      if (!target) return null;
      const rows = await db.query(
        'SELECT id, email, password_hash, artist_name, plan, status, stripe_customer_id, stripe_session_id, tonegrid_artist_id, tonegrid_release_ids, tonegrid_track_ids, tonegrid_release_at, email_confirmed_at, created_at, updated_at FROM users WHERE stripe_customer_id = $1 LIMIT 1',
        [target]
      );
      return rows[0] || null;
    },

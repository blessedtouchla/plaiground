...
function run() {
  ...
  assert.ok(/\$149/.test(ar) && /one-time/i.test(ar) && !/draft/i.test(ar), 'Act Pack shows the locked $149 one-time');
  ...
  assert.ok(/Release QC/i.test(ar) && /Compliance routing/i.test(ar) && /PLAI Research/i.test(ar), 'Act Pack includes release QC, compliance routing, and PLAI Research');
  assert.ok(/Human<\/strong>\s*·\s*<strong>AI-assisted<\/strong>\s*·\s*<strong>Fully AI/i.test(ar), 'disclosure names the three classes');
  assert.ok(/one update/i.test(ar), 'hosted EPK includes one update');
  assert.ok(/\$0/.test(ar) && /\$2\.49/.test(ar) && /\$79/.test(ar) && /\$99/.test(ar) && /\$199/.test(ar), 'A&R page shows every locked dollar');
  assert.ok(/\$149 \+ \$2\.49 \(\$151\.49\)/.test(ar), 'A&R page shows the Act Pack plus first song bundle');
  assert.ok(/Distribution is not required to buy A&amp;R/.test(ar), 'A&R does not require Distro');
  assert.ok(/No placement promise/.test(ar), 'Sync package makes no placement promise');
  assert.ok(!/pricing on request/i.test(ar), 'A&R does not say pricing on request');
  const arHeader = ar.match(/<header class="nav">[\s\S]*?<\/header>/);
  assert.ok(arHeader && !/Growth campaign|Sync package|href="[^"]*sync\.html"/.test(arHeader[0]), 'Growth and Sync stay off the A&R header');
  ...
  assert.ok(/DIY template/i.test(epk) && /\$79/.test(epk) && /one update/i.test(epk), 'EPK contrasts free DIY, the $79 down-sell, and one Act Pack update');
  assert.ok(!/pricing on request/i.test(epk) && !/data-checkout-plan/.test(epk), 'EPK shows prices without checkout');
  ...
  assert.ok(/\$149/.test(visible(ar)) && !/\$14\.99|\$19\.99/.test(visible(ar)), 'A&R price is the locked Act Pack, not a membership');

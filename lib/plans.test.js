...
function run() {
  ...
  assert.strictEqual(emptyBasic.album_allowed, false);

  const basicAlbum = plans.evaluate(row('basic', []), now, { type: 'album' });
  assert.strictEqual(basicAlbum.allowed, false);
  assert.strictEqual(basicAlbum.album_allowed, false);
  assert.strictEqual(basicAlbum.error, plans.ALBUM_ERROR);

  const creatorAlbum = plans.evaluate(row('creator', [{ id: uuid(1), at: '2026-08-01T00:00:00.000Z' }]), now, { type: 'album' });
  assert.strictEqual(creatorAlbum.allowed, true);
  assert.strictEqual(creatorAlbum.album_allowed, true);
  assert.strictEqual(creatorAlbum.used, 1, 'one album counts as one release');
  ...
  const continueSame = plans.evaluate(row('basic', [uuid(1)]), now, { continueReleaseId: uuid(1) });
  assert.strictEqual(continueSame.allowed, true);
  assert.strictEqual(continueSame.continuing, true);

  const continueOther = plans.evaluate(row('basic', [uuid(1)]), now, { continueReleaseId: uuid(2) });
  assert.strictEqual(continueOther.allowed, false);
  ...
  assert.strictEqual(pro.album_allowed, true);
  assert.ok(!pro.error);

  const heldPro = plans.evaluate(Object.assign(row('pro', [uuid(1)]), { status: 'hold' }), now);
  assert.strictEqual(heldPro.allowed, false, 'hold locks upload beyond Basic');
  assert.strictEqual(heldPro.plan, 'basic');
  assert.strictEqual(heldPro.error, plans.BASIC_ERROR);

  const heldEmpty = plans.evaluate(Object.assign(row('creator', []), { status: 'hold' }), now);
  assert.strictEqual(heldEmpty.allowed, true, 'hold still allows the Basic one-release slot');

  const warnedPro = plans.evaluate(Object.assign(row('pro', [uuid(1)]), { status: 'warning' }), now);
  assert.strictEqual(warnedPro.allowed, true, 'warning does not lock upload beyond Basic');
  assert.strictEqual(warnedPro.plan, 'pro');

  const staffBasic = plans.evaluate(Object.assign(row('basic', eightThisMonth.concat(lastMonthOnly)), {
    email: 'email.plaiground+lab@gmail.com',
  }), now);
  assert.strictEqual(staffBasic.plan, 'pro', 'staff Gmail alias is unlimited Pro on evaluate');
  assert.strictEqual(staffBasic.allowed, true);
  assert.strictEqual(staffBasic.limit, null);
  assert.strictEqual(staffBasic.album_allowed, true);

  const victoriaCreator = plans.evaluate(Object.assign(row('creator', eightThisMonth), {
    email: 'victoriaimtanes@gmail.com',
  }), now);
  assert.strictEqual(victoriaCreator.plan, 'creator', 'Victoria stays on her stored Creator plan');
  assert.strictEqual(victoriaCreator.allowed, false);

...
function teslaCut(css) {
  const start = css.indexOf('/* —— Public nav sections (Product / After upload / Trust / Listen / Stories) —— */');
  assert.ok(start !== -1, 'public nav sections cut is in site.css');
...
function run() {
  ...
  assert.ok(!index.includes('site.css?v=20260924w1'), 'w1 stamp is retired after the pricing story');
  assert.ok(!index.includes('site.css?v=20260924p1'), 'p1 stamp is retired after the locked prices');
  assert.ok(!index.includes('site.css?v=20260924p2'), 'p2 stamp is retired after the pricing ladder');
  ...
  assert.ok(js.includes('setupPublicNavSections'), 'shared builder groups the public nav');
  assert.ok(!js.includes('hop.put') && !/ToneGrid|DistroKid/.test(js), 'public nav JS does not invent hop or a distributor');

  assert.ok(/\.public-menu-toggle \{ display: inline-flex !important; \}/.test(cut), 'phone shows Menu so the sectioned drawer can open');
  assert.ok(/data-nav-group="listen"|nav-group-label/.test(css), 'Listen / Stories labels exist for the drawer');
  assert.ok(/\.nav\.nav-open \.nav-group-label/.test(cut), 'open Menu shows section labels');
  assert.ok(/\.nav\.nav-open \.nav-links \{[\s\S]*flex-direction:\s*column/.test(cut), 'open Menu stacks the sections');
  assert.ok(/flex-direction:\s*row/.test(cut), 'Log in stays quiet on the same top row');
  assert.ok(/\.nav-links,\s*\n\s*\.nav-actions \{ display: none !important; \}/.test(cut), 'closed phone bar hides the link row so gold Enter stays the primary');
  ...
  assert.ok(/<p class="side-label">Create<\/p>/.test(dash) && /<p class="side-label">Act<\/p>/.test(dash) && /<p class="side-label">Money<\/p>/.test(dash) && /<p class="side-label">PLAI<\/p>/.test(dash) && /<p class="side-label">Account<\/p>/.test(dash), 'signed-in nav is Create / Act / Money / PLAI / Account');

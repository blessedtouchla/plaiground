    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          const trackCalls = calls.filter((call) => String(call.url).indexOf('/tracks') !== -1);
        ...
        await withAccountUser({
          ...
          assert.strictEqual(calls.length, 1);
          assert.strictEqual(JSON.parse(calls[0].options.body).position, 2);
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(calls.length, 1, 'Basic with one leftover catalog track must still hop a new store track');
          assert.strictEqual(
            calls[0].url,
            'https://api-sandbox.tonegrid.pro/api/releases/bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb/tracks'
          );
          assert.ok(!/cccccccc-cccc-4ccc-8ccc-cccccccccccc/.test(calls[0].options.body || ''));
        });
      } finally {
        global.fetch = originalFetch;
      }
    ...
    async () => {
      ...
      try {
        await withAccountUser({}, async (_row, headers) => {
          ...
          assert.notStrictEqual(hopKeyOf(calls[0]), leftover);
          assert.notStrictEqual(hopKeyOf(calls[1]), leftover);
          assert.notStrictEqual(hopKeyOf(calls[0]), hopKeyOf(calls[1]));
        });
      } finally {
        global.fetch = originalFetch;
      }
      ...
      async () => {
        await withAccountUser({}, async (row, headers) => {
          ...
          try {
            ...
            assert.ok(Buffer.isBuffer(storeCall.options.body));
            const hopType = String((storeCall.options.headers && storeCall.options.headers['Content-Type']) || '');
            const hopHead = storeCall.options.body.slice(0, 400).toString('latin1');
            assert.match(hopType, /multipart\/form-data/i, 'pulled bytes must hop as multipart');
            assert.doesNotMatch(hopType, /application\/json/i, 'live fail: JSON content-type on a multipart audio body never reaches the store');
            assert.match(hopHead, /name="audio"/, 'store hop field must be audio');
            assert.match(hopHead, /filename="song.wav"/i);
            assert.ok(storeCall.options.body.indexOf(Buffer.from('RIFF....WAVE')) !== -1);
            ...
            assert.ok(failCalls.some((call) => /\/tracks\/33333333-3333-4333-8333-333333333333\/audio/.test(call.url)));
            ...
            const mp3Head = mp3Store.options.body.slice(0, 500).toString('latin1');
            assert.match(mp3Head, /filename="song.wav"/i, 'only an MP3 hop reconverts on the server');
            assert.ok(mp3Store.options.body.indexOf(Buffer.from('RIFF')) !== -1);
          } finally {
            global.fetch = originalFetch;
          }
        });

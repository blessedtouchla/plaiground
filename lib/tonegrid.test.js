...
const tracks = require('../api/tonegrid/tracks');
const trackAudio = require('../api/tonegrid/tracks/[id]/audio');
...
async function withAccountUser(attrs, fn) {
  ...
  const row = await accounts.createUser({
...
async function run() {
  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        assert.deepStrictEqual(trackAudio.config, { api: { bodyParser: false } });
        const res = mockRes();
        await trackAudio({
          method: 'POST',
  ...
  const files = [
    ...
    'api/tonegrid/tracks.js',
    'api/tonegrid/tracks/[id]/audio.js',
    'api/me/catalog.js',
    'tonegrid.js',
    'earnings.js',
    'catalog.js',

...
async function run() {
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        if (String(url).includes('/releases/' + mine) && options && options.method === 'PUT') {
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          ...
          await submitRelease({
            ...
            headers,
            id: mine,
            body: { document_id: 'doc_split_sheet_01' },
          }, res);
          assert.strictEqual(res.statusCode, 200);
          const body = json(res);
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url) {
        ...
        return { ok: true, status: 200, json: async () => ({ release: { uuid: '11111111-1111-4111-8111-111111111111', status: 'draft' } }) };
      };
      try {
        await withAccountUser({ releaseId: '11111111-1111-4111-8111-111111111111' }, async (_row, headers) => {
          ...
          assert.strictEqual(json(res).code, 'SIGNWELL_TRIAL');
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        calls.push({ url: String(url), method: options && options.method, body: options && options.body });
        if (options && options.method === 'PUT' && String(url).includes('/releases/' + mine)) {

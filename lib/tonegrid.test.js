    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 201);
          assert.strictEqual(json(res).data.uuid, created);
          assert.ok(!json(res).continued);
          assert.ok(calls.some((call) => String((call.options && call.options.method) || '') === 'DELETE' && String(call.url).includes('/releases/' + leftover)));
          assert.ok(!calls.some((call) => String((call.options && call.options.method) || '') === 'DELETE' && /Lightning|Dolly|Metete|Cgi|vhnjuk/i.test(String(call.url))));
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 201);
          assert.strictEqual(json(res).data.uuid, created);
          assert.ok(!json(res).continued, 'Lightning catalog id must not be continued over');
          assert.ok(!json(res).uuid || json(res).uuid === created);
          const deleted = calls.filter((call) => String((call.options && call.options.method) || '') === 'DELETE');
          assert.strictEqual(deleted.length, 1);
          assert.ok(String(deleted[0].url).includes('/releases/' + leftover));

...
function languages(req, res) {
  return tonegridApi(Object.assign({ url: '/api/tonegrid/languages' }, req), res);
}
function genres(req, res) {
  return tonegridApi(Object.assign({ url: '/api/tonegrid/genres' }, req), res);
}
function storesRoute(req, res) {
  return tonegridApi(Object.assign({ url: '/api/tonegrid/stores' }, req), res);
}
function publishing(req, res) {
  return tonegridApi(Object.assign({ url: '/api/tonegrid/publishing' }, req), res);
}
function boost(req, res) {
  return tonegridApi(Object.assign({ url: '/api/tonegrid/boost' }, req), res);
}
...
async function withAccountUser(attrs, fn) {
  ...
  } else if (attrs && (attrs.artistId || attrs.releaseId)) {
...
async function run() {
    ...
    async () => {
      ...
      try {
        await withAccountUser({}, async (row, headers) => {
          ...
          assert.strictEqual(sent.language, undefined);
          assert.strictEqual(sent.release_date, undefined);
          assert.strictEqual(sent.stores, undefined);
          assert.ok(calls[0].url.endsWith('/releases'));
          const dspCall = calls.find((call) => /\/releases\/[^/]+\/dsps$/.test(String(call.url)));
          assert.ok(dspCall);
          const dspBody = JSON.parse(dspCall.options.body);
          assert.ok(dspBody.dsps.indexOf('youtube-music') !== -1);
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          await artists({ method: 'POST', headers, body: { name: 'Ada Night' } }, artistBlocked);
  ...
  await withEnv(
    ...
    async () => {
      ...
      try {
        await withAccountUser({ plan: 'creator', releaseId: mine }, async (_row, headers) => {
          const res = mockRes();
          await analytics({
            method: 'GET',
            headers,
  ...
  const langRes = mockRes();
  await languages({ method: 'GET', headers: {} }, langRes);
  assert.strictEqual(langRes.statusCode, 200);
  assert.ok(json(langRes).languages.length >= 180);
  assert.ok(json(langRes).languages.some((row) => row.code === 'en' && row.name === 'English'));
  assert.strictEqual(json(langRes).source, 'iso-639-1');

  const genreRes = mockRes();
  await genres({ method: 'GET', headers: {} }, genreRes);
  assert.strictEqual(genreRes.statusCode, 200);
  assert.strictEqual(json(genreRes).subgenre, false);
  assert.ok(json(genreRes).genres.length > 40);
  assert.ok(json(genreRes).genres.indexOf('Afrobeats') !== -1);
  assert.ok(JSON.stringify(json(genreRes).genres) !== JSON.stringify([
    'Electronic', 'Pop', 'Hip-Hop', 'Country', 'R&B', 'Latin', 'Other',
  ]));

  const storeRes = mockRes();
  await storesRoute({ method: 'GET', headers: {} }, storeRes);
  assert.strictEqual(storeRes.statusCode, 200);
  assert.ok(json(storeRes).stores.some((row) => row.slug === 'youtube-music' && /youtube music/i.test(row.name)));
  assert.strictEqual(json(storeRes).youtube_music, 'youtube-music');

  await withAccountUser({ plan: 'basic' }, async (_row, headers) => {
    const blockedPub = mockRes();
    await publishing({ method: 'POST', headers, body: {} }, blockedPub);
    assert.strictEqual(blockedPub.statusCode, 403);
    assert.strictEqual(json(blockedPub).code, 'PLAN_FORBIDDEN');
    assert.strictEqual(json(blockedPub).feature, 'publishing');

    const blockedBoost = mockRes();
    await boost({ method: 'POST', headers, body: {} }, blockedBoost);
    assert.strictEqual(blockedBoost.statusCode, 403);
    assert.strictEqual(json(blockedBoost).code, 'PLAN_FORBIDDEN');
  });

  await withAccountUser({ email: 'creator-pub@example.com', plan: 'creator' }, async (_row, headers) => {
    const okPub = mockRes();
    await publishing({ method: 'POST', headers, body: {} }, okPub);
    assert.strictEqual(okPub.statusCode, 200);
    assert.strictEqual(json(okPub).ok, true);

    const noBoost = mockRes();
    await boost({ method: 'POST', headers, body: {} }, noBoost);
    assert.strictEqual(noBoost.statusCode, 403);
    assert.strictEqual(json(noBoost).code, 'BOOST_UNAVAILABLE');
  });
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        calls.push({ url: String(url), options: options || {} });
        if (String(url).includes('/supply-chain/dsps')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ data: { dsps: [{ slug: 'youtube-music', name: 'YouTube Music' }, { slug: 'spotify', name: 'Spotify' }] } }),
          };
        }
        return {
          ok: true,
      ...
      try {
        await withAccountUser({ email: 'lang@example.com', plan: 'pro' }, async (_row, headers) => {
          const res = mockRes();
          await releases({
            method: 'POST',
            headers,
          ...
          assert.strictEqual(res.statusCode, 201);
          const created = calls.find((call) => call.options.method === 'POST' && String(call.url).endsWith('/releases'));
          const sent = JSON.parse(created.options.body);
          assert.strictEqual(sent.language, 'en');
          assert.strictEqual(sent.genre, 'Afrobeats');
          assert.strictEqual(sent.stores, undefined);
          assert.strictEqual(sent.subgenre, undefined);
          const dspCall = calls.find((call) => /\/releases\/[^/]+\/dsps$/.test(String(call.url)));
          const dsps = JSON.parse(dspCall.options.body).dsps;
          assert.ok(dsps.indexOf('spotify') !== -1);
          assert.ok(dsps.indexOf('youtube-music') !== -1);
        });
      } finally {
        global.fetch = originalFetch;
      }
      ...
      global.fetch = async function mockFetch(url) {
        ...
        if (String(url).includes('/analytics/releases')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({ data: [{ release_uuid: mine, title: 'Night Drive', streams: 12 }] }),
          };
        }
        return { ok: true, status: 200, json: async () => ({ data: [{ dsp: 'Spotify', streams: 12 }] }) };
      };
      try {
        await withAccountUser({ email: 'basic-analytics@example.com', plan: 'basic', releaseId: mine }, async (_row, headers) => {
          const res = mockRes();
          await analytics({ method: 'GET', headers, query: {} }, res);
          const body = json(res);
          assert.strictEqual(body.plan, 'basic');
          assert.strictEqual(body.summary.total_streams, 12);
          assert.deepStrictEqual(body.dsps, []);
          assert.deepStrictEqual(body.territories, []);
          assert.deepStrictEqual(body.releases, []);
          assert.strictEqual(body.summary.top_dsp, '');
  ...
  const files = [
    ...
    'lib/plans.js',
    'lib/plans.test.js',
    'lib/tonegrid-lists.js',
    'lib/route.js',
    'account.js',
    'lib/tonegrid-lists.test.js',
    'api/auth.js',
    'lib/mail.js',
    'vercel.json',
  ];

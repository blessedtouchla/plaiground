    ...
    async () => {
      ...
      try {
        await withAccountUser({}, async (row, headers) => {
          ...
          assert.ok(calls.some((call) => /\/rights$/.test(String(call.url)) && String((call.options && call.options.method) || '') === 'PUT'));
          ...
          const datedSent = JSON.parse(releaseCreateCalls(calls)[1].options.body);
    ...
    async () => {
      ...
      try {
        await withAccountUser({}, async (_row, headers) => {
          ...
          assert.ok(calls.some((call) => /\/writers$/.test(String(call.url)) && String((call.options && call.options.method) || 'POST') === 'POST'));
          assert.ok(calls.some((call) => /\/tracks\/[^/]+\/writers$/.test(String(call.url)) && String((call.options && call.options.method) || '') === 'PUT'));
    ...
    async () => {
      ...
      try {
        ...
        await withAccountUser({
          ...
          assert.strictEqual(albumRes.statusCode, 201);
          assert.strictEqual(releaseCreateCalls(calls).length, 1, 'album must create a new ToneGrid release, not reuse the single');

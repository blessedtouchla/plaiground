...
async function run() {
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 201);
          assert.ok(calls.some((call) => String(call.url).indexOf('/releases/22222222-2222-4222-8222-222222222222') !== -1));
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        calls.push({ url: String(url), method: options && options.method });
        if (options && options.method === 'POST' && String(url).includes('/takedown') && !String(url).includes('/ddex/')) {
          return { ok: true, status: 202, json: async () => ({ message: 'Takedown notice sent to 5 DSP(s).' }) };
        }
        if (String(url).includes('/ddex/deliveries')) {
      ...
      try {
        await withAccountUser({ plan: 'creator', releaseId: mine }, async (row, headers) => {
          ...
          assert.ok(calls.some((call) => call.method === 'POST' && /\/releases\/[^/]+\/takedown$/.test(String(call.url))));
    ...
    async () => {
      ...
      try {
        await withAccountUser({ plan: 'creator', releaseId: mine }, async (row, headers) => {
          ...
          await oneRelease({ method: 'DELETE', headers, id: mine }, res);
          assert.strictEqual(res.statusCode, 202);
          assert.strictEqual(json(res).takedown, true);
          assert.strictEqual(json(res).removed, false);
          assert.ok(calls.some((call) => call.method === 'POST' && String(call.url).includes('/ddex/purge')));
          assert.ok(!calls.some((call) => call.method === 'DELETE'));
          const next = await accounts.findById(row.id);
          assert.deepStrictEqual(next.tonegrid_release_ids, [mine]);
        });
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        if (options && options.method === 'POST' && String(url).includes('/ddex/purge')) {
          return { ok: false, status: 422, json: async () => ({ error: 'Release is not live.' }) };
        }
        if (String(url).includes('/ddex/deliveries')) {
          return { ok: true, status: 200, json: async () => ({ deliveries: [] }) };
        }
      ...
      try {
        await withAccountUser({ plan: 'creator', releaseId: mine }, async (row, headers) => {
          ...
          assert.strictEqual(json(res).error, 'Release is not live.');
          assert.ok(!/only draft or rejected releases can be deleted/i.test(json(res).error));
          assert.ok(!calls.some((call) => call.method === 'DELETE'));
          const next = await accounts.findById(row.id);
          assert.deepStrictEqual(next.tonegrid_release_ids, [mine]);
        });

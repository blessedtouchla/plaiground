...
async function run() {
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url) {
        calls.push(String(url));
        if (/\/releases\/11111111-1111-4111-8111-111111111111(?:\?|$)/.test(String(url))) {
          return {
            ok: true,
            status: 200,
        ...
        if (String(url).includes('/analytics/summary')) {
          return {
            ...
            json: async () => ({
              ...
              top_release: { uuid: theirs, title: 'Tenant Hit', streams: 9000 },
              top_dsp: 'Spotify',
            }),
          };
        }
        ...
        if (String(url).includes('/royalties/statements')) {
          return { ok: true, status: 200, json: async () => ({ statements: [] }) };
        }
  ...
  assert.ok(sources.includes('/api/tonegrid/webhook'));
  assert.strictEqual(
    (vercel.rewrites || []).find((row) => row.source === '/api/tonegrid/webhook').destination,
    '/api/tonegrid?resource=webhook'
  );

...
async function run() {
  ...
  await withEnv(
    ...
    async () => {
      ...
      const calls = [];
      const existingArtistId = '04c74127-11a8-40cf-beec-d1ffa16abd70';
      const spotifyUrl = 'https://open.spotify.com/artist/0TnOYISbd1XYRBk9myaseg';
      global.fetch = async function mockFetch(url, options) {
        const method = String((options && options.method) || 'GET').toUpperCase();
        calls.push({ url: String(url), method: method, options: options || {} });
        if (method === 'GET' && String(url).indexOf('/artists/' + existingArtistId) !== -1) {
        ...
        if ((method === 'PATCH' || method === 'PUT') && String(url).indexOf('/artists/' + existingArtistId) !== -1) {
          const sent = JSON.parse((options && options.body) || '{}');
          return { ok: true, status: 200, json: async () => Object.assign({ uuid: existingArtistId }, sent) };
        }
        if (method === 'POST' && /\/artists$/.test(String(url))) {
        ...
        await withAccountUser({
          ...
          await accounts.updateProfile(row.id, {
            profile: profile.upsertArtist(stored, Object.assign({}, stored.artists[0], {
              platform_links: [{ platform: 'spotify', url: spotifyUrl }],
            })),
          });
          const res = mockRes();
          await artists({
          ...
          assert.strictEqual(json(res).uuid, existingArtistId);
          assert.strictEqual(json(res).continued, true);
          const patches = calls.filter((call) => (
            (call.method === 'PATCH' || call.method === 'PUT')
            && String(call.url).indexOf('/artists/' + existingArtistId) !== -1
          ));
          assert.ok(patches.length >= 1, 'first-link continue must merge saved mapping URLs onto the store artist');
          const sent = JSON.parse((patches[0].options && patches[0].options.body) || '{}');
          assert.strictEqual(sent.spotify_url, spotifyUrl);
    ...
    async () => {
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          ...
          assert.ok(!emptyContribs.some((row) => row.role === 'Performer' || row.role === 'Producer'), 'empty collected credits must not invent a performer or producer');
        });
      } finally {
        global.fetch = originalFetch;
        if (prevKey === undefined) delete process.env.SIGNWELL_API_KEY;

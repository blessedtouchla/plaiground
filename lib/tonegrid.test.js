...
async function run() {
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(json(reusedDead).uuid, liveId);
          assert.ok(!json(reusedDead).error);
          assert.ok(!json(reusedDead).continued, 'stale leftover id must not be returned as continued');
          assert.ok(!/tenant/i.test(reusedDead.body));
          const artistPostAfter = calls.filter((call) => call.method === 'POST' && /\/artists$/.test(call.url));
          assert.strictEqual(artistPostAfter.length, 2, 'stale leftover store id falls through to one create');
          const leftoverCreated = await accounts.findById(row.id);
          const leftoverStored = profile.recoverRoster(profile.readStored(leftoverCreated), leftoverCreated.artist_name, leftoverCreated.tonegrid_artist_id);
          const leftoverArtist = profile.findArtist(leftoverStored, 'pg-leftover');
          assert.strictEqual(leftoverArtist.tonegrid_artist_id, liveId, 'stale leftover id must attach the new live store artist');
  ...
  await withEnv(
    ...
    async () => {
      ...
      const calls = [];
      const liveId = 'cccccccc-cccc-4ccc-8ccc-cccccccccccc';
      const strayHint = 'dddddddd-dddd-4ddd-8ddd-dddddddddddd';
      global.fetch = async function mockFetch(url, options) {
        const method = String((options && options.method) || 'GET').toUpperCase();
        calls.push({ url: String(url), method: method, options: options || {} });
        if (method === 'GET' && String(url).indexOf('/artists/' + strayHint) !== -1) {
          return { ok: false, status: 404, json: async () => ({ error: 'artist not found in this tenant' }) };
        }
        if (method === 'POST' && /\/artists$/.test(String(url))) {
          const sent = JSON.parse((options && options.body) || '{}');
          assert.strictEqual(sent.name, 'Hint Act');
          assert.ok(sent.slug);
          assert.ok(!sent.artist_id, 'store create payload must not include a local artist_id');
          return { ok: true, status: 201, json: async () => ({ uuid: liveId, name: 'Hint Act' }) };
        }
        const credit = creditFollowupResponse(url, options);
        if (credit) return credit;
        throw new Error('unexpected store call ' + method + ' ' + String(url));
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 201);
          assert.strictEqual(json(res).uuid, liveId);
          assert.ok(!json(res).continued, 'a uuid missing from the local roster must not continue as a store artist');
          assert.ok(!calls.some((call) => (
            call.method === 'GET' && String(call.url).indexOf('/artists/' + strayHint) !== -1
          )), 'must not GET a hint uuid until matchingTonegridArtist confirms it is a store artist');
          const artistPosts = calls.filter((call) => call.method === 'POST' && /\/artists$/.test(call.url));
          assert.strictEqual(artistPosts.length, 1);
          const localRow = mockRes();
          await artists({
            method: 'POST',
            headers,
            body: {
          ...
          assert.ok(!calls.some((call) => (
            call.method === 'GET' && String(call.url).indexOf('/artists/' + row.id) !== -1
          )), 'must never continue the local row.id as a store artist');
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  const matchingFn = tonegridSrc.match(/function matchingTonegridArtist\(row, body\) \{[\s\S]*?\n\}/);
  assert.ok(matchingFn, 'matchingTonegridArtist must stay a real function');
  assert.ok(!matchingFn[0].includes('if (!asLocal) return artistHint'), 'never continue a hint uuid just because it is missing from the local roster');
  assert.ok(matchingFn[0].includes('row.id') || matchingFn[0].includes('localRowId'), 'matchingTonegridArtist must not return the local row.id');
  assert.ok(tonegridSrc.includes('const payload = { name, slug }'), 'createArtist store payload stays { name, slug }');
  assert.ok(tonegridSrc.includes("tonegridFetch('/artists/' + continueId"), 'continue still GETs /artists/{id} before reuse');

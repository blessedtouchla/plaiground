    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        if (method === 'POST' && /\/releases$/.test(path.split('?')[0])) {
          ...
          return { ok: true, status: 201, json: async () => ({ success: true, data: { uuid: created, title: 'Rainbow Road' } }) };
        }
        if (method === 'GET' && path.indexOf('/releases/7a928125-b12e-4609-bd37-26ce0edf819e') !== -1) {
          return { ok: false, status: 404, json: async () => ({ error: 'Release not found.' }) };
        }
        if (method === 'GET' && path.indexOf('/releases/' + leftover) !== -1) {
          return {
        ...
        if (method === 'GET' && /\/releases(\?|$)/.test(path) && !/\/releases\/[0-9a-f-]{36}/i.test(path)) {
          return {
            ...
            json: async () => ({
              data: [
                { uuid: lightning, title: 'Lightning', status: 'draft', artist_id: artistId },
                { uuid: leftover, title: 'Rainbow Road', status: 'draft', artist_id: artistId },
              ],
              total: 2,
              page: 1,
              per_page: 100,
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 200);
          assert.strictEqual(json(res).uuid, leftover);
          assert.strictEqual(json(res).continued, true, 'same-title leftover attaches even if credits patch fails');
          assert.ok(!calls.some((call) => String((call.options && call.options.method) || '') === 'DELETE'), 'must not delete a same-title leftover');

...
async function run() {
  ...
  assert.strictEqual(
    ...
    'This artist is already on the store.'
  );
  assert.strictEqual(
    tonegrid.tonegridErrorMessage({ error: 'Artist slug already taken.' }),
    'This artist is already on the store.'
  );
  assert.strictEqual(
    tonegrid.tonegridErrorMessage({ error: 'DSP `youtube-music` is not found or inactive' }),
    tonegrid.INACTIVE_STORE_COPY
  );
  assert.strictEqual(
    tonegrid.tonegridErrorMessage({ error: 'store not found or inactive' }),
    tonegrid.INACTIVE_STORE_COPY
  );
  assert.ok(!/tonegrid|distributor|not found or inactive/i.test(tonegrid.INACTIVE_STORE_COPY));
  assert.ok(!/ToneGrid/.test(tonegrid.tonegridErrorMessage({ error: 'DSP `youtube-music` is not found or inactive' })));
  ...
  assert.deepStrictEqual(
    tonegrid.parseStoreSlugs({
      data: {
        dsps: [
          { slug: 'spotify', name: 'Spotify', active: true },
          { slug: 'youtube-music', name: 'YouTube Music', active: false, status: 'inactive' },
          { slug: 'tidal', name: 'Tidal', is_active: false },
          { slug: 'deezer', name: 'Deezer', status: 'inactive' },
          { slug: 'apple-music', name: 'Apple Music' },
        ],
      },
    }),
    ['spotify', 'apple-music'],
    'live catalog must drop inactive DSPs and hard-exclude youtube-music'
  );
  assert.deepStrictEqual(
    tonegrid.parseStoreSlugs({ data: { stores: ['spotify', 'YouTube Music', 'youtube-music', 'tidal'] } }),
    ['spotify', 'tidal'],
    'catalog must hard-exclude YouTube Music labels even when the partner still returns them'
  );
  assert.ok(tonegrid.documentedStores().indexOf('youtube-music') === -1);
  assert.deepStrictEqual(tonegrid.selectableDspSlugs(['spotify', 'youtube-music', 'tidal']), ['spotify', 'tidal']);
  assert.deepStrictEqual(tonegrid.withYouTubeMusic(['spotify']), ['spotify']);
  await withEnv(
    { key: 'test-key-value-not-for-commit', base: 'https://api-sandbox.tonegrid.pro/api' },
    async () => {
      ...
      try {
        ...
        assert.ok(!body.stores.some((row) => row.slug === 'youtube-music'), 'live store catalog must not offer youtube-music');
      } finally {
  ...
  await withEnv(
    ...
    async () => {
      const originalFetch = global.fetch;
      global.fetch = async function mockFetch(url) {
        if (String(url).indexOf('/supply-chain/dsps') === -1) {
          return { ok: false, status: 404, json: async () => ({ error: 'Endpoint not found.' }) };
        }
        return {
          ok: true,
          ...
          json: async () => ({
            ...
            data: {
              dsps: [
                ...
                { slug: 'youtube-music', name: 'YouTube Music', status: 'inactive' },
                { slug: 'apple-music', name: 'Apple Music' },
              ],
            },
            meta: { total: 3, current_page: 1, last_page: 1, per_page: 100 },
          }),
        };
      };
      try {
        ...
        assert.strictEqual(res.statusCode, 200);
        const body = json(res);
        assert.deepStrictEqual(body.stores.map((row) => row.slug), ['spotify', 'apple-music']);
      } finally {
        global.fetch = originalFetch;
      }
    }
    ...
    async () => {
      ...
      try {
        ...
        await withAccountUser({ email: 'nolegal@example.com', noLegal: true }, async (_row, headers) => {
          ...
          assert.strictEqual(missing.statusCode, 201, 'collected artist name is enough to create; do not invent legal names');
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        const method = String((options && options.method) || 'GET').toUpperCase();
        if (method === 'POST' && /\/releases$/.test(path.split('?')[0])) {
          return { ok: true, status: 201, json: async () => ({ uuid: minted, title: 'I Set the Tone' }) };
        }
        if (method === 'GET' && path.indexOf('/releases/' + leftover) !== -1) {
          throw new Error('new Vickilicious I Set the Tone must not continue leftover ' + leftover);
        }
        if (method === 'DELETE') {
          throw new Error('must not delete leftover for a new artist ' + path);
        }
        const credit = creditFollowupResponse(url, options);
        if (credit) return credit;
        throw new Error('unexpected store call ' + method + ' ' + path);
      ...
      try {
        await withAccountUser({
          email: 'vickilicious-iset-the-tone@example.com',
          artist: 'Victoria PLAIGROUND',
          plan: 'creator',
          artistId: artistId,
          releases: [lightning],
          ...
          assert.strictEqual(res.statusCode, 201, 'new artist I Set the Tone must mint, not leftover-continue');
          assert.strictEqual(json(res).uuid, minted);
          assert.ok(!json(res).continued);
          assert.ok(!json(res).error || !/already exists/i.test(String(json(res).error)));
          assert.ok(releaseCreateCalls(calls).length >= 1, 'must POST create for Vickilicious');
          assert.ok(!calls.some((call) => String(call.url).includes(leftover)));
          const stored = await accounts.findById(row.id);
          assert.ok(stored.tonegrid_release_ids.includes(minted));
          assert.ok(!stored.tonegrid_release_ids.includes(leftover));
        });
  ...
  // Store 409 "A record with these details already exists." (not slug-worded)
  // after Create new artist Vickilicious — adopt the listed artist and continue.
  await withEnv(
    { key: 'test-key-value-not-for-commit', base: 'https://api.tonegrid.pro/api' },
    async () => {
      const originalFetch = global.fetch;
      const calls = [];
      const existingArtistId = 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb';
      global.fetch = async function mockFetch(url, options) {
        const method = String((options && options.method) || 'GET').toUpperCase();
        calls.push({ url: String(url), method: method, options: options || {} });
        if (method === 'POST' && /\/artists$/.test(String(url))) {
        ...
        if (method === 'GET' && /\/artists(\?|$)/.test(String(url))) {
          return {
            ...
            status: 200,
            json: async () => ({
              data: [{ uuid: existingArtistId, name: 'Vickilicious', slug: 'vickilicious' }],
              total: 1,
              page: 1,
              per_page: 100,
            }),
      ...
      try {
        await withAccountUser({
          email: 'vickilicious-artist-409@example.com',
          plan: 'creator',
          artist: 'Vickilicious',
        }, async (row, headers) => {
          const stored = profile.recoverRoster(profile.readStored(row), row.artist_name, row.tonegrid_artist_id);
          const localArtistId = stored.artists[0].id;
          assert.ok(!stored.artists[0].tonegrid_artist_id, 'roster starts without a store artist id');

          const res = mockRes();
          await artists({
            method: 'POST',
            headers,
            body: { name: 'Vickilicious', plaiground_artist_id: localArtistId },
          }, res);
          assert.strictEqual(res.statusCode, 200);
          assert.strictEqual(json(res).uuid, existingArtistId);
          assert.strictEqual(json(res).continued, true, 'generic 409 must adopt the existing artist, not fail');
          assert.ok(!json(res).error);
          assert.ok(!/already exists|slug|ToneGrid/i.test(res.body));

          const artistPosts = calls.filter((call) => call.method === 'POST' && /\/artists$/.test(call.url));
          assert.strictEqual(artistPosts.length, 1, 'must not retry POST /artists after finding the existing one');

          const after = await accounts.findById(row.id);
          assert.strictEqual(after.tonegrid_artist_id, existingArtistId);
        });
      } finally {
        global.fetch = originalFetch;
      }
    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.strictEqual(res.statusCode, 409);
          assert.strictEqual(json(res).error, 'This artist is already on the store.');
          assert.ok(!json(res).continued);
          assert.ok(!json(res).uuid, 'must not adopt an unrelated artist just because a slug collided');
          assert.ok(!/slug|ToneGrid|tonegrid/i.test(res.body));
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  await withEnv(
    ...
    async () => {
          ...
          return {
            ...
            json: async () => ({
              release: {
                ...
                approved_at: '2026-09-08T16:10:57Z',
              },
            }),
          };
        }
        if (String(url).includes('/releases/' + pending) && !String(url).includes('/ddex/')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({
              release: { uuid: pending, title: 'Waiting', type: 'single', status: 'pending' },
            }),
          };
        }
        if (String(url).includes('/releases/' + rejected) && !String(url).includes('/ddex/')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (row, headers) => {
          await accounts.updateCatalog(row.id, { releaseId: pending });
          await accounts.updateCatalog(row.id, { releaseId: rejected });
          const res = mockRes();
          await releases({ method: 'GET', headers, query: {} }, res);
          assert.strictEqual(res.statusCode, 200, res.body);
          const listed = json(res).releases;
          const liveRow = listed.find((item) => item.uuid === mine);
          const pendingRow = listed.find((item) => item.uuid === pending);
          const rejectedRow = listed.find((item) => item.uuid === rejected);
          assert.ok(liveRow, 'approved catalog row is listed');
          assert.strictEqual(liveRow.delivered_at, '2026-09-08T16:10:57Z', 'list proxy must pass delivered_at through');
          const releaseStatus = require('./release-status');
          assert.strictEqual(releaseStatus.displayInfo(liveRow).label, 'Live');
          assert.strictEqual(releaseStatus.displayInfo(liveRow).live, true);
          assert.strictEqual(pendingRow.status, 'pending');
          assert.strictEqual(pendingRow.delivered_at, '');
          assert.strictEqual(releaseStatus.displayInfo(pendingRow).label, 'PLAIGROUND QC');
          assert.strictEqual(releaseStatus.displayInfo(rejectedRow).label, 'QC rejected');
        });
      } finally {
        global.fetch = originalFetch;
      }
    ...
    async () => {
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (row, headers) => {
          ...
          assert.ok(listed.writers && listed.writers.length, 'writers come from persist or track contributors');
          assert.ok(listed.c_line || listed.copyright_line, '©/℗ line comes from persist or GET /rights');
          const releaseStatus = require('./release-status');
          assert.strictEqual(releaseStatus.problemAlert(listed), '');
          assert.strictEqual(releaseStatus.displayInfo(listed).label, 'PLAIGROUND QC');
        });
      } finally {
        global.fetch = originalFetch;
      }
    ...
    async () => {
        ...
        return {
          ...
          json: async () => ({
            release: {
              ...
              title: 'Night Drive',
              status: 'draft',
              tracks: [{ uuid: '33333333-3333-4333-8333-333333333333', title: 'Night Drive' }],
            },
          }),
        };
      };
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          ...
          assert.strictEqual(res.statusCode, 400);
          assert.strictEqual(json(res).error, 'Audio required — upload your master before sending');
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        if (String(url).includes('/releases/' + mine)) {
          return { ok: true, status: 200, json: async () => ({ release: { uuid: mine, title: 'Night Drive', status: 'draft', artwork_url: 'https://cdn.example/night-drive.jpg', tracks: [{ uuid: '33333333-3333-4333-8333-333333333333', title: 'Night Drive', audio_url: 'https://cdn.example/night-drive.wav' }] } }) };
        }
        return { ok: true, status: 200, json: async () => ({}) };
      };
      try {
    ...
    async () => {
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          ...
          assert.ok(body.dsps.indexOf('youtube-music') === -1, 'submit must not attach youtube-music');
          ...
          const fullAiPatch = fullAiHops.find((call) => {
            ...
            return String(call.body || '').includes('includes_ai') || String(call.body || '').includes('track_properties');
          });
          ...
          const credited = mockRes();
          const beforeCredited = calls.length;
          await submitRelease({
            method: 'POST',
            headers,
            id: mine,
          ...
          assert.strictEqual(credited.statusCode, 200, credited.body);
          const creditHops = calls.slice(beforeCredited);
          const contribPatch = creditHops.find((call) => {
            if (call.method !== 'PATCH' || !/\/tracks\/[^/]+$/.test(String(call.url))) return false;
          ...
          assert.strictEqual(emptyCredits.statusCode, 200, emptyCredits.body);
          const emptyHops = calls.slice(beforeEmpty);
          const emptyPatch = emptyHops.find((call) => {
            if (call.method !== 'PATCH' || !/\/tracks\/[^/]+$/.test(String(call.url))) return false;
            const body = JSON.parse(call.body || '{}');
            return Array.isArray(body.contributors);
          });
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        return { ok: true, status: 200, json: async () => ({ release: { uuid: mine, status: 'draft', artwork_url: 'https://cdn.example/night-drive.jpg', tracks: [{ uuid: '33333333-3333-4333-8333-333333333333', title: 'Night Drive', audio_url: 'https://cdn.example/night-drive.wav' }] } }) };
      };
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          const res = mockRes();
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        if (String(url).includes('/tracks/' + trackId) && (!options || !options.method || options.method === 'GET')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({
      ...
      try {
        await withAccountUser({ trackId: trackId }, async (_row, headers) => {
          const res = mockRes();
          await updateTrack({
            method: 'GET',
            headers,
          ...
          assert.strictEqual(res.statusCode, 200);
          assert.strictEqual(json(res).uuid, trackId);
          assert.strictEqual(json(res).audio_s3_key, 'tracks/28/2026/09/e166bab099116ab2da9aeebf64740fff.wav');
          assert.strictEqual(json(res).file_size, 36885522);
          assert.ok(calls.some(function (call) {
            return String(call.url).includes('/tracks/' + trackId) && call.method === 'GET';
          }), 'GET /tracks/:id refreshes the store track');
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        calls.push({ url: String(url), method: (options && options.method) || 'GET' });
        if (String(url).includes('/tracks/' + leftoverTrack)) {
          return { ok: true, status: 200, json: async () => ({ uuid: leftoverTrack, title: 'I Set the Tone' }) };
        }
        return { ok: false, status: 404, json: async () => ({ error: 'Track not found.' }) };
      };
      try {
        await withAccountUser({}, async (_row, headers) => {
          const leftoverRes = mockRes();
          await updateTrack({ method: 'GET', headers, id: leftoverTrack }, leftoverRes);
          assert.strictEqual(leftoverRes.statusCode, 200, 'preferred leftover GET must not 404 before catalog write');
          assert.strictEqual(json(leftoverRes).uuid, leftoverTrack);

          const missing = mockRes();
          await updateTrack({ method: 'GET', headers, id: foreign }, missing);
          assert.strictEqual(missing.statusCode, 404);
          assert.strictEqual(json(missing).error, 'Track not found.');
          assert.ok(!calls.some((call) => String(call.url).includes('/tracks/' + foreign)), 'unowned track must not hop the store');
        });
      } finally {
        global.fetch = originalFetch;
      }
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        const path = String(url);
        if (path.includes('/releases/' + leftover) && !path.includes('/submit') && !path.includes('/dsps')) {
          return {
            ok: true,
            status: 200,
            json: async () => ({
              ...
              tracks: [{
                uuid: leftoverTrack,
                title: 'I Set the Tone',
                audio_url: 'https://cdn.example/tone.wav',
                file_size: 4096,
              }],
            }),
          };
        }
        if (path.includes('/tracks/' + stale)) {
          return { ok: false, status: 404, json: async () => ({ error: 'Track not found.' }) };
        }
        if (path.includes('/releases/' + leftover + '/submit')) {
          return { ok: true, status: 200, json: async () => ({ status: 'pending' }) };
        }
        return { ok: true, status: 200, json: async () => ({}) };
      };
      try {
        await withAccountUser({ releaseId: leftover }, async (_row, headers) => {
          const res = mockRes();
          await submitRelease({
            method: 'POST',
            headers,
          ...
          assert.strictEqual(res.statusCode, 200, res.body);
          assert.ok(!calls.some((call) => String(call.url).includes(stale)), 'submit must not hop a stale body.track_id');
          assert.ok(calls.some((call) => {
            return /\/tracks\/[^/]+\/writers$/.test(String(call.url)) && String(call.url).includes(leftoverTrack);
          }), 'writers attach to the leftover store track');
        });
      } finally {
        global.fetch = originalFetch;
      }
    ...
    async () => {
      ...
      try {
        await withAccountUser({ releaseId: mine }, async (_row, headers) => {
          ...
          const patchKey = hopKeyOf(mutating.find((call) => hopKeyOf(call).startsWith('plaiground-patch-date-')));
          ...
          assert.strictEqual(
            hopKeyOf(retryMutating.find((call) => hopKeyOf(call).startsWith('plaiground-patch-date-'))),
            patchKey
          );
          assert.strictEqual(
    ...
    async () => {
      ...
      global.fetch = async function mockFetch(url, options) {
        ...
        return {
          ...
          status: 200,
          json: async () => ({
            release: { uuid: mine, title: 'Night Drive', status: 'draft', artwork_url: 'https://cdn.example/night-drive.jpg', dsps: ['spotify', 'youtube-music'], tracks: [{ uuid: '33333333-3333-4333-8333-333333333333', title: 'Night Drive', audio_url: 'https://cdn.example/night-drive.wav' }] },
          }),
        };
      };
      try {
    ...
    async () => {
      ...
      try {
        await withAccountUser({ plan: 'creator', releaseId: mine }, async (row, headers) => {
          ...
          assert.strictEqual(json(res).error, 'The store still has this release.');
  ...
  assert.ok(audioApi.includes("The store did not finish attaching the audio. Try again."));
  assert.ok(audioApi.includes('hopResult.timedOut || hopResult.status === 504'));
  assert.ok(!/Vercel Pro|upgrade to Pro/i.test(audioApi), 'must not require a Vercel Pro upgrade');
  ...
  await withEnv(
    ...
    async () => {
      ...
      global.fetch = function mockFetch(url) {
        if (String(url).includes('/releases/' + leftover) && !/\/(rights|ddex|tracks|dsps|artwork)/.test(String(url))) {
          return new Promise(function () {});
        }
  ...
  assert.ok(tonegridSrc.includes('bodyMayAdoptKnown'), 'known leftover continue is gated for a new named artist');
  ...
  assert.ok(tonegridSrc.includes('function collectSubmitTrackIds'));
  assert.ok(tonegridSrc.includes('function trustedBodyTrackId'));
  assert.ok(tonegridSrc.includes('isPreferredLeftoverTrackId(trackId)'));
  ...
  assert.ok(apexRedirects.some((row) => row.source === '/login' && row.destination === 'https://www.wannaplai.com/login'), 'apex /login must 308 to www so the session cookie is not host-only on the wrong host');
  const membershipHeader = (vercel.headers || []).find((row) => row.source === '/membership.js');
  assert.ok(membershipHeader && membershipHeader.headers.some((item) => item.key === 'Cache-Control' && /no-store/.test(item.value)), 'membership.js is cache-busted');
  ...
  assert.strictEqual(fs.readdirSync(path.join(root, 'api')).filter((name) => name.endsWith('.js')).length, 8);

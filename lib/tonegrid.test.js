    ...
    async () => {
      ...
      try {
        await withAccountUser({
          ...
          assert.ok(!json(reusedDead).uuid, 'stale leftover id must not remint a new store artist');
          ...
          assert.strictEqual(artistPostAfter.length, 1, 'stale leftover store id must not remint');

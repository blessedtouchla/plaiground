...
const artistCheck = require('./artist-check');
...
async function run() {
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    await meProfile({
      ...
      body: {
        artist: 'Ada Remix',
        profile: { legal_name: 'Ada Lovelace', country: 'Canada', username: 'ada_remix' },
      },
    }, accountFields);
    ...
    assert.strictEqual(json(accountFields).username, 'ada_remix');
    assert.strictEqual(json(accountFields).profile.username, 'ada_remix');
    ...
    assert.strictEqual(json(fieldsReload).username, 'ada_remix', 'GET /api/me must return the saved username after reload');
    ...
    assert.strictEqual(json(photoKeepsFields).profile.username, 'ada_remix', 'photo-only save must keep username');
    assert.strictEqual(json(photoKeepsFields).artist, 'Ada Remix');

    const otherSignup = mockRes();
    await signup({ method: 'POST', headers: {}, body: { email: 'other.handle@example.com', password: 'password1', username: 'ada_remix' } }, otherSignup);
    assert.strictEqual(otherSignup.statusCode, 409);
    assert.strictEqual(json(otherSignup).code, 'USERNAME_TAKEN');
    assert.strictEqual(json(otherSignup).error, 'That username is taken.');

    const takenSave = mockRes();
    await signup({ method: 'POST', headers: {}, body: { email: 'taken.save@example.com', password: 'password1', username: 'other_handle' } }, takenSave);
    assert.strictEqual(takenSave.statusCode, 200);
    const takenRow = await accounts.findByEmail('taken.save@example.com');
    await accounts.confirmEmail('taken.save@example.com');
    const takenCookie = auth.COOKIE + '=' + auth.signSession(takenRow.id);
    const clash = mockRes();
    await meProfile({
      method: 'POST',
      headers: { cookie: takenCookie },
      body: { profile: { username: 'ADA_REMIX' } },
    }, clash);
    assert.strictEqual(clash.statusCode, 409);
    assert.strictEqual(json(clash).code, 'USERNAME_TAKEN');

    const usernameLogin = mockRes();
    await login({ method: 'POST', headers: {}, body: { email: 'ada_remix', password: 'password1' } }, usernameLogin);
    assert.strictEqual(usernameLogin.statusCode, 401, 'sign-in stays email; username is not a login');
    const usernameOnly = mockRes();
    await login({ method: 'POST', headers: {}, body: { username: 'ada_remix', password: 'password1' } }, usernameOnly);
    assert.strictEqual(usernameOnly.statusCode, 401, 'login body.username must not sign in');

    const createdArtist = mockRes();
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', name: 'Ada Night', legal_first: 'Ada', legal_last: 'Night' },
    }, createdArtist);
    ...
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', name: 'Sia', legal_first: 'Ada', legal_last: 'Night' },
    }, yellow);
    ...
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', name: 'Sia', confirm_different: true, legal_first: 'Ada', legal_last: 'Night' },
    }, yellowOk);
    ...
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', name: 'Drake', legal_first: 'Ada', legal_last: 'Night' },
    }, red);
    ...
    assert.ok(Array.isArray(json(linked).created.platform_links));
    assert.strictEqual(json(linked).created.platform_links[0].platform, 'spotify');
    ...
    try {
      ...
      assert.strictEqual(mailCalls.length, 2);
      const mailedBodies = mailCalls.map((call) => JSON.parse(call.init.body));
      const confirmBody = mailedBodies.find((body) => String(body.subject || '').indexOf('Confirm') !== -1);
      const welcomeBody = mailedBodies.find((body) => String(body.subject || '').indexOf('You’re in') !== -1);
      assert.ok(confirmBody);
      assert.ok(welcomeBody);
      assert.strictEqual(confirmBody.from, 'PLAIGROUND <confirm@wannaplai.com>');
      assert.ok(confirmBody.text.indexOf('email=mailer%40example.com') !== -1);
      assert.ok(confirmBody.text.indexOf('token=') !== -1);
      assert.strictEqual(welcomeBody.from, 'PLAIGROUND <confirm@wannaplai.com>');
      assert.ok(welcomeBody.text.indexOf('One song. No card.') !== -1);
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', memory: true }, async () => {
    ...
    const apexRemember = mockRes();
    await login({
      method: 'POST',
      headers: { host: 'wannaplai.com', 'x-forwarded-proto': 'https' },
      body: { email: 'keepin@example.com', password: 'password1', remember: true },
    }, apexRemember);
    assert.strictEqual(apexRemember.statusCode, 200);
    assert.ok(cookieFrom(apexRemember).indexOf('Domain=.wannaplai.com') !== -1, 'apex keep-me-signed-in shares the cookie with www');
    assert.ok(cookieFrom(apexRemember).indexOf('Max-Age=2592000') !== -1);
    assert.ok(cookieFrom(apexRemember).indexOf('SameSite=Lax') !== -1);
    assert.ok(cookieFrom(apexRemember).indexOf('Secure') !== -1);

    const wwwRemember = mockRes();
    await login({
      method: 'POST',
      headers: { host: 'www.wannaplai.com', 'x-forwarded-proto': 'https' },
      body: { email: 'keepin@example.com', password: 'password1', remember: true },
    }, wwwRemember);
    assert.ok(cookieFrom(wwwRemember).indexOf('Domain=.wannaplai.com') !== -1, 'www keep-me-signed-in is also valid on apex');

    const previewLogin = mockRes();
    await login({
      method: 'POST',
      headers: { host: 'plaiground-preview.vercel.app', 'x-forwarded-proto': 'https' },
      body: { email: 'keepin@example.com', password: 'password1', remember: true },
    }, previewLogin);
    assert.ok(cookieFrom(previewLogin).indexOf('Domain=') === -1, 'preview hosts stay host-only');

    const apexShort = mockRes();
    await login({
      method: 'POST',
      headers: { host: 'www.wannaplai.com', 'x-forwarded-proto': 'https' },
      body: { email: 'keepin@example.com', password: 'password1' },
    }, apexShort);
    assert.ok(cookieFrom(apexShort).indexOf('Domain=.wannaplai.com') !== -1, 'short sessions also share apex/www');
    assert.ok(cookieFrom(apexShort).indexOf('Max-Age=1800') !== -1);
    assert.ok(cookieFrom(apexShort).indexOf('Max-Age=2592000') === -1);
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', name: 'Fuvtu', legal_first: 'Ada', legal_last: 'Night' },
    }, made);
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    await meApi({
      ...
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', artist_action: 'create', name: 'Second Act', legal_first: 'Ada', legal_last: 'Night' },
    }, fromAccount);
    ...
    await meApi({
      ...
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', artist_action: 'create', name: 'Fresh Act', legal_first: 'Ada', legal_last: 'Night' },
    }, fromEmpty);
    ...
    const missingLegal = mockRes();
    await meApi({
      url: '/api/me?resource=artists',
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: { action: 'create', artist_action: 'create', name: 'No Legal Act' },
    }, missingLegal);
    assert.strictEqual(missingLegal.statusCode, 400, 'Create artist without legal first+last must not save');
    assert.ok(/Legal first and last name are both required/.test(json(missingLegal).error || ''));
    assert.ok(!/stage name|rapper|\bband\b/i.test(json(missingLegal).error || ''), 'API legal error must not lecture about stage names');
    const twoWordLast = mockRes();
    await meApi({
      url: '/api/me?resource=artists',
      method: 'POST',
      headers: { cookie: sessionCookie },
    ...
    assert.strictEqual(twoWordLast.statusCode, 200, 'two-word legal last name must create');
    assert.strictEqual(json(twoWordLast).created.name, 'The Interceptors');
    assert.strictEqual(json(twoWordLast).created.legal_last, 'German Nunez');
    assert.strictEqual(json(twoWordLast).check.level, artistCheck.checkArtistName('The Interceptors').level);
  ...
  assert.ok(signupHtml.includes('<label for="username">Username</label>'));
  assert.ok(signupHtml.includes('placeholder="Optional community handle"'));
  assert.ok(signupHtml.includes('Optional. Unique if you set one. Sign-in stays email.'));
  assert.ok(signupHtml.includes('body.username = username'));
  assert.ok(signupHtml.includes('USERNAME_TAKEN'));
  ...
  assert.ok(!signupHtml.includes('id="first-name"') && !signupHtml.includes('id="last-name"'), 'signup does not collect first/last');
  assert.ok(!signupHtml.includes('First and last name are required.'), 'signup does not gate Join on a name');
  assert.ok(!signupHtml.includes('legal_first:') && !signupHtml.includes('first_name:'), 'signup does not send a second name store');
  ...
  const uploadAudio = fs.readFileSync(path.join(__dirname, '..', 'lib', 'upload-audio-bind.js'), 'utf8');
  assert.ok(uploadHtml.includes('data-audio-player'));
  assert.ok(uploadAudio.includes('createObjectURL'));
  assert.ok(uploadAudio.includes('revokeObjectURL'));
  ...
  assert.ok(loginHtml.includes('Continue with Google'), 'login has one Continue with Google control');
  assert.ok(/id="google-auth"[^>]*\bhidden\b/.test(loginHtml), 'Google stays hidden until env is configured');
  assert.ok(loginHtml.includes('/api/auth/google'), 'login asks the server whether Google is configured');
  assert.ok(loginHtml.includes('/api/auth/google-start'), 'login Google control starts the real OAuth path');
  assert.ok(loginHtml.includes('Google sign-in did not finish'), 'login explains a failed Google return');
  assert.ok(!/>\s*Apple\s*</.test(loginHtml), 'login has no Apple button');
  assert.ok(!/class="or"/.test(loginHtml), 'login does not revive the old social OR divider');
  assert.ok(loginHtml.includes('site.css?v=20260921g1'), 'login.html cache-busts site.css for the Google control');
  ...
  assert.ok(loginHtml.includes('recordSignedIn({ remember: remember })'), 'checked keep-me-signed-in persists on the client');
  assert.ok(loginHtml.includes('membership.js?v=20260912k1'), 'login.html cache-busts membership.js');
  const dashboardHtml = fs.readFileSync(path.join(__dirname, '..', 'dashboard.html'), 'utf8');
  assert.ok(dashboardHtml.includes('membership.js?v=20260912k1'), 'dashboard.html cache-busts membership.js');
  assert.ok(loginHtml.includes('<label for="email">Email</label>'), 'sign-in stays email');
  assert.ok(!loginHtml.includes('<label for="username">Username</label>'), 'do not ship username login');
  assert.ok(loginHtml.includes('{ email: email, password: password, remember: remember }'), 'login posts email, not username');
  const sessionSrc = fs.readFileSync(path.join(__dirname, 'auth.js'), 'utf8');
  assert.ok(sessionSrc.includes("COOKIE_SHARE_DOMAIN = '.wannaplai.com'"), 'session cookies cover apex and www');
  assert.ok(sessionSrc.includes('cookieDomain'), 'live cookie domain follows the request host');
  const authSrc = fs.readFileSync(path.join(__dirname, '..', 'api', 'auth.js'), 'utf8');
  assert.ok(authSrc.includes('Sign-in stays email'), 'login handler must not look up username');
  assert.ok(!/findByUsername/.test(authSrc), 'login must not look up accounts by username');
  assert.ok(!signupHtml.includes('class="social-row"'), 'signup has no Google/Apple social row');
  assert.ok(signupHtml.includes('Continue with Google'), 'signup has one Continue with Google control');
  assert.ok(/id="google-auth"[^>]*\bhidden\b/.test(signupHtml), 'signup Google stays hidden until env is configured');
  assert.ok(signupHtml.includes('/api/auth/google'), 'signup asks the server whether Google is configured');
  assert.ok(signupHtml.includes('/api/auth/google-start'), 'signup Google control starts the real OAuth path');
  assert.ok(!/>\s*Apple\s*</.test(signupHtml), 'signup has no Apple button');
  assert.ok(signupHtml.includes('site.css?v=20260921g1'), 'signup.html cache-busts site.css for the Google control');
  ...
  assert.ok(!settingsHtml.includes('<label>Artist name</label>'), 'Settings no longer collects artist name');
  assert.ok(settingsHtml.includes('<label>Username</label>'));
  assert.ok(settingsHtml.includes('data-account-username'));
  assert.ok(settingsHtml.includes('<label>Legal name</label>'));
  assert.ok(settingsHtml.includes('data-account-email'));
  assert.ok(settingsHtml.includes('Account, login, and billing. Artist names live on Artist Profiles.'));
  assert.ok(accountJs.includes('data-account-username'));
  assert.ok(accountJs.includes('nextProfile.username'));

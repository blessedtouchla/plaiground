...
async function run() {
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    try {
      const created = mockRes();
      await signup({ method: 'POST', headers: {}, body: { email: 'map@example.com', password: 'password1', artist: 'Map Act' } }, created);
      assert.strictEqual(created.statusCode, 200);
      await confirm({ method: 'POST', headers: {}, body: { token: mail.signToken('map@example.com') } }, mockRes());
      const loginRes = mockRes();
      await login({ method: 'POST', headers: {}, body: { email: 'map@example.com', password: 'password1' } }, loginRes);

  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    await meArtists({
      ...
      body: {
        ...
        bio: 'should not silently save',
      },
    }, silent);
    assert.strictEqual(silent.statusCode, 409);
    assert.strictEqual(json(silent).code, 'ARTIST_SUBMIT_EDIT');
    ...
    assert.strictEqual(json(aiSaved).submitted_edit, true);
    assert.strictEqual(json(aiSaved).updated.edit_status, 'pending');
    ...
    assert.strictEqual(json(namePatch).updated.edit_status, 'pending');

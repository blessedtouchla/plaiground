...
async function run() {
  ...
  await withEnv({ DATABASE_URL: 'postgres://memory', SESSION_SECRET: 'unit-test-session-secret', CONFIRM_SECRET: 'unit-confirm-secret', memory: true }, async () => {
    ...
    assert.strictEqual(json(linked).created.platform_links[0].id, '0TnOYISbd1XYRBk9myaseg');

    const twoPlatforms = mockRes();
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: {
    ...
    assert.strictEqual(twoPlatforms.statusCode, 200);
    assert.strictEqual(json(twoPlatforms).updated.platform_links.length, 2);
    assert.strictEqual(json(twoPlatforms).updated.spotify_id, '0TnOYISbd1XYRBk9myaseg');
    assert.strictEqual(json(twoPlatforms).updated.apple_id, '123456789');
    ...
    await meArtists({
      ...
      body: {
        ...
        id: json(createdArtist).created.id,
        platform_links: [
          { platform: 'spotify', url: 'https://music.apple.com/us/artist/demo/123456789' },
        ],
      },
    }, mismatch);
    assert.strictEqual(mismatch.statusCode, 400);
    assert.match(json(mismatch).error || '', /does not match Spotify/);
    ...
    assert.strictEqual(json(duplicate).error, 'That platform is already on the list.');

    const legacyIds = mockRes();
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: {
    ...
    assert.strictEqual(legacyIds.statusCode, 200);
    assert.strictEqual(json(legacyIds).updated.spotify_id, '4dpARuHxo51G3z768sgnrY');
    assert.strictEqual(json(legacyIds).updated.apple_id, '987654321');
    assert.strictEqual(json(legacyIds).updated.platform_links.length, 2);
    assert.strictEqual(json(legacyIds).updated.platform_links[0].platform, 'spotify');
    assert.strictEqual(json(legacyIds).updated.platform_links[1].platform, 'apple');
    ...
    assert.strictEqual(json(namePatch).updated.spotify_id, '4dpARuHxo51G3z768sgnrY');

    const lockedPlatforms = mockRes();
    await meArtists({
      method: 'POST',
      headers: { cookie: sessionCookie },
      body: {
        ...
        id: json(createdArtist).created.id,
        platform_links: [
          { platform: 'youtube-music', url: 'https://music.youtube.com/channel/UCafterlive' },
        ],
      },
    }, lockedPlatforms);
    assert.strictEqual(lockedPlatforms.statusCode, 200);
    assert.strictEqual(json(lockedPlatforms).updated.spotify_id, '4dpARuHxo51G3z768sgnrY', 'platform IDs stay locked after live');
    assert.strictEqual(json(lockedPlatforms).updated.apple_id, '987654321');
    assert.ok(!json(lockedPlatforms).updated.platform_links.some(function (row) {
      return row.platform === 'youtube-music';
    }), 'locked artist cannot add or replace platform rows');

...
(function (root, factory) {
  ...
  function paintTile(tile, url) {
    ...
    if (!tile) return;
    if (tile.style) {
      tile.style.background = art ? ('center / cover no-repeat url("' + safe + '")') : '';
    ...
    if (tile.querySelector) {
      img = tile.querySelector('img[data-cover-photo]');
    } else if (tile.children && tile.children.length) {
      var kids = tile.children;
      var i;
      for (i = 0; i < kids.length; i += 1) {
        if (kids[i] && kids[i].getAttribute && kids[i].getAttribute('data-cover-photo') != null) {
          img = kids[i];
          break;
        }
      }
  ...
  function paintReviewCover() {
    ...
    var draft = {};
    try {
      var raw = (win.sessionStorage && win.sessionStorage.getItem('plaiground.store.draft'))
        || (win.localStorage && win.localStorage.getItem('plaiground.store.draft'))
        || '{}';
    ...
    if (stored) paintTile(tile, stored);
    ...
    if (files && typeof files.keepHeldFiles === 'function') {
      files.keepHeldFiles(win).then(function (held) {
        ...
        if (!file || !isCoverFile(file)) return;
        try {
          paintTile(tile, URL.createObjectURL(file));
        } catch (err2) {}
      });
    }
  }

...
function validateAttestExtras(fields) {
  return validateWriterLines(fields);
}
...
function rememberLegal(first, last, win) {
  win = win || (typeof globalThis !== 'undefined' ? globalThis : null);
  var payload = JSON.stringify({ first: trim(first), last: trim(last) });
  try {
    if (win && win.localStorage) win.localStorage.setItem(LEGAL_KEY, payload);
  } catch (err) {}
  try {
    if (win && win.sessionStorage) win.sessionStorage.setItem(LEGAL_KEY, payload);
  } catch (err2) {}
}
...
function rememberedLegal(win) {
  ...
  var raw = '';
  try {
    if (win && win.localStorage) raw = win.localStorage.getItem(LEGAL_KEY) || '';
  } catch (err) {}
  if (!raw) {
    try {
      if (win && win.sessionStorage) raw = win.sessionStorage.getItem(LEGAL_KEY) || '';
    } catch (err2) {}
  }
  if (!raw) return { first: '', last: '' };
  try {
    var parsed = JSON.parse(raw) || {};
    return { first: trim(parsed.first), last: trim(parsed.last) };
  } catch (err3) {
    return { first: '', last: '' };
  }
}

function rosterFromWin(win) {
  var me = win && win.PlaigroundMembership && typeof win.PlaigroundMembership.account === 'function'
    ? win.PlaigroundMembership.account()
    : null;
  return me && me.profile && Array.isArray(me.profile.artists) ? me.profile.artists : [];
}

function findRosterArtist(id, win) {
  var want = trim(id);
  if (!want) return null;
  var found = null;
  rosterFromWin(win).forEach(function (row) {
    if (!row) return;
    if (trim(row.id) === want || trim(row.artist_id) === want || trim(row.plaiground_artist_id) === want) {
      found = row;
    }
  });
  return found;
...
function writeLegalToArtist(id, first, last, win) {
  win = win || (typeof globalThis !== 'undefined' ? globalThis : null);
  var nextFirst = trim(first);
  var nextLast = trim(last);
  var artist = findRosterArtist(id, win);
  if (!nextFirst || !nextLast) return Promise.resolve(artist || null);
  if (artist && trim(artist.legal_first) && trim(artist.legal_last)) {
    return Promise.resolve(artist);
  }
  if (artist) {
    artist.legal_first = nextFirst;
    artist.legal_last = nextLast;
  }
  if (!win || typeof win.fetch !== 'function' || !trim(id)) return Promise.resolve(artist || null);
  return win.fetch('/api/me/artists', {
    ...
    body: JSON.stringify({
      ...
      id: trim(id),
      artist_id: trim(id),
      legal_first: nextFirst,
      legal_last: nextLast,
    }),
  }).then(function (res) {
    return res && typeof res.json === 'function' ? res.json() : artist || null;
  }).catch(function () {
    return artist || null;
  });
}
...
function statusApi() {
  if (typeof PlaigroundReleaseStatus !== 'undefined' && PlaigroundReleaseStatus) return PlaigroundReleaseStatus;
  if (typeof globalThis !== 'undefined' && globalThis.PlaigroundReleaseStatus) return globalThis.PlaigroundReleaseStatus;
  return null;
}

function isReturnedRelease(row) {
  var api = statusApi();
  return !!(api && typeof api.isReturnedRelease === 'function' && api.isReturnedRelease(row));
}

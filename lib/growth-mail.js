...
const DASHBOARD_URL = SITE_ORIGIN + '/dashboard.html';
const FAQ_URL = SITE_ORIGIN + '/faq.html';
const SIQA_URL = SITE_ORIGIN + '/charts';
...
const COPY = {
  ...
  first_upload: {
    subject: 'Woo-hoo! Your first song has been submitted!',
    body: 'Woo-hoo! Your first song has been submitted! We are now going through QC and will email you as soon as it’s Live!',
    linksLead: 'While you wait:',
    links: [
      { label: 'Dashboard', href: DASHBOARD_URL },
      { label: 'FAQ', href: FAQ_URL },
      { label: 'SIQA Charts', href: SIQA_URL },
    ],
    signoff: 'Sincerely,\nPlaiground Team',
...
function buildFirstUpload() {
  ...
  const linkLines = (copy.links || []).map(function (item) {
    return item.label + ' → ' + item.href;
  });
  const text = [copy.body, '', copy.linksLead].concat(linkLines).concat(['', copy.signoff, '', copy.ps]).join('\n');
  const htmlLinks = (copy.links || []).map(function (item) {
    return '<a href="' + escapeHtml(item.href) + '">' + escapeHtml(item.label) + '</a>';
  }).join(' · ');
  const html = [
    '<p>' + escapeHtml(copy.body) + '</p>',
    '<p>' + escapeHtml(copy.linksLead) + '<br>' + htmlLinks + '</p>',

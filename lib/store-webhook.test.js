...
async function run() {
  ...
  assert.strictEqual(storeWebhook.isLiveStoreHost('https://api.tonegrid.pro'), true);
  assert.strictEqual(storeWebhook.isLiveStoreHost('https://api.tonegrid.pro/api'), true);
  assert.strictEqual(storeWebhook.isLiveStoreHost('https://api-sandbox.tonegrid.pro/api'), false);

  storeWebhook.resetSubscriptionCache();
  const hops = [];
  const already = await storeWebhook.ensureSubscription({
    url: storeWebhook.PUBLIC_URL,
    fetch: async (path, opts) => {
      hops.push({ path, method: opts && opts.method, body: opts && opts.body });
      return {
        ok: true,
        status: 200,
        data: { data: [{ url: storeWebhook.PUBLIC_URL, events: ['release.dsp.*.*'], is_active: true }] },
      };
    },
  });
  assert.strictEqual(already.subscribed, true);
  assert.strictEqual(already.created, false);
  assert.strictEqual(hops.filter((row) => row.method === 'POST').length, 0);

  storeWebhook.resetSubscriptionCache();
  const createdHops = [];
  const created = await storeWebhook.ensureSubscription({
    url: storeWebhook.PUBLIC_URL,
    idempotencyKey: 'test-webhook-create',
    fetch: async (path, opts) => {
      createdHops.push({ path, method: opts && opts.method, body: opts && opts.body });
      if ((opts && opts.method) === 'GET') return { ok: true, status: 200, data: { data: [] } };
      return {
        ok: true,
        status: 201,
        data: {
          data: {
            url: storeWebhook.PUBLIC_URL,
            secret: 'tgwhsec_unit-test-do-not-print',
            events: storeWebhook.SUBSCRIBE_EVENTS,
          },
        },
      };
    },
  ...
  const post = createdHops.find((row) => row.method === 'POST');
  assert.ok(post);
  assert.strictEqual(post.path, '/webhooks');
  assert.strictEqual(post.body.url, storeWebhook.PUBLIC_URL);
  assert.ok(post.body.events.indexOf('release.dsp.*.*') !== -1);
  assert.ok(post.body.events.indexOf('ingestion.*') !== -1);
  assert.ok(!JSON.stringify(created).includes('tgwhsec_'));

  const cached = await storeWebhook.ensureSubscription({
    url: storeWebhook.PUBLIC_URL,
    fetch: async () => {
      throw new Error('cache must skip a second hop');
    },
  });
  assert.strictEqual(cached.cached, true);

  storeWebhook.resetSubscriptionCache();
  const failedList = await storeWebhook.ensureSubscription({
    fetch: async () => ({ ok: false, status: 502, data: { error: 'The store rejected the request.' } }),
  });
  assert.strictEqual(failedList.subscribed, false);
  assert.strictEqual(failedList.created, false);

  const prevKey = process.env.TONEGRID_API_KEY;
  const prevBase = process.env.TONEGRID_BASE_URL;
  process.env.TONEGRID_API_KEY = 'test-key-value-not-for-commit';
  process.env.TONEGRID_BASE_URL = 'https://api.tonegrid.pro';
  storeWebhook.resetSubscriptionCache();
  const originalFetch = global.fetch;
  const healthCalls = [];
  global.fetch = async function mockFetch(url, opts) {
    healthCalls.push({ url: String(url), method: opts && opts.method, body: opts && opts.body });
    if (String(url).includes('/webhooks') && String((opts && opts.method) || 'GET').toUpperCase() === 'GET') {
    ...
    if (String(url).includes('/webhooks')) {
      return {
        ...
        status: 201,
        json: async () => ({
          data: { url: storeWebhook.PUBLIC_URL, secret: 'tgwhsec_health-must-not-echo' },
        }),
      };
    }
  ...
  try {
    const res = mockRes();
    await tonegridApi({ method: 'GET', url: '/api/tonegrid/health', headers: {} }, res);
    assert.strictEqual(res.statusCode, 200);
    assert.deepStrictEqual(json(res), { configured: true, sandbox: false });
    assert.ok(healthCalls.some((row) => row.url.includes('/webhooks') && String(row.method || 'GET').toUpperCase() === 'GET'));
    assert.ok(healthCalls.some((row) => row.url.includes('/webhooks') && String(row.method).toUpperCase() === 'POST'));
    assert.ok(!res.body.includes('tgwhsec_'));

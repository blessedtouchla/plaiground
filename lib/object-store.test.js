...
async function run() {
  ...
  await withEnv(READY, async () => {
    ...
    assert.strictEqual(store.PUT_EXPIRES, 1800);
    const now = new Date('2026-08-26T03:00:00.000Z');
    const defaultPut = store.presignPut(audioKey, 'audio/wav', { now: now });
    assert.strictEqual(defaultPut.expires_in, 1800, 'slow phones get a 30 min PUT window');
    assert.ok(defaultPut.expires_in >= 900 && defaultPut.expires_in <= 3600);
    ...
    global.fetch = function hangGet() {
      return new Promise(function () {});
    };
    try {
      const started = Date.now();
      await assert.rejects(
        () => store.getObject(audioKey, { timeoutMs: 30 }),
        (err) => err && err.message === store.STEP_FAIL_COPY && !/Cloudflare|R2|ToneGrid/i.test(err.message)
      );
      assert.ok(Date.now() - started < 1000, 'hung object pull must time out');
    } finally {
  ...
  assert.strictEqual(store.GET_TIMEOUT_MS, 20000);

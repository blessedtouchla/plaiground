...
async function run() {
      ...
      return {
        ...
        json: async () => ({
          release: {
            ...
            title: 'Dolly',
            status: 'rejected',
            tracks: [{ uuid: trackId, title: 'Dolly' }],
          },
        }),
      };
    }
  ...
  try {
    await withAccountUser(async (_row, headers) => {
      ...
      assert.ok(
        !emptyContribs.some((row) => row.role === 'Performer' || row.role === 'Producer'),
        'empty collected credits must not invent a performer or producer'
      );
    });
  } finally {

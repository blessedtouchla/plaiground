...
async function run() {
  ...
  }), 'Save draft never mints or hops the store');
  assert.strictEqual(saved.plaiground_artist_id, 'act-2', 'Save draft keeps the local profile id on plaiground_artist_id');
  assert.notStrictEqual(saved.artist_id, 'act-2', 'Save draft must not write the local profile id as a store artist_id');

...
function loadPage(opts) {
  ...
  const wrapFirst = opts.omitLegalFields ? null : el({ id: 'tg-legal-first', value: opts.wrapFirst || '' });
  const wrapLast = opts.omitLegalFields ? null : el({ id: 'tg-legal-last', value: opts.wrapLast || '' });
  const createFirst = opts.omitLegalFields ? null : el({ id: 'tg-legal-first-create', value: opts.createFirst || '' });
  const createLast = opts.omitLegalFields ? null : el({ id: 'tg-legal-last-create', value: opts.createLast || '' });
...
async function run() {
  ...
  const savedResult = await savePage.api.saveDraft('');
  assert.strictEqual(savedResult, false, 'Save draft is cancelled');
  const saved = JSON.parse(savePage.local.getItem('plaiground.store.draft') || '{}');
  assert.notStrictEqual(saved.saved_draft, true, 'saveDraft does not write a leftover saved draft');
  assert.ok(!savePage.fetchCalls.some(function (call) {
    return /\/api\/tonegrid\//.test(String(call.url || ''));
  }), 'cancelled Save draft never mints or hops the store');
  ...
  assert.notStrictEqual(reopen.title.value, 'The Interceptors', 'leftover saved draft does not resume on Upload');
  const afterDrop = JSON.parse(reopen.local.getItem('plaiground.store.draft') || '{}');
  assert.notStrictEqual(afterDrop.saved_draft, true, 'leftover saved draft flag is dropped');
  assert.notStrictEqual(afterDrop.title, 'The Interceptors', 'leftover title does not resume');
  assert.strictEqual(afterDrop.legal_first, 'German', 'picked artist legal first still persists after leftover drop');
  assert.strictEqual(afterDrop.legal_last, 'Nunez', 'picked artist legal last still persists after leftover drop');
  ...
  assert.strictEqual(createPage.label.value, 'PLAIGROUND', 'Record label starts as PLAIGROUND');
  ...
  assert.strictEqual(pickPage.label.value, 'PLAIGROUND', 'existing-artist label defaults to PLAIGROUND');
  ...
  assert.strictEqual(lockedDraft.label.value, 'PLAIGROUND', 'PLAIGROUND stays visible as the default label');
  ...
  const noFields = loadPage({
    mode: 'choose',
    artistId: 'act-2',
    omitLegalFields: true,
    artists: [{ id: 'act-2', name: 'Night Drive', legal_first: 'Ada', legal_last: 'Night' }],
  });
  const persisted = JSON.parse(noFields.local.getItem('plaiground.store.draft') || '{}');
  assert.strictEqual(persisted.legal_first, 'Ada', 'selected artist legal first persists without a second upload pair');
  assert.strictEqual(persisted.legal_last, 'Night', 'selected artist legal last persists without a second upload pair');
  const collected = noFields.api.collectPageFields();
  assert.strictEqual(collected.legal_first, 'Ada');
  assert.strictEqual(collected.legal_last, 'Night');

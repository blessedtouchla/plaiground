...
function loadPage(opts) {
  ...
  const label = el({ id: 'tg-label', value: 'PLAIGROUND' });
  const copyright = el({ id: 'tg-copyright-line', value: '' });
...
async function run() {
  ...
  assert.strictEqual(createPage.label.value, 'PLAIGROUND', 'Record label is locked to PLAIGROUND');
  assert.ok(!/Victoria/i.test(createPage.copyright.value), 'copyright line must not use the account name on create');
  ...
  assert.ok(createPage.copyright.value.indexOf('Ada Night') !== -1, 'copyright line uses the typed songwriter');
  createPage.label.value = 'Other';
  createPage.label.dispatch('input');
  assert.strictEqual(createPage.label.value, 'PLAIGROUND', 'Record label snaps back to PLAIGROUND');
  ...
  assert.ok(!/Victoria/i.test(createPage.copyright.value) || createPage.createLast.value === 'Night', 'empty first does not refill Victoria');

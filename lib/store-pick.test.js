...
function run() {
  ...
  assert.deepStrictEqual(storePick.selected(root), ['spotify', 'apple-music', 'tidal']);
  ...
  assert.deepStrictEqual(storePick.selected(root), ['spotify', 'tidal']);
  ...
  assert.strictEqual(storePick.formatReviewSummary(0, 0, true, 'included'), 'All stores · included');
  assert.strictEqual(storePick.formatReviewSummary(87, 87, true, 'included'), 'All 87 stores · included');
  assert.strictEqual(storePick.formatReviewSummary(40, 55, false, 'included'), '40 of 55 stores');
  ...
  const includedRoot = el({ attrs: { 'data-store-copy': 'included' } });
  const includedAll = el({ type: 'checkbox', checked: true, attrs: { 'data-store-all': '' } });
  const includedCustomize = el({ attrs: { 'data-store-customize': '' } });
  const includedSummary = el({ attrs: { 'data-store-summary': '', 'data-store-copy': 'included' } });
  const includedList = el({ attrs: { 'data-store-list': '', 'data-edit-stores': '' } });
  includedList.hidden = true;
  includedRoot.appendChild(includedAll);
  includedRoot.appendChild(includedCustomize);
  includedRoot.appendChild(includedSummary);
  includedRoot.appendChild(includedList);
  storePick.bind(includedRoot, {
    document: doc,
    stores: [
      { slug: 'spotify', name: 'Spotify' },
      { slug: 'apple-music', name: 'Apple Music' },
  ...
  assert.strictEqual(includedSummary.textContent, 'All 2 stores · included');

  const defaultRoot = el({ attrs: { 'data-store-copy': 'included' } });
  const defaultAll = el({ type: 'checkbox', checked: true, attrs: { 'data-store-all': '' } });
  const defaultCustomize = el({ attrs: { 'data-store-customize': '' } });
  const defaultSummary = el({ attrs: { 'data-store-summary': '', 'data-store-copy': 'included' } });
  const defaultList = el({ attrs: { 'data-store-list': '', 'data-edit-stores': '' } });
  defaultList.hidden = true;
  defaultRoot.appendChild(defaultAll);
  defaultRoot.appendChild(defaultCustomize);
  defaultRoot.appendChild(defaultSummary);
  defaultRoot.appendChild(defaultList);
  storePick.bind(defaultRoot, {
    document: doc,
    stores: storePick.DEFAULT_STORES,
    selected: null,
  });
  assert.strictEqual(defaultSummary.textContent, 'All stores · included');
  assert.ok(!/\b17\b/.test(defaultSummary.textContent), 'fallback catalog stays soft, not a hardcoded count');
  ...
  assert.strictEqual(faq.sentence, 'We deliver to 50+ platforms.');
  ...
  assert.strictEqual(faqCount.textContent, 'We deliver to 50+ platforms.');
  ...
  assert.strictEqual(groupCount.textContent, 'We deliver to 50+ platforms.');
  ...
  assert.ok(!storePick.DEFAULT_STORES.some(function (row) { return row.slug === 'youtube-music'; }));
  assert.deepStrictEqual(
    storePick.selectableStores([
      { slug: 'spotify', name: 'Spotify' },
      { slug: 'youtube-music', name: 'YouTube Music' },
      { slug: 'tidal', name: 'Tidal', status: 'inactive' },
      { slug: 'deezer', name: 'Deezer' },
    ]).map(function (row) { return row.slug; }),
    ['spotify', 'deezer']
  );
  const blockedRoot = el();
  const blockedAll = el({ type: 'checkbox', checked: true, attrs: { 'data-store-all': '' } });
  const blockedCustomize = el({ attrs: { 'data-store-customize': '' } });
  const blockedSummary = el({ attrs: { 'data-store-summary': '' } });
  const blockedList = el({ attrs: { 'data-store-list': '', 'data-edit-stores': '' } });
  blockedRoot.appendChild(blockedAll);
  blockedRoot.appendChild(blockedCustomize);
  blockedRoot.appendChild(blockedSummary);
  blockedRoot.appendChild(blockedList);
  storePick.bind(blockedRoot, {
    document: doc,
    stores: [
      { slug: 'spotify', name: 'Spotify' },
      { slug: 'youtube-music', name: 'YouTube Music' },
  ...
  assert.deepStrictEqual(storePick.selected(blockedRoot), ['spotify', 'apple-music']);
  assert.strictEqual(blockedList.querySelectorAll('input[type="checkbox"]').length, 2);
  assert.ok(!blockedList.querySelectorAll('input[type="checkbox"]').some(function (box) {
    return box.value === 'youtube-music';
  }));

  const leftoverRoot = el();
  const leftoverList = el({ attrs: { 'data-store-list': '', 'data-edit-stores': '' } });
  const leftoverYt = el({ type: 'checkbox', checked: true });
  leftoverYt.value = 'youtube-music';
  const leftoverSpotify = el({ type: 'checkbox', checked: true });
  leftoverSpotify.value = 'spotify';
  leftoverList.appendChild(leftoverYt);
  leftoverList.appendChild(leftoverSpotify);
  leftoverRoot.appendChild(leftoverList);
  assert.deepStrictEqual(storePick.selected(leftoverRoot), ['spotify']);
  assert.deepStrictEqual(storePick.selectedBlocked(leftoverRoot).map(function (row) {
    return row.slug;
  }), ['youtube-music']);
  assert.strictEqual(
    storePick.inactiveStoreError(storePick.selectedBlocked(leftoverRoot)),
    'YouTube Music is not available. Choose other stores and submit again.'
  );
  assert.ok(!/tonegrid|distributor|distrokid|interspace/i.test(storePick.INACTIVE_STORE_COPY));
  assert.strictEqual(
    storePick.inactiveStoreError([{ slug: 'tidal', name: 'Tidal', status: 'inactive' }]),
    'Tidal is not available. Choose other stores and submit again.'
  );

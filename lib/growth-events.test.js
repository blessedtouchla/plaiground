...
async function run() {
  ...
  assert.strictEqual(uploadMail.subject, 'We have it.');
  assert.ok(uploadMail.text.indexOf('It’s in. Stores take a few days. We’ll mail you when the first one is live.') !== -1);
  ...
  assert.ok(!/emailplaiground@gmail\.com/.test(growthMailSrc));
  ...
  assert.ok(accountsSrc.includes('recordFirstUpload'));

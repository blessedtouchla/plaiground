...
function run() {
  ...
  assert.ok(rules.audioAttachAccepted({ audio_status: 'processing' }), 'ToneGrid processing is an accepted attach');
  assert.ok(rules.audioAttachAccepted({ audio_url: 'https://cdn.example/a.wav' }));
  assert.ok(!rules.audioAttachAccepted({ audio_url: null, audio_s3_key: null, file_size: null }), 'explicit null master is not attached');
  assert.ok(!rules.audioAttachAccepted({}), 'empty 200 is not a master');
  assert.ok(!rules.audioAttachAccepted({ uuid: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', title: 'Lost 2 the music' }), 'track JSON without audio is not attached');
  assert.ok(!rules.audioAttachAccepted({ error: 'audio file is required.' }));
  assert.ok(rules.trackHasStoreAudio({ uuid: 'cccccccc-cccc-4ccc-8ccc-cccccccccccc', audio_status: 'processing' }));

...
function run() {
  ...
  assert.strictEqual(input._plaigroundFile, first, 'local File must stick on the input');
  ...
  win.listeners.pageshow();
  assert.ok(api.hasLocal(), 'pageshow restores the stuck File preview');
  assert.ok(/blob:cover-/.test(tile.style.backgroundImage), 'pageshow paints a fresh object URL');
  ...
  pages.forEach(function (page) {
    ...
    assert.ok(page.html.includes('lib/cover-url.js'), page.name + ' must load cover URL mapping');
  });

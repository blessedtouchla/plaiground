...
(function (root) {
  ...
  function paintSelect() {
    ...
    load().forEach(function (row) {
      ...
      var at = -1;
      var j;
      for (j = 0; j < sel.options.length; j += 1) {
        var modeVal = String(sel.options[j].value || '');
        if (modeVal === '__create__' || modeVal === '__link__') {
          at = j;
          break;
        }
      }
      if (at === -1) sel.appendChild(opt);
      else sel.insertBefore(opt, sel.options[at]);
      have[key] = true;
    });
  }

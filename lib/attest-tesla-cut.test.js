...
const STAMP = '20260917at2';
const JS_STAMP = '20260917at1';
const REVIEW_STAMP = '20260917r2';
const SUBMITTED_STAMP = '20260917s4';
...
function run() {
  ...
  assert.ok(html.includes('class="attest-help"'), 'contribution lecture sits under ?');
  assert.ok(html.includes('Pick everything a person did. A couple of sentences is plenty.'), 'one helper stays under ?');
  ...
  assert.ok(/\.attest-page \.attest-help p[\s\S]*?position:\s*static/.test(cut), 'contribution helper stays in flow on phone');

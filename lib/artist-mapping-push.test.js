...
async function run() {
  ...
  assert.ok(meSrc.includes('backfillLinkedMappings'));
  assert.ok(meSrc.includes("action === 'update'"));
  assert.ok(mappingPush.isStoreArtistId('377'));
  assert.ok(mappingPush.isStoreArtistId('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa'));
  assert.ok(!mappingPush.isStoreArtistId(''));
  assert.ok(!mappingPush.isStoreArtistId('pg-local'));
  assert.ok(mappingPush.artistNeedsBackfill({
    tonegrid_artist_id: '377',
    platform_links: [{ platform: 'spotify', url: 'https://open.spotify.com/artist/0TnOYISbd1XYRBk9myaseg' }],
  }));
  assert.ok(!mappingPush.artistNeedsBackfill({
    tonegrid_artist_id: '377',
    platform_links: [],
  ...
  accounts.useMemoryStore();
  const hermanId = '377';
  const hermanUrl = 'https://open.spotify.com/artist/0TnOYISbd1XYRBk9myaseg';
  const loadCalls = [];
  const loadFetch = global.fetch;
  const loadErrors = [];
  const loadError = console.error;
  console.error = function () { loadErrors.push(Array.prototype.slice.call(arguments)); };
  global.fetch = async function mockLoad(url, options) {
    const method = String((options && options.method) || 'GET').toUpperCase();
    loadCalls.push({ url: String(url), method: method, body: options && options.body });
    if (method === 'GET' && String(url).indexOf('/artists/' + hermanId) !== -1) {
      return {
        ok: true,
        status: 200,
        json: async () => ({
    ...
    if (method === 'PATCH' && String(url).indexOf('/artists/' + hermanId) !== -1) {
      const sent = JSON.parse((options && options.body) || '{}');
      return { ok: true, status: 200, json: async () => Object.assign({ id: 377 }, sent) };
    }
    throw new Error('unexpected store call ' + method + ' ' + String(url));
  };
  try {
    ...
    await authApi(Object.assign({ url: '/api/auth/signup' }, {
      method: 'POST',
      headers: {},
      body: { email: 'herman-map@example.com', password: 'password1', artist: 'Herman Watson' },
    }), created);
    assert.strictEqual(created.statusCode, 200);
    await authApi(Object.assign({ url: '/api/auth/confirm' }, {
      method: 'POST',
      headers: {},
      body: { token: mail.signToken('herman-map@example.com') },
    }), mockRes());
    const loginRes = mockRes();
    await authApi(Object.assign({ url: '/api/auth/login' }, {
      method: 'POST',
      headers: {},
      body: { email: 'herman-map@example.com', password: 'password1' },
    }), loginRes);
    const cookie = cookieFrom(loginRes).split(';')[0];
    const minted = mockRes();
    await meApi(Object.assign({ url: '/api/me/artists' }, {
      method: 'POST',
      headers: { cookie: cookie },
      body: { action: 'create', name: 'Herman Watson', legal_first: 'Herman', legal_last: 'Watson' },
    }), minted);
    assert.strictEqual(minted.statusCode, 200, minted.body);
    const artistId = json(minted).created.id;
    const row = await accounts.findByEmail('herman-map@example.com');
    const stored = profile.recoverRoster(profile.readStored(row), row.artist_name, row.tonegrid_artist_id);
    await accounts.updateProfile(row.id, {
      profile: profile.upsertArtist(stored, Object.assign({}, stored.artists[0], {
        id: artistId,
        tonegrid_artist_id: hermanId,
        platform_links: [{ platform: 'spotify', url: hermanUrl }],
      })),
    });
    loadCalls.length = 0;
    const loaded = mockRes();
    await meApi(Object.assign({ url: '/api/me' }, {
      method: 'GET',
      headers: { cookie: cookie },
    }), loaded);
    assert.strictEqual(loaded.statusCode, 200, loaded.body);
    const patches = loadCalls.filter((call) => (
      call.method === 'PATCH' && String(call.url).indexOf('/artists/' + hermanId) !== -1
    ));
    assert.ok(patches.length >= 1, 'GET /api/me must backfill saved mapping URLs onto a linked store artist');
    const sent = JSON.parse(patches[0].body || '{}');
    assert.strictEqual(sent.spotify_url, hermanUrl);
    assert.strictEqual(sent.spotify_id, '0TnOYISbd1XYRBk9myaseg');
    assert.ok(!sent.apple_url, 'must not invent an Apple URL');

    global.fetch = async function failLoad(url, options) {
      const method = String((options && options.method) || 'GET').toUpperCase();
      loadCalls.push({ url: String(url), method: method });
      if (method === 'GET' && String(url).indexOf('/artists/' + hermanId) !== -1) {
        return {
          ok: true,
          status: 200,
          json: async () => ({ id: 377, spotify_url: null, spotify_id: null }),
        };
      }
      return { ok: false, status: 500, json: async () => ({ error: 'store mapping refused' }) };
    };
    const stillOk = mockRes();
    await meApi(Object.assign({ url: '/api/me' }, {
      method: 'GET',
      headers: { cookie: cookie },
    }), stillOk);
    assert.strictEqual(stillOk.statusCode, 200, 'profile load must not 502 when backfill PATCH fails');
    assert.ok(loadErrors.some((row) => String(row[0]).indexOf('ToneGrid artist mapping PATCH failed') !== -1));
  } finally {
    console.error = loadError;
    accounts.resetStore();
    if (loadFetch === undefined) delete global.fetch;
    else global.fetch = loadFetch;
    if (prev.db === undefined) delete process.env.DATABASE_URL;
    else process.env.DATABASE_URL = prev.db;
    if (prev.secret === undefined) delete process.env.SESSION_SECRET;
    else process.env.SESSION_SECRET = prev.secret;

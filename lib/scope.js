...
function trackIdsFrom(row) {
  const raw = row && Array.isArray(row.tonegrid_track_ids) ? row.tonegrid_track_ids : [];
  const seen = new Set();
  const out = [];
  raw.forEach((value) => {
    const id = String(value || '').trim();
...
async function personalScope(req, res) {
  ...
  const trackIds = trackIdsFrom(row);
  ...
  return {
    ...
    trackAllow: allowedSet(trackIds),
    empty: releaseIds.length === 0,
  };
}

function rejectHold(res, scope) {
  if (!scope || !scope.user || scope.user.status !== 'hold') return false;
  sendJson(res, 403, { error: 'Account is on hold.', status: 'hold' });
  return true;
}

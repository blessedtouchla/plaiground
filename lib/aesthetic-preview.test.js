...
function run() {
  ...
  [upload, attest, splits, review, dashboard, releases, settings, boosts, artists, index, how, howIn, faq, problem].forEach(function (html) {
    assert.ok(html.includes('site.css?v=' + STAMP), 'touched page cache-busts site.css at ' + STAMP);
    assert.ok(html.includes('family=Space+Grotesk') || html === upload, 'Space Grotesk loads');

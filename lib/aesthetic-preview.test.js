...
const FLOW_STAMP = '20260912ae1';
const DRAWER_STAMP = '20260917pl2';
const ARTISTS_STAMP = '20260922ar3';
const PUBLIC_NAV_STAMP = '20260924p2';
...
function run() {
  ...
  assert.ok(splits.includes('site.css?v=' + FLOW_STAMP), 'flow/public page cache-busts site.css at ' + FLOW_STAMP);
  assert.ok(how.includes('site.css?v=' + PUBLIC_NAV_STAMP), 'How it works cache-busts site.css at the public nav Tesla stamp');
  [splits, how].forEach(function (html) {
    assert.ok(html.includes('family=Space+Grotesk'), 'Space Grotesk loads');
    assert.ok(html.includes('family=Inter'), 'Inter stays loaded');
  });
  assert.ok(attest.includes('site.css?v=20260917at3'), 'Attest Tesla energy cache-busts site.css at 20260917at3');
  assert.ok(!attest.includes('site.css?v=20260917at2'), 'at2 stamp is retired after the quiet-line leftover');
  assert.ok(!attest.includes('site.css?v=20260917at1'), 'at1 stamp is retired after the in-flow helper leftover');
  assert.ok(!attest.includes('site.css?v=20260912ae1'), 'ae1 stamp is retired on Attest after the Tesla energy pass');
  assert.ok(attest.includes('family=Space+Grotesk'), 'Attest still loads Space Grotesk');
  assert.ok(attest.includes('family=Inter'), 'Attest still loads Inter');
  assert.ok(index.includes('site.css?v=' + PUBLIC_NAV_STAMP), 'homepage public-nav cache-busts site.css at ' + PUBLIC_NAV_STAMP);
  assert.ok(!index.includes('site.css?v=20260912ae1'), 'ae1 stamp is retired on homepage after the Tesla energy pass');
  assert.ok(index.includes('family=Space+Grotesk'), 'homepage still loads Space Grotesk');
  assert.ok(index.includes('family=Inter'), 'homepage still loads Inter');
  assert.ok(review.includes('site.css?v=20260917r2'), 'Review Tesla energy cache-busts site.css at 20260917r2');
  assert.ok(!review.includes('site.css?v=20260917r1'), 'r1 stamp is retired after the sticky/soft-copy leftover');
  assert.ok(!review.includes('site.css?v=20260912ae1'), 'ae1 stamp is retired on Review after the Tesla energy pass');
  assert.ok(review.includes('family=Space+Grotesk'), 'Review still loads Space Grotesk');
  assert.ok(review.includes('family=Inter'), 'Review still loads Inter');
  assert.ok(faq.includes('site.css?v=' + PUBLIC_NAV_STAMP), 'FAQ cache-busts site.css at the public nav Tesla stamp');
  [dashboard, releases, settings, boosts, howIn, problem].forEach(function (html) {
    assert.ok(html.includes('site.css?v=' + DRAWER_STAMP), 'signed-in drawer page cache-busts site.css at ' + DRAWER_STAMP);
    assert.ok(html.includes('family=Space+Grotesk'), 'Space Grotesk loads');
    assert.ok(html.includes('family=Inter'), 'Inter stays loaded');
  });
  assert.ok(artists.includes('site.css?v=' + ARTISTS_STAMP), 'Artist Profiles cache-busts site.css at ' + ARTISTS_STAMP);
  assert.ok(artists.includes('family=Space+Grotesk'), 'Artist Profiles loads Space Grotesk');
  assert.ok(artists.includes('family=Inter'), 'Artist Profiles loads Inter');
  ...
  assert.ok(submitted.indexOf('Back to Overview') < submitted.indexOf('data-have-problem'), 'Submitted Have a problem? sits after the primary CTA');

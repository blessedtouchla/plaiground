...
function run() {
  ...
  assert.ok(accept.ACCEPT.indexOf('audio/*') === -1, 'ACCEPT must not use audio/* (iOS Photo Library sheet)');
  assert.ok(!/audio\//.test(accept.ACCEPT), 'ACCEPT is extension-only — audio MIME opens iOS Photo Library / Take Video');
  assert.ok(!/\*\/\*/.test(accept.ACCEPT), 'ACCEPT must not use */*');
  assert.ok(!/image|video/i.test(accept.ACCEPT), 'ACCEPT must not accept images or video');
  assert.ok(!/\.mpeg/.test(accept.ACCEPT), '.mpeg is a video extension on iOS Take Video');
  assert.ok(accept.ACCEPT.indexOf('.mp3') !== -1);
  assert.ok(accept.ACCEPT.indexOf('.wav') !== -1);
  assert.ok(accept.ACCEPT.indexOf('.flac') !== -1);
  assert.ok(!/m4a|aac|ogg|ffmpeg|aiff|aif/i.test(accept.ACCEPT));

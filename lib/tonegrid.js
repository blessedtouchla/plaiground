...
function storeForwardTimeoutMs(converted, startedAt) {
  if (!converted) return STORE_FORWARD_TIMEOUT_MS;
  const used = Date.now() - Number(startedAt || Date.now());
  return Math.max(8000, STORE_FORWARD_TIMEOUT_MS - Math.max(0, used));
}

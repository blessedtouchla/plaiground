...
const INACTIVE_STORE_COPY = 'YouTube Music is not available. Choose other stores and submit again.';
...
const ARTIST_EXISTS_COPY = 'This artist is already on the store.';
...
function sanitizePartnerCopy(text) {
  ...
  if (isInactiveDspError(raw)) return INACTIVE_STORE_COPY;
  ...
  if (/\bslug\b/i.test(raw) && /taken|already|exist|duplicate/i.test(raw)) return ARTIST_EXISTS_COPY;
  return raw
...
function documentedStores() {
  return selectableDspSlugs(DOCUMENTED_DSPS);
}

function normalizeStoreKey(value) {
  return normalizeDspSlug(value).replace(/[\s_]+/g, '-');
}

function isExcludedDspSlug(value) {
  const slug = normalizeStoreKey(value);
  return slug === YOUTUBE_MUSIC_SLUG || slug === 'youtubemusic';
}

function isExcludedStore(row) {
  if (typeof row === 'string') return isExcludedDspSlug(row);
  if (!row || typeof row !== 'object') return false;
  if (isExcludedDspSlug(row.slug || row.dsp || row.dsp_slug || '')) return true;
  const name = String(row.name || row.title || '').trim();
  return Boolean(name) && isExcludedDspSlug(name);
}
...
function selectableDspSlugs(slugs) {
  const out = [];
  const seen = new Set();
  (Array.isArray(slugs) ? slugs : []).forEach((value) => {
    const slug = normalizeDspSlug(value);
    if (!slug || seen.has(slug) || isExcludedDspSlug(slug)) return;
    seen.add(slug);
    out.push(slug);
  });
  return out;
...
// Hold leftover: youtube-music is inactive / not found. Never inject it.
function withYouTubeMusic(slugs) {
  return selectableDspSlugs(slugs);
}
...
function storeRowFrom(row) {
  if (!isSelectableStore(row)) return null;
  ...
  if (!slug || isExcludedDspSlug(slug)) return null;
  const name = String(row.name || row.title || '').trim();
  return {
    slug: slug,

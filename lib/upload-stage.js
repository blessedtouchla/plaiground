...
(function (root, factory) {
  ...
  function missingCopy(state) {
    ...
    if (!state.credits) {
      if (!filled(state.title)) return 'Type the song title.';
  ...
  var FILE_ONLY_AUDIO = '.mp3,.wav,.flac';

  function lockAudioPicker(input) {
    if (!input) return;
    try { input.removeAttribute('capture'); } catch (err) {}
    var g = (typeof globalThis !== 'undefined' && globalThis)
      || (typeof window !== 'undefined' && window)
      || null;
    var helper = g && g.PlaigroundAudioAccept;
    var list = (helper && helper.ACCEPT) || FILE_ONLY_AUDIO;
    if (/audio\/|video\/|image\/|\*\/\*/i.test(list)) list = FILE_ONLY_AUDIO;
  ...
  function openExistingPicker(kind, doc) {
    ...
    if (kind !== 'cover') lockAudioPicker(input);
    pickerOpening = true;
    try {
  ...
  function isProtectedHit(node) {
    while (node && node !== (typeof document !== 'undefined' ? document : null)) {
      ...
      if (node.getAttribute && (
        ...
        || node.getAttribute('data-audio-clear') != null

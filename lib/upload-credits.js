...
(function (root) {
  ...
  function songwriterName(doc) {
    ...
    if (modeOf() === 'create') return val('tg-artist-new', doc);
  ...
  function hasLegalInputs(doc) {
    doc = doc || root.document;
    if (modeOf() === 'create') {
      return Boolean(doc && (doc.getElementById('tg-legal-first-create') || doc.getElementById('tg-legal-last-create')));
    }
    return Boolean(doc && (doc.getElementById('tg-legal-first') || doc.getElementById('tg-legal-last')));
  }

  function persistLegal(doc) {
    if (!credits) return;
    var legal = legalInputs(doc);
    var artist = selectedArtist();
    var saved = artistSavedLegal(artist);
    var mode = modeOf();
    var typed = Boolean(legal.first || legal.last);
    var first = legal.first;
    var last = legal.last;
    ...
    if (typed || hasLegalInputs(doc) || first || last) {
      legalPatch.legal_first = first;
      legalPatch.legal_last = last;
    }
    ...
    if (first && last) {
      credits.rememberLegal(first, last, root);
      credits.writeLegalToArtist(artist && (artist.id || artist.artist_id), first, last, root);
    }
  }

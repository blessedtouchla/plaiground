...
(function (root) {
  ...
  function lockLabel(doc) {
    fillVal('tg-label', 'PLAIGROUND', doc);
  }

  function copyrightYear(doc) {
    var draft = credits && credits.readDraft ? credits.readDraft(root) : {};
    var raw = String((draft && draft.release_date) || '').trim();
    var match = raw.match(/^(\d{4})/);
    if (match) return match[1];
    return String((root.Date ? new root.Date() : new Date()).getFullYear());
  }

  function paintCopyright(doc) {
    lockLabel(doc);
    var legal = legalInputs(doc);
    var year = copyrightYear(doc);
    var name = [legal.first, legal.last].filter(Boolean).join(' ');
    var line = name
      ? ('© ' + year + ' ' + name + ' / ℗ ' + year + ' ' + name)
      : ('© ' + year + ' / ℗ ' + year);
    fillVal('tg-copyright-line', line, doc);
  }
  ...
  function bind(doc) {
    ...
    var label = doc.getElementById('tg-label');
    if (label && label.addEventListener) {
      label.addEventListener('input', function () { lockLabel(doc); });
      label.addEventListener('change', function () { lockLabel(doc); });
    }

...
function run() {
  ...
  return hop.put('audio', fat, { fetch: fetchFn }).then(function (key) {
    ...
    assert.ok(src.includes('xhr.abort'));
    assert.ok(src.includes('AbortController'));
    assert.ok(src.includes("The audio upload timed out. Try again."));
    assert.strictEqual(hop.PUT_MS, 90000);
    assert.strictEqual(hop.PUT_HARD_MS, 15 * 60 * 1000);
    assert.strictEqual(hop.AUDIO_TIMEOUT, 'The audio upload timed out. Try again.');

    const hangStarted = Date.now();
    return hop.put('audio', fat, {
      fetch: function hangPut(url, init) {
        if (String(url) === hop.UPLOADS_URL) return fetchFn(url, init);
        return new Promise(function () {});
      },
      timeoutMs: 40,
    }).then(function () {
      assert.fail('hung PUT must not resolve');
    }, function (err) {
      assert.ok(Date.now() - hangStarted < 1000, 'iOS-safe watchdog must abort a hung PUT');
      assert.strictEqual(err && err.result && err.result.timedOut, true);
      assert.strictEqual(err && err.result && err.result.uploadTimedOut, true);
      assert.match(String((err && err.message) || ''), /audio upload timed out/i);
      assert.doesNotMatch(String((err && err.message) || ''), /could not reach the store/i);
    }).then(function () {
      const pages = ['upload.html', 'song.html', 'review.html'].map((name) => fs.readFileSync(path.join(__dirname, '..', name), 'utf8'));
      pages.forEach((html) => {

 ...
 * Stripe does not follow redirects. Apex POST must not 308 to www.
 * Events:
...
const { recordPaid } = require('./growth-events');

async function notifyPaid(user, plan) {
  if (!user || (plan !== 'creator' && plan !== 'pro')) return;
  try {
    await recordPaid(user, plan);
  } catch {
    /* paid event must not block the existing webhook writer */
  }
}
...
async function applyPaidSessionToAccount(sessionId, user, retrieve) {
  ...
  await notifyPaid(next || user, plan);
  return {
    applied: true,
    plan,
    status: 'active',
...
async function writePlan(obj, plan, extra) {
  ...
  if (status === 'active') await notifyPaid(next || user, plan);
  return {
    applied: true,
    plan,
    status,
...
module.exports = {
  ...
  recoverPaidPlan,
  resolvePaidPlan,
  retrieveStripe: defaultRetrieve,
  signStripePayload,

 ...
 * write that plan on the user, and drop to Basic when the subscription is gone.
 *
...
const { normalizePaidPlan } = require('./auth');
...
async function writePlan(obj, plan, extra) {
  ...
  return { applied: true, plan, userId: user.id, email: next && next.email };
}
...
async function applyStripeEvent(event, options) {
  ...
  if (type === 'checkout.session.completed') {
    ...
    return writePlan(obj, plan, { sessionId: obj.id, customerId: customerIdOf(obj) });
  }
  ...
  if (type === 'invoice.paid') {
    ...
    const plan = await resolvePaidPlan(obj, retrieve);
    if (!plan) return { applied: false, reason: 'unknown_price' };
    return writePlan(obj, plan, { customerId: customerIdOf(obj) });
  }

  if (type === 'customer.subscription.updated') {
    const status = String(obj.status || '');
    if (INACTIVE_STATUSES[status]) {
      return writePlan(obj, 'basic', { customerId: customerIdOf(obj) });
    }

...
function run() {
  ...
  assert.strictEqual(filled.title.value, 'Mexeu', 'Cancel does not wipe the form just to stay');
  assert.strictEqual(JSON.parse(filled.localStorage.getItem(leave.DRAFT_KEY)).title, 'Mexeu');
  assert.strictEqual(filled.confirms.length, 0, 'Cancel does not ask to wipe');

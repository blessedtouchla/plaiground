...
function run() {
  ...
  assert.ok(!/data-upload-save-draft/.test(upload), 'Save draft control is cancelled');
  assert.ok(!/Save draft/.test(upload), 'Save draft copy is gone');
  assert.ok(upload.indexOf('upload-continue-row') !== -1, 'Continue stays on phone');
  assert.ok(upload.indexOf('upload-continue-row') < upload.indexOf('data-store-continue'), 'Continue stays in the continue row');
  assert.ok(!/Delete draft/.test(upload), 'must not add a Delete draft control');
  assert.ok(/data-upload-start-over[^>]*aria-label="Start over"/.test(upload), 'Start over stays Start over');
  assert.ok(/data-upload-cancel[^>]*aria-label="Cancel"/.test(upload), 'Cancel stays the live secondary control');
  assert.ok(/data-upload-start-over[^>]*aria-label="Start over"/.test(upload), 'Start over accessible name is exact');
  assert.ok(upload.indexOf('Go to Artist Profiles') !== -1, 'Import note offers Go to Artist Profiles');
  ...
  assert.ok(upload.indexOf('lib/upload-draft-files.js') !== -1, 'submit flow still keeps picked files');
  assert.ok(upload.indexOf('upload-leave-actions') !== -1);
  assert.ok(review.indexOf('data-upload-start-over') !== -1, 'Review matches Upload leave icons');
  assert.ok(read('attest.html').indexOf('data-upload-start-over') !== -1, 'Attest matches Upload leave icons');
  assert.ok(read('split-sheet.html').indexOf('data-upload-start-over') !== -1, 'Splits matches Upload leave icons');
  assert.ok(/\.upload-leave-actions\s*\{[\s\S]*?flex-wrap:\s*wrap/.test(css), 'phone actions wrap instead of overlap');
  assert.ok(/\.upload-continue-row\s*\{[\s\S]*?display:\s*flex/.test(css), 'phone Continue row stays visible');
  assert.ok(store.indexOf("leaveAfterCancel('dashboard.html')") !== -1, 'store-client Cancel lands on Overview');
  ...
  assert.ok(String(keep.win.location.href).indexOf('upload.html?new=1') !== -1, 'Start over does not wait on a leftover confirm');

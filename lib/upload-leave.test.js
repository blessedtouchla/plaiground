...
function run() {
  ...
  assert.ok(upload.indexOf('upload-continue-row') !== -1, 'Save draft is next to Continue for phone');
  assert.ok(upload.indexOf('upload-continue-row') < upload.indexOf('data-store-continue'), 'Save draft in the continue row is left of Continue');
  ...
  assert.ok(/\.upload-continue-row\s*\{[\s\S]*?display:\s*flex/.test(css), 'phone Continue row keeps Save draft visible');
  assert.ok(/\[data-upload-save-draft\][\s\S]*?visibility:\s*visible/.test(css), 'Save draft stays visible on phone');

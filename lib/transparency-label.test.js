...
function run() {
  ...
  assert.ok(hero[0].includes('Upload once.'), 'existing homepage headline stays');
  assert.ok(/Get paid <span class="accent">everywhere\.<\/span>/.test(hero[0]), 'existing paid-everywhere line stays');

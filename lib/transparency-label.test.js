...
function run() {
  ...
  assert.ok(/<h1>AI-powered music business for independent artists\.<\/h1>/.test(hero[0]), 'homepage headline is an AI-powered music business');
  assert.ok(/A&amp;R, press kits, pitching, and distribution, powered by AI and checked by people\./.test(hero[0]), 'homepage sub names distribution with the rest of the work');
  assert.ok(!/when you're ready|when you’re ready/i.test(index), 'distribution is not framed as a later step');
  assert.ok(!/\$2\.49/.test(hero[0]), 'homepage hero does not lead with the per-song price');
  assert.ok(/class="coming-soon">Coming soon</.test(index), 'coming soon is one removable class');
  assert.ok(/href="\/ar"/.test(index) && /href="\/epk"/.test(index), 'A&R and EPK tiles link to the public pages');

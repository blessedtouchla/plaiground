...
function shouldCheckOmissions(status) {
  var s = normalize(status);
  return Boolean(PENDING[s] || PROCESSING[s] || NEEDS_FIX[s] || QC_REJECTED[s]);
}
...
function dropInventedLines(lines, status) {
  ...
  if (isNeedsFix(status) || group(status) === 'pending') return list.slice();
...
function problemAlert(row, statusHint) {
  ...
  var g = group(status);
  if (g === 'live' || g === 'draft' || g === 'removing' || g === 'store_gone' || g === 'takedown_failed' || g === 'signatures') {
    return '';
  }

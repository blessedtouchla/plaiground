...
function matchingStoreDraft(row, draft) {
  ...
  if (!id || id === 'local-draft') return null;
  if (String(draft.release_id || '').trim().toLowerCase() === id) return draft;
  return null;
}
...
function isUnsubmittedDraft(row, draft) {
  ...
  if (match && (match.submitted === false || match.submitted === 'false')) return true;
  return status === 'draft';
}

...
function dropInventedLines(lines, status) {
  ...
  if (isNeedsFix(status) || isQcRejected(status)) return list.slice();
...
function problemAlert(row, statusHint) {
  ...
  var parts = [];
  if (msg && !isLeftoverQcLine(msg)) parts.push(msg);
...
function displayInfo(row, statusHint) {
  ...
  if (alert && !mapped.live && (mapped.group === 'needs_fix' || mapped.group === 'qc_rejected')) {
    var qc = mapped.group === 'qc_rejected';
    return {
      status: mapped.status,
      group: mapped.group,
      label: qc ? 'QC rejected' : 'Needs fix',
      dot: 'red',
      live: false,
      alert: alert,
    };

...
function dot(status) {
  ...
  if (g === 'pending' || g === 'processing' || g === 'removing') return 'yellow';
  return 'gray';
}
...
function isKnownPipeline(status) {
  var s = normalize(status);
  return Boolean(LIVE[s] || PROCESSING[s] || PENDING[s] || NEEDS_FIX[s] || QC_REJECTED[s] || DRAFT[s]
...
function problemAlert(row, statusHint) {
  ...
  var g = group(status);
  if (g === 'live' || g === 'draft' || g === 'removing' || g === 'store_gone' || g === 'takedown_failed' || g === 'signatures' || g === 'pending' || g === 'processing') {
    return '';
  }
...
function isHiddenFromList(status) {
  return group(status) === 'store_gone';
}

function isPendingPipeline(card) {
  if (!card || card.live) return false;
  var g = card.group || group(card.status);
  return g === 'pending' || g === 'processing' || g === 'needs_fix' || g === 'qc_rejected' || g === 'removing';
}
...
function matchingStoreDraft(row, draft) {
  draft = draft && typeof draft === 'object' ? draft : null;
  if (!draft) return null;
  var id = String((row && (row.uuid || row.id)) || '').trim().toLowerCase();
  if (id && id !== 'local-draft' && String(draft.release_id || '').trim().toLowerCase() === id) return draft;
  var rowTitle = String((row && row.title) || '').trim().toLowerCase();
  var draftTitle = String(draft.title || '').trim().toLowerCase();
  if (rowTitle && draftTitle && rowTitle === draftTitle) return draft;
  return null;
}

function draftMarkedSubmitted(draft) {
  return Boolean(draft && (draft.submitted === true || draft.submitted === 'true'));
}
...
function isUnsubmittedDraft(row, draft) {
  ...
  if (g === 'live' || g === 'processing' || g === 'needs_fix' || g === 'qc_rejected' || g === 'removing' || g === 'takedown_failed') {
    return false;
  }
  var match = matchingStoreDraft(row, draft);
  if (!match || draftMarkedSubmitted(match)) return false;
  if (match.submitted === false || match.submitted === 'false') return true;
  return normalize(match.tonegrid_status) === 'draft';
}

function filledResumeValue(value) {
  return String(value == null ? '' : value).trim() !== '';
}

function uploadStepComplete(row, draft) {
  var match = matchingStoreDraft(row, draft);
  var src;
  if (match) src = Object.assign({}, row || {}, match);
  else if (row && (row.local_draft === true || row.id === 'local-draft') && draft) src = Object.assign({}, row, draft);
  else src = row || {};
  var title = filledResumeValue(src.title);
  var name = filledResumeValue(src.name || src.artist);
  var genre = filledResumeValue(src.genre);
  var hasAudio = Boolean(
    src.audio_uploaded === true
    || src.audio_uploaded === 'true'
    || src.audio_attached === true
    || filledResumeValue(src.audio_name)
    || filledResumeValue(src.audio_url)
    || filledResumeValue(src.audio_object_key)
    || releaseHasAudioHint(src)
  );
  var hasArt = Boolean(
    filledResumeValue(src.artwork_name)
    || filledResumeValue(src.artwork_url)
    || filledResumeValue(src.artwork_object_key)
    || filledResumeValue(src.cover_art_url)
    || filledResumeValue(src.cover_url)
  );
  return Boolean(title && name && genre && hasAudio && hasArt);
}

function releaseHasAudioHint(row) {
  var tracks = tracksOf(row);
  var i;
  for (i = 0; i < tracks.length; i += 1) {
    var track = tracks[i] || {};
    if (filledResumeValue(track.audio_url) || filledResumeValue(track.audio_s3_key) || filledResumeValue(track.s3_key) || Number(track.file_size) > 0) {
      return true;
    }
  }
  return false;
...
function rowId(row) {
  var id = String((row && (row.uuid || row.id)) || '').trim();
  if (!id || id === 'local-draft') return '';
  return id;
}

function resumeHref(row, draft) {
  if (isUnsubmittedDraft(row, draft)) {
    return uploadStepComplete(row, draft) ? 'attest.html' : 'upload.html';
  }
  var id = rowId(row);
  return id ? ('song.html?id=' + encodeURIComponent(id) + '&edit=1') : 'releases.html';
}

function openHref(row, draft) {
  if (isUnsubmittedDraft(row, draft)) return resumeHref(row, draft);
  var id = rowId(row);
  return id ? ('song.html?id=' + encodeURIComponent(id)) : 'releases.html';
}
...
function ownedReleases(me) {
  ...
  for (i = 0; i < list.length; i += 1) {
    ...
    if (isHiddenFromList(row.tonegrid_status || row.status)) {
...
var api = {
  ...
  matchingStoreDraft: matchingStoreDraft,
  openHref: openHref,
  resumeHref: resumeHref,
  uploadStepComplete: uploadStepComplete,
  label: label,
  liveCount: liveCount,

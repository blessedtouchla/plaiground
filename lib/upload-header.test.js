...
function run() {
  ...
  assert.ok(upload.includes('site.css?v=20260912sg4'), 'header paint cache-busts site.css');
  ...
  assert.ok(upload.indexOf('class="stepper"') < upload.indexOf('data-upload-heading'), 'quiet top steps sit above the Upload page label');
  ...
  assert.ok(/data-upload-heading>Upload</.test(upload), 'Upload stays a page label');
  assert.ok(/\.upload-page \.upload-page-label[\s\S]*?font-size:\s*13px/.test(headerCss), 'Upload is quieter than the hero');
  assert.ok(/\.upload-page \.upload-page-label[\s\S]*?color:\s*var\(--muted\)/.test(headerCss), 'Upload is a muted step/page label');

...
function run() {
  ...
  assert.ok(upload.includes('site.css?v=20260912sg1'), 'header paint cache-busts site.css');
  ...
  assert.ok(headerCss.includes('"Space Grotesk"'), 'titled words and pills use Space Grotesk');
  assert.ok(/\.upload-page \.upload-title[\s\S]*?font-family:\s*"Space Grotesk"/.test(headerCss), 'Upload title uses Space Grotesk');
  ...
  assert.ok(/\.upload-page \.upload-title[\s\S]*?font-weight:\s*700/.test(headerCss), 'Upload title weight matches the pills');

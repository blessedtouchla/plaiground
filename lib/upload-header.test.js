...
function stepperBlock(html) {
  ...
  assert.ok(end !== -1, 'Upload stepper is a nav under the title');
...
function run() {
  ...
  const headerCssStart = css.indexOf('/* —— Upload header stage (dots + type line + tiny leave) —— */');
  ...
  assert.ok(upload.includes('site.css?v=20260912h1'), 'header paint cache-busts site.css');
  ...
  assert.ok(!/<section class="flow-card upload-quiet-card" data-release-type>/.test(upload), 'Release type card is soft-stopped');
  ...
  assert.ok(headerCss.includes('.upload-page .st.on'), 'header stepper paint is scoped to Upload');
  assert.ok(headerCss.includes('background: transparent'), 'Upload steps are not purple cards');
  assert.ok(headerCss.includes('content: "·"') || headerCss.includes("content: '·'"), 'steps use middot separators');
  assert.ok(headerCss.includes('border-radius: 50%'), 'steps use dots');

...
function run() {
  ...
  assert.ok(upload.includes('site.css?v=20260912ba3'), 'brand accents cache-busts site.css at 20260912ba3');
  ...
  assert.ok(/\.upload-page \.upload-type-toggle a\.on[\s\S]*?background:\s*var\(--stage-gold\)/.test(brandCss), 'selected Single | Album is Gold fill');
  ...
  const coverChip = (brandCss.match(/\.upload-page \.stage-chip\.chip-cover \{[\s\S]*?\n\}/) || [])[0] || '';
  assert.ok(/var\(--stage-magenta\)/.test(audioChip), 'Add audio is Magenta outline+text');
  assert.ok(!/var\(--stage-purple\)/.test(audioChip), 'Add audio is not purple-clumped with the active step');
  assert.ok(/var\(--muted-2\)/.test(coverChip), 'Add cover is a quieter muted outline');
  assert.ok(!/var\(--stage-gold\)/.test(coverChip), 'Add cover does not twin the Gold toggle');
  assert.ok(!/var\(--stage-magenta\)/.test(coverChip), 'Add cover does not twin Add audio magenta');
  assert.ok(brandCss.includes('var(--stage-purple)'), 'progress purple token stays in the accent system');
  assert.ok(brandCss.includes('var(--stage-gold)'), 'toggle gold token stays in the accent system');
  assert.ok(brandCss.includes('var(--stage-magenta)'), 'magenta is the Add audio job');

...
function applyStaffPlan(row) {
  // Late require: auth.js already depends on this file.
  const auth = require('./auth');
  if (typeof auth.applyStaffPlan === 'function') return auth.applyStaffPlan(row);
  return row;
}

function evaluate(row, now, options) {
  const view = applyStaffPlan(row);
  const onHold = String((view && view.status) || '').trim().toLowerCase() === 'hold';
  const plan = onHold ? 'basic' : (normalizePlan(view && view.plan) || 'basic');
  const ids = uniqueReleaseIds(row);

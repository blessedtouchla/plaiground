...
const PUBLISHING_ERROR = 'Publishing registration is included with Creator and Pro.';
const BOOST_PLAN_ERROR = 'Marketing Boost is included with Creator and Pro.';
const BOOST_UNAVAILABLE = 'Marketing Boost is not for sale.';
const PAID_EXTRAS = { creator: true, pro: true };
...
function canUseExtras(plan) {
  return Boolean(PAID_EXTRAS[normalizePlan(plan)]);
}

function featureBody(feature, plan) {
  const next = normalizePlan(plan) || 'basic';
  const publishing = feature === 'publishing';
  return {
    error: publishing ? PUBLISHING_ERROR : BOOST_PLAN_ERROR,
    code: 'PLAN_FORBIDDEN',
    feature: publishing ? 'publishing' : 'boost',
    plan: next,
    upgrade: {

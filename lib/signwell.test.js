...
async function run() {
  ...
  await withSignwell({ key: 'signwell-test-key-not-for-commit', template: 'tpl_test_not_for_commit' }, async () => {
    ...
    try {
      ...
      assert.strictEqual(json(res).code, 'solo_not_signwell');
      assert.ok(json(res).error.indexOf('attested') !== -1);
      assert.strictEqual(createBody, null, '1-writer POST must not call create-from-template');
      const direct = await signwellApi.createSplitDocument({
        songTitle: 'Live Gate Song',
        writers: [{ name: 'Writer One', email: 'writer1@example.com', share: 100 }],
      });
      assert.strictEqual(direct.ok, false);
      assert.strictEqual(direct.status, 400);
      assert.strictEqual(direct.data.code, 'solo_not_signwell');
      assert.strictEqual(createBody, null, 'createSplitDocument must not mint for one writer');
    ...
    try {
      ...
      await signwellApi({
        ...
        body: {
          songTitle: 'Live Gate Song',
          writers: [
            { name: 'Writer One', email: 'writer1@example.com', share: 50, pro: '' },
            { name: 'Writer Two', email: 'writer2@example.com', share: 50, pro: '' },
          ],
        },
      }, res);
      assert.strictEqual(res.statusCode, 400);
  ...
  assert.ok(!signwellApiSrc.includes('exclude_placeholders'), '1-writer Writer 2 exclude path is gone');
  assert.ok(signwellApiSrc.includes('solo_not_signwell'));
  assert.ok(!signwellApiSrc.includes('NEXT_PUBLIC_'));
  assert.ok(split.includes('payload.writers.length < 2'), 'split-sheet.html refuses a 1-writer mint');
  assert.ok(!split.includes('Publishing Agreement'));
  const terms = fs.readFileSync(path.join(__dirname, '..', 'terms.html'), 'utf8');
  assert.ok(!terms.includes('document_id'));
  assert.ok(!terms.includes('as described below'), 'Publishing Agreement must not point at SignWell');
  assert.ok(!terms.includes('Split sheets and the Publishing Agreement are signed electronically through SignWell'));
  assert.ok(terms.includes('People listed must sign (electronic signature OK) before we complete publishing registration or, where we require it, before delivery.'));
  assert.ok(!terms.includes('Multi-writer split sheets are signed electronically through SignWell.'), 'Terms must not route the Publishing Agreement through SignWell');
  assert.ok(!terms.includes('One-writer 100% ownership is attested in the app and is not sent to SignWell.'));
  assert.ok(terms.includes('A signed <strong>Publishing Agreement</strong> is required before PLAIGROUND claims any publisher'));
  assert.ok(terms.includes('These Terms alone do not transfer that share.'));

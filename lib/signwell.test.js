...
async function run() {
  ...
  assert.ok(terms.includes('Multi-writer split sheets are signed electronically through SignWell.'));
  assert.ok(terms.includes('One-writer 100% ownership is attested in the app and is not sent to SignWell.'));
  assert.ok(terms.includes('We may refuse to deliver a work until every required co-writer has signed.'));
  assert.ok(terms.includes('PLAIGROUND claims that share only after a Publishing Agreement for that work is signed.'));

...
function catalogRows(list) {
  var pick = storePick();
  if (pick && pick.selectableStores) return pick.selectableStores(list);
  if (pick && pick.normalizeStores) {
    return pick.normalizeStores(list).filter(function (row) {
      return pick.isSelectableStore ? pick.isSelectableStore(row) : true;
    });
  }
  return [];
}
...
function fallbackCatalog() {
  var pick = storePick();
  return catalogRows(pick && pick.DEFAULT_STORES ? pick.DEFAULT_STORES : []);
}

function setCatalog(list) {
  if (list && list.length) {
    liveCatalog = catalogRows(list);
...
var HINT_LEAD = ['spotify', 'apple-music', 'amazon-music', 'deezer', 'tidal'];
var MAX_HINT_STORES = 10;
...
function hintSlugs() {
  ...
  function push(slug) {
    var found = findPlatform(slug);
    var key = found ? found.slug : trim(slug).toLowerCase();
    if (!key || seen[key]) return;
    seen[key] = true;
    out.push(key);
  }
  HINT_LEAD.forEach(push);
  platformList().forEach(function (row) {
    if (out.length >= MAX_HINT_STORES) return;
    push(row.slug);
  });
  return out.slice(0, MAX_HINT_STORES);
}

function hasStoreHint(slug) {
  var found = findPlatform(slug);
  var key = found ? found.slug : trim(slug).toLowerCase();
  return hintSlugs().indexOf(key) !== -1;
}

function urlPlaceholder(slug) {
  if (!hasStoreHint(slug)) return 'Artist URL';
  var found = findPlatform(slug);
...
function urlHint(slug) {
  if (!trim(slug)) return 'Pick a platform. Top stores show a short artist URL hint.';
  if (!hasStoreHint(slug)) return 'Paste that store’s public artist page.';
  var found = findPlatform(slug);
...
var api = {
  ...
  HINT_LEAD: HINT_LEAD,
  MAX_HINT_STORES: MAX_HINT_STORES,
  hasStoreHint: hasStoreHint,
  hintSlugs: hintSlugs,
  urlHint: urlHint,

...
function isTypeaheadBound(select) {
  ...
  return select.getAttribute('data-typeahead') === 'on' && !!typeaheadInput(select);
}

// One visible type-to-autofill field. The native select stays in the
// DOM for the form value and is hidden with is-typeahead-source.
// Phone and desktop share this overlay so Genre/Language never stack
// a typeable box on a still-visible chevron select.

...
function run() {
  ...
  assert.ok(upload.indexOf('id="tg-legal-first"') === -1, 'Upload does not duplicate Artist Profiles legal first');
  assert.ok(upload.indexOf('id="tg-legal-last"') === -1, 'Upload does not duplicate Artist Profiles legal last');
  assert.ok(upload.indexOf('id="tg-legal-first-create"') === -1, 'Upload create does not collect a second legal first');
  assert.ok(upload.indexOf('id="tg-legal-last-create"') === -1, 'Upload create does not collect a second legal last');
  assert.ok(upload.indexOf('id="artist-legal-wrap"') === -1, 'Upload does not keep a second legal wrap');
  ...
  assert.ok(!/id="tg-copyright-owner"[^>]*\breadonly\b/.test(upload));
  assert.ok(!/id="tg-phonogram-owner"[^>]*\breadonly\b/.test(upload));
  assert.ok(upload.indexOf('The song.') !== -1);
  assert.ok(upload.indexOf('The recording.') !== -1);
  ...
  assert.ok(upload.indexOf('Optional, helps sync licensing') === -1);
  assert.ok(/id="tg-lyrics-open"[^>]*type="checkbox"/.test(upload));
  assert.ok(upload.indexOf('Add lyrics') !== -1);

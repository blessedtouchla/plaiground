...
function run() {
  ...
  assert.ok(/class="release-lede"/.test(dash), 'catalog line and Pending share a lede so they do not wrap the pills');
  assert.ok(/class="release-status" data-latest-status>Pending<\/span>/.test(dash), 'Pending stays yellow status text next to the catalog line');
  const pills = dash.match(/<div class="release-pills">[\s\S]*?<\/div>/);
  assert.ok(pills, 'Your releases action pills are grouped');
  assert.ok(!/data-latest-status/.test(pills[0]), 'Pending is not a fourth item in the pill row');
  assert.ok(/data-new-release/.test(pills[0]) && /data-latest-edit/.test(pills[0]) && /data-all-releases/.test(pills[0]), 'pill row keeps All releases, Edit release, and New release');
  const siteCss = read('site.css');
  assert.ok(/\.release-pills \{\s*[\s\S]*?flex-wrap:\s*nowrap/.test(siteCss), 'phone Your releases pills stay on one line');
  assert.ok(siteCss.includes('.release-pills [data-all-releases] { order: 1; }'), 'phone order starts with All releases');
  assert.ok(siteCss.includes('.release-pills [data-latest-edit] { order: 2; }'), 'phone order keeps Edit release in the middle');
  assert.ok(siteCss.includes('.release-pills [data-new-release] { order: 3; }'), 'phone order ends with New release');
  assert.ok(/\.release-pills \.btn \{\s*[\s\S]*?padding:\s*6px 8px/.test(siteCss), 'phone pills shrink so three fit at ~390px');

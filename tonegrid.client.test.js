...
async function run() {
  ...
  assert.strictEqual(lyricsPage.lyricsField.hidden, false, 'Add lyrics toggle must open the textarea');
  ...
  payNoCover.payBtn.listeners.click({ preventDefault() {} });
  await flush(6);
  assert.strictEqual(payNoCover.status.textContent, 'Cover art is required.');
  assert.strictEqual(payNoCover.status.hidden, false);
  assert.ok(payNoCover.status.classList.contains('upload-status-error'));
  assert.ok(!payNoCover.calls.some(function (call) { return call.url === '/api/tonegrid/releases'; }), 'missing cover must not mint');

  const payHeldCoverMints = load({
    bind: 'review',
    reviewCover: true,
    releaseDate: '2026-09-20',
    heldFile: AUDIO,
  ...
  await flush(4);
  assert.ok(payHeldCoverMints.reviewCover.classList.contains('has-art'), 'Review hydrates cover from the held Upload file');
  payHeldCoverMints.payBtn.listeners.click({ preventDefault() {} });
  await flush(16);
  assert.notStrictEqual(payHeldCoverMints.status.textContent, 'Cover art is required.');
  assert.ok(payHeldCoverMints.calls.some(function (call) {
    return call.url === '/api/tonegrid/releases';
  }), 'held master+cover must mint a new release_id');
  assert.ok(payHeldCoverMints.calls.some(function (call) {
    return call.url === '/api/tonegrid/tracks';
  }), 'held master+cover must create a track');

  const paySessionCover = load({
    bind: 'review',
    releaseDate: '2026-09-20',
    heldFile: AUDIO,
    artist: 'Ada Night',
    ...
    sessionDraft: Object.assign(attestDraft(), {
      ...
      title: 'Night Drive',
      artwork_name: 'cover.jpg',
      artwork_url: 'https://cdn.example/from-session.jpg',
  ...
  paySessionCover.payBtn.listeners.click({ preventDefault() {} });
  await flush(16);
  assert.strictEqual(draftOf(paySessionCover.localStorage).artwork_url, 'https://cdn.example/from-session.jpg');
  assert.ok(paySessionCover.calls.some(function (call) {
    return call.url === '/api/tonegrid/releases';
  }), 'session cover+audio keys must mint a release');
  ...
  assert.ok(uploadHtml.includes('upload-continue-row'), 'Continue stays on phone');
  assert.ok(!/Save draft/.test(uploadHtml), 'Save draft is cancelled');
  assert.ok(!/data-upload-save-draft/.test(uploadHtml), 'Save draft control is gone');
  ...
  assert.ok(uploadHtmlForBust.includes('store-client.js?v=20260912ly3'), 'upload.html cache-busts store-client.js at 20260912ly3');
  assert.ok(!uploadHtmlForBust.includes('site.css?v=20260912ly3'), 'upload.html must cache-bust past 20260912ly3');
  assert.ok(uploadHtmlForBust.includes('site.css?v=20260912ly4'), 'upload.html cache-busts site.css at 20260912ly4');
  assert.ok(uploadHtmlForBust.includes('lib/upload-audio-bind.js?v=20260912ly4'), 'upload.html cache-busts audio bind at 20260912ly4');
  ...
  assert.ok(source.includes('PlaigroundStorePick.selectableStores'), 'Upload/Review catalog must drop inactive and youtube-music');
  assert.ok(source.includes('PlaigroundStorePick.selectableSlugs'), 'Submit must not send youtube-music from leftover draft.dsps');
  assert.ok(source.includes('selectedBlockedStores'), 'Review blocks Submit when an inactive DSP is still checked');
  assert.ok(source.includes('YouTube Music is not available. Choose other stores and submit again.'), 'inactive DSP error is readable');
  assert.ok(source.includes('isInactiveDspError'), 'partner not-found-or-inactive copy must be rewritten');
  assert.ok(source.indexOf('YouTube Music is not available') !== -1 && source.indexOf('ToneGrid is not available') === -1);

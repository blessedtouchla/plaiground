...
- `GET /api/tonegrid/analytics` (session; filtered to that user’s store ids; empty until a release is live)
- `POST /api/tonegrid/webhook` (HMAC if `TONEGRID_WEBHOOK_SECRET` is set; persist real store status)

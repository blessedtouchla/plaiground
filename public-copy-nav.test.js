...
function run() {
  ...
  ['how-it-works.html', 'how.html'].forEach(function (file) {
    ...
    const stepsIdx = html.indexOf('how-cols');
    assert.strictEqual(cols.length, 3, file + ' has exactly three numbered steps');
  ...
  assert.ok(howAppCopy.includes('class="how-who"') && howAppCopy.includes('Who this is for'), 'signed-in how.html matches the public who-this-is-for block');
  assert.ok(/\.how-cols \{[\s\S]*?grid-template-columns:\s*repeat\(3/.test(read('site.css')), 'How it works columns stay three');
  ...
  assert.ok(/data-faq-store-groups/.test(faq) && /data-faq-stores/.test(faq), 'FAQ paints platforms from the live store catalog');
  assert.ok(/paint\(\);/.test(faq), 'FAQ paints the fallback catalog before the live fetch');
  assert.ok(!/paint\(PlaigroundStorePick\.DEFAULT_STORES\)/.test(faq), 'FAQ must not paint the short default picker list');
  assert.ok(/Monetize \(AI\)/.test(faq), 'FAQ has Monetize (AI)');
  assert.ok(/No AI/.test(faq) && /AI-assisted/.test(faq) && /Fully AI/.test(faq), 'Monetize (AI) uses the three attest labels');
  assert.ok(/Full AI tracks are allowed/.test(faq), 'Fully AI tracks are allowed');
  assert.ok(/We still distribute even when a store will not pay/.test(faq), 'we still distribute when a store will not pay');
  assert.ok(/Tidal: will not pay royalties on music they identify as wholly AI-generated/.test(faq), 'Tidal Fully AI pay rule is locked');
  assert.ok(/Deezer: tags fully AI, keeps it out of recs\/playlists/.test(faq), 'Deezer Fully AI rule is locked');
  assert.ok(/We do not promise they will pay every stream/.test(faq), 'no invented all-store pay promise');
  assert.ok(/treated as a human work with AI help/.test(faq), 'AI-assisted is a human work with AI help');
  assert.ok(/No AI follows normal store pay rules/.test(faq), 'No AI follows normal store pay rules');
  assert.ok(/FAQ is current as of August 28, 2026/.test(faq), 'Monetize (AI) is dated');
  assert.ok(/A store can list a song and still not pay/.test(faq), 'listing is not the same as pay');
  assert.ok(/YouTube Content ID is real collection on PLAIGROUND/.test(faq), 'Content ID is real collection');
  assert.ok(/not just sending the song to YouTube Music/.test(faq), 'Content ID is not YouTube Music delivery');
  ...
  assert.ok(/first royalty cycle after a match/.test(faq), 'first pay is the first cycle after a match');
  assert.ok(/collection and monetize, not a views buy/.test(faq), 'Video Collect Monetize is collection');
  assert.ok(/Not the same as buying YouTube views/.test(faq), 'not a YouTube views buy');
  assert.ok(/Views packages stay parked/.test(faq), 'views packages stay parked');
  assert.ok(/<li>YouTube Content ID<\/li>/.test(faq) && /<li>Instagram\/Facebook Meta Rights Manager<\/li>/.test(faq) && /<li>TikTok native-video ID<\/li>/.test(faq), 'Video Collect platforms list is on the page');
  ...
  assert.ok(!js.includes('setupOverviewSubmenu') && !js.includes('isOverviewHomeLink') && !js.includes('data-overview-menu'), 'signed-in JS must not keep the Overview nest');
  assert.ok(js.includes('setupAppBlogLink'), 'Blog injector stays; not invented into Account HTML');
